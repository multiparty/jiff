...
const all_parties = config.compute_parties.concat(config.input_parties);

// Read command line args
const computation_id = 'test';
...
const jiffClient = new JIFFClient('http://localhost:8080', computation_id, {
...
const compute = function () {
  jiffClient.wait_for(all_parties, function () {
    ...
    const shares = jiffClient.share_ND_array(null, skeletons, null, config.compute_parties, config.input_parties);
    ...
    const total_sum = sadd_array(partial_sums);


    /*
     * Compute the inner product
     */
    const pairwise_product = shares[config.input_parties[0]];
    ...
    const inner_product = sadd_array(pairwise_product);


    // The results are returned as an array
    const results = [total_sum, inner_product];

    // Send back the result to any parties that are still connected
    const promise = jiffClient.open_ND_array(results, all_parties, config.compute_parties);
...
jiffClient.wait_for(config.compute_parties, function () {
  if (config.preprocessing !== false) {
    ...
    const num_mults = config.input_parties.length * config.input_length;
