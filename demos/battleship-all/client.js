...
var version; // 1 = index, 2 = binary, 3 = row-column
...
var guesses_len = 5;
var ships_len = 15;
var answer_len = 5; // if its binary, then specifically change to numRow*numCol

var emptyBoard = []; // if its binary, will be filled and copied for guesses and myShips

var myShips = [];
var myShipsCount = 0; // for checking against ships_len

// these reset every turn
var guesses = [];
var guessesCount = 0; // for checking against guesses_len
var canPlay = false;
...
function connect() {
    ...
    switch(version) {
        case 1:
            $('#title').text('Gati\'s BattleShip Game - Index');
            break;
        case 2:
            $('#title').text('Gati\'s BattleShip Game - Binary');
            answer_len = numRows*numCols; // answer array is 64 for binary
            break;
        case 3:
            $('#title').text('Gati\'s BattleShip Game - Row-Column');
            break;
        default:
            $('#title').text('WARNING! INVALID VERSION');
            break;
    }

    // getting computation_id -- now computation_id+version
    var computation_id = $('#computation_id').val() + ' v.' + version;
    ...
    mpc.connect(hostname, computation_id, options, version);
}
...
// creates and stores secret shares of ships, returns jiff_instance's id and stores it to partyID
...
function handleAnswers(result) {
    ...
    let p1_answers = result.splice(0, answer_len);
...
function updateOppoBoard(data) {
    ...
    for(let i = 0; i < data.length; i++) {
        ...
        switch(version) {
            case 1:
                id = '#o_' + guesses[i];
                if(data[i] === 0) match = true;
                break;
            case 2:
                id = '#o_' + i;
                if(data[i] === 1) match = true;
                break;
            case 3:
                id = '#o_' + (guesses[2*i]*(numRows) + guesses[2*i+1]);
        ...
        if(match) { // same for all
            // hits are brownish
            $(id).css("background-color", "Crimson");
            numHitsOnOppo++;
        }
        else if(version !== 2 || (version === 2 && guesses[i] === 1)) {
...
function updateMyBoard(data) {
    ...
    for(let i = 0; i < data.length; i++) {
        ...
        if((version === 2 && data[i] === 1) || (version !== 2 && data[i] === 0)) {
            numHitsOnMe++;
        }
    }
