module.exports = function (jiffClient) {
  ...
  jiffClient.emit = function (tag, receivers, message, encrypt) {
    ...
    for (let p = 0; p < receivers.length; p++) {
      if (receivers[p] === jiffClient.id) {
        continue;
      }

      let message_to_send = { tag: tag, party_id: receivers[p], message: message, encrypted: encrypt };
  ...
  jiffClient.listen = function (tag, handler) {
    ...
    const stored_messages = jiffClient.custom_messages_mailbox[tag];
    if (stored_messages == null) {
      return;
    }

    for (let i = 0; i < stored_messages.length; i++) {
      const sender_id = stored_messages[i].sender_id;
      const message = stored_messages[i].message;
