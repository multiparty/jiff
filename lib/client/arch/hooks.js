...
function Hooks(jiffClient) {
  ...
  const optionHooks = jiffClient.options.hooks || {};
...
Hooks.prototype.generateKeyPair = function (jiffClient) {
  if (jiffClient.sodium_ !== false) {
    const key = jiffClient.sodium_.crypto_box_keypair(); // this party's public and secret key
...
Hooks.prototype.execute_array_hooks = function (hook_name, params, acc_index) {
  let arr = this.jiffClient.hooks[hook_name];
  arr = arr == null ? [] : arr;
