...
class PreprocessDaemon {
  ...
  find_closest_namespace(op, starting_namespace) {
    ...
    while (namespace_index >= 0) {
      const namespace = this.jiffClient.extensions[parseInt(namespace_index, 10)];
      if (this.jiffClient.preprocessing_function_map[String(namespace)] != null && this.jiffClient.preprocessing_function_map[String(namespace)][String(op)] != null) {
        return namespace;
      }
      namespace_index--;
    }
  ...
  taskFinished(task, result) {
    ...
    if (task.receivers_list.indexOf(this.jiffClient.id) > -1) {
      this.jiffClient.preprocess.api.store_preprocessing(task.id, result.share);
    }
  ...
  expandTask(task) {
    ...
    let preprocessing_dependencies = this.jiffClient.preprocessing_function_map[String(namespace)][String(task.dependent_op)];
