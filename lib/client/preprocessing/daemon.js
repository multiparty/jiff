module.exports = function (jiffClient) {
  ...
  var executeTask = function (task) {
    ...
    //taken from daemonCOPY.js lines 58-66 TODO: remove this comment when everything works
    var namespace = find_closest_namespace(task.dependent_op, task.params['namespace']);
    if (namespace == null) {
      // TODO: confirm protocols attribute is being copied/assigned properly between tasks
  ...
  jiffClient.preprocessingDaemon = function () {
    ...
    if (taskIsExecutable(task)) {
      ...
      //executeTask(task).then(jiffClient.preprocessingDaemon); // then is defined in executeTask()
