module.exports = function (jiffClient) {
  ...
  const getFirstTask = function (task) {
    if (task.count > 1) {
  ...
  const taskIsExecutable = function (task) {
  ...
  const find_closest_namespace = function (op, starting_namespace) {
  ...
  const executeTask = function (task) {
    ...
    const protocol = task.protocols[task.dependent_op] || jiffClient.default_preprocessing_protocols[task.dependent_op];
  ...
  const taskFinished = function (task, result) {
    currentBatchLoad--;
