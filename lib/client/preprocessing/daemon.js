module.exports = function (jiffClient) {
  const linkedList = require('../../common/linkedlist.js');
  let currentBatchLoad = 0;
  let suspendedTasks = 0;

  let getFirstTask = function (task) {
    if (task.count > 1) {
      let remainingTasks = Object.assign({}, task);
  ...
  let checkIfDone = function () {
    if (currentBatchLoad === 0 && suspendedTasks === 0) {
      const callback = jiffClient.preprocessingCallback;
      jiffClient.preprocessingCallback = null;
      callback(jiffClient);
    }
  };

  let buildID = function (task) {
  ...
  let taskIsExecutable = function (task) {
    // if the protocol name is in the map, it can be directly executed
    const namespace = find_closest_namespace(task.dependent_op, task.params['namespace']);
    return namespace == null;
  };

  let find_closest_namespace = function (op, starting_namespace) {
    let namespace_index = jiffClient.extensions.indexOf(starting_namespace);
    while (namespace_index >= 0) {
      const namespace = jiffClient.extensions[namespace_index];
  ...
  let executeTask = function (task) {
    currentBatchLoad++;

    let _params = Object.assign({}, task.params);
    _params.output_op_id = task.id;

    let protocol = task.protocols[task.dependent_op] || jiffClient.default_preprocessing_protocols[task.dependent_op];
  ...
  let taskFinished = function (task, result) {
    currentBatchLoad--;
  ...
  let expandTask = function (task) {
    ...
    const namespace = find_closest_namespace(task.dependent_op, _params['namespace']);
    let preprocessing_dependencies = jiffClient.preprocessing_function_map[namespace][task.dependent_op];
  ...
  jiffClient.preprocessingDaemon = function () {
    while (currentBatchLoad < jiffClient.preprocessingBatchSize) {
      let task = jiffClient.currentPreprocessingTasks.popHead();
