module.exports = function (jiffClient) {
  ...
  var checkIfDone = function () {
    if (jiffClient.preprocessingCallback != null && currentBatchLoad === 0 && suspendedTasks === 0) {
  ...
  var expandTask = function (task) {
    ...
    if (typeof(preprocessing_dependencies) === 'function') {
      preprocessing_dependencies = preprocessing_dependencies(task.dependent_op, task.count, task.protocols, task.threshold, task.receivers_list, task.compute_list, task.Zp, task.id_list, _params);
    }
    ...
    for (var k = 0; k < preprocessing_dependencies.length; k++) {
      ...
      if (next_count == null) {
        next_count = 1;
        next_id_list[0] = task.id + dependency['op_id'];
      } else {
      ...
      if (dependency.requires == null) {
        ...
        Promise.all(required_ops).then(function (nextTask) {
          jiffClient.preprocessingTasks.pushHead(nextTask);
          suspendedTasks--;
          jiffClient.preprocessingDaemon();
        }.bind(null, nextTask));
      }
