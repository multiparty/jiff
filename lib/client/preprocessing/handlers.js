...
class PreprocessHandlers {
  ...
  constant_bits_count() {
    return this.bits_count.apply(null, arguments) - 1;
  }
  dynamic_bits_cmult(dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, task_id, params) {
    ...
    const ops = [];
    for (let i = 0; i < constantBits; i++) {
      const accLength = i === 0 ? 1 : bitLength + i;
  ...
  dynamic_bits_smult(dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, task_id, params) {
    ...
    let left = params.bitLengthLeft;
    let right = params.bitLengthRight;
    left = left != null ? left : bitLength;
    right = right != null ? right : bitLength;
    const max = Math.max(left, right);
    const min = Math.max(left, right);
  ...
  dynamic_bits_sdiv(dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, task_id, params) {
    ...
    for (let i = 0; i < left; i++) {
      const accLength = Math.min(i + 1, min + 1);
  ...
  dynamic_bits_cdiv(dir) {
    return function (dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, task_id, params) {
      ...
      const min = Math.min(bitLength, constantBits);

      const ops = [];
      const loopCounter = dir === 'left' ? bitLength : constantBits;
  ...
  dynamic_rejection_sampling(dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, task_id, params, task, jiff) {
    ...
    if (previousPreprocessing == null || previousPreprocessing === 'RETRY' || (previousPreprocessing[0] != null && previousPreprocessing[0].value === 'RETRY')) {
      if (!params.defaultBounds && (params.lower_bound == null || params.upper_bound == null)) {
        jiff.preprocess.api.store_preprocessing(task.id, { ondemand: true });
        return [];
      }

      let compute_threshold = params.compute_threshold;
      if (compute_threshold == null) {
        // honest majority BGW
        compute_threshold = Math.floor((compute_list.length + 1) / 2);
      }

      const extra_params = { compute_threshold: compute_threshold };
      const reject_count = ++params.reject_count;
      ...
      dependent_ops.push({ op: 'sampling', op_id: '', requires: requires, params: extra_params }, { op: 'rejection_sampling', op_id: '', requires: [dependent_ops.length] });
  ...
  dynamic_random_and_quotient(dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, task_id, params, task, jiff) {
    ...
    const newParams = { compute_threshold: params.compute_threshold };
    ...
    if (constantNotProvided) {
      jiff.preprocess.api.store_preprocessing(task_id, { ondemand: true });
      return dependent_ops;
    }

    // we want to sample uniformly in [0, largest multiple of constant <= Zp) and [0, constant)
    const largestQuotient = jiff.share_helpers['floor'](jiff.share_helpers['/'](Zp, params.constant));
  ...
  dynamic_fast_exponentiation(dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, task_id, params, task, jiff) {
    ...
    const constantBits = params.constantBits == null ? Zp.toString(2).length : params.constantBits;
    ...
    const ops = [];
    if (constantNotProvided) {
      for (let i = 0; i < constantBits - 1; i++) {
