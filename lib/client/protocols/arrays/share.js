...
class ShareArray {
  share_array = (jiff, array, lengths, threshold, receivers_list, senders_list, Zp, share_id) => {
    let skeletons = null;
    if (lengths != null) {
    ...
    return this.share_ND_array(jiff, array, skeletons, threshold, receivers_list, senders_list, Zp, share_id);
  };

  share_2D_array = (jiff, array, lengths, threshold, receivers_list, senders_list, Zp, share_id) => {
    // Check format of lengths
    ...
    lengths_promise.then(function (lengths) {
      ...
      let max = 0;
      for (let i = 0; i < senders_list.length; i++) {
        const l = lengths[senders_list[i]].rows;
        max = l > max ? l : max;
      }
      ...
      for (let r = 0; r < max; r++) {
        ...
        const row = r < array.length ? array[r] : [];
        row_lengths = empty ? null : row_lengths;
        let round_results = this.share_array(jiff, row, row_lengths, threshold, receivers_list, round_senders, Zp, share_id + 'row' + r + ':');
  ...
  share_from_skeleton_unbound = (jiff, that, sender, skeleton) => {
    const shares = typeof skeleton === 'string' ? JSON.parse(skeleton) : skeleton;

    const promise = this.share_array_single_sender(jiff, shares, that.threshold, that.receivers_list, sender, that.Zp, that.share_id + ':p_id_' + sender);
  ...
  sanitize = (input) => {
    // Simple example of sanitization: allow only alphanumeric characters, colons, underscores, and dashes
  ...
  share_array_single_sender = (jiff, secrets, threshold, receivers_list, sender, Zp, share_id) => {
    if (secrets != null && secrets.length === 0) {
      ...
      for (let j = 0; j < secrets.length; j++) {
        share_id = this.sanitize(share_id);
        let indexed_share_id = share_id + ':' + j;
        indexed_share_id = this.sanitize(indexed_share_id);
        const parameters = [receivers_list, sender_list, threshold, Zp, indexed_share_id];
        [receivers_list, sender_list, threshold, Zp, indexed_share_id] = util.sanitize_array_params.bind(null, jiff).apply(null, parameters);
        promised_array.push(this.share_array_single_sender(jiff, secrets[j], threshold, receivers_list, sender, Zp, indexed_share_id));
      }
  ...
  share_ND_array_deferred = (jiff, secrets, skeletons, threshold, receivers_list, senders_list, Zp, share_id) => {
    ...
    const share_from_skeleton = this.share_from_skeleton_unbound.bind(null, jiff).bind(null, {
  ...
  share_ND_array_static = (jiff, secrets, skeletons, threshold, receivers_list, senders_list, Zp, share_id) => {
    const parameters = [receivers_list, senders_list, threshold, Zp, share_id];
    [receivers_list, senders_list, threshold, Zp, share_id] = util.sanitize_array_params.bind(null, jiff).apply(null, parameters);
  ...
  share_ND_array = (jiff, secrets, skeletons, threshold, receivers_list, senders_list, Zp, share_id) => {
    const share_ND_array = skeletons != null ? this.share_ND_array_static : this.share_ND_array_deferred;
    return share_ND_array(jiff, secrets, skeletons, threshold, receivers_list, senders_list, Zp, share_id);
  };
}

module.exports = ShareArray;
