module.exports = {
  ...
  jiff_sanitize_array_params : function (jiff, receivers_list, senders_list, threshold, Zp, share_id) {
    [receivers_list, senders_list] = [receivers_list, senders_list].map(function (party_list) {
      ...
      console.log('party_list sorted', party_list.sort(function (x, y) {return x - y;}));
      return party_list.sort(function (x, y) {return x - y;});  // sort to get the same order
    });
  ...
  jiff_share_from_skeleton : function (jiff, sender, skeleton) {
    let shares;
    if (typeof(skeleton) === 'string') {  // else create the symbolic shares for receiving
      shares = JSON.parse(skeleton);
    } else {
      shares = skeleton;
    }

    let promise = jiff.share_array_single_sender(jiff, shares, this.threshold, this.receivers_list, sender, this.Zp, this.share_id + ':p_id_' + sender);

    promise.then(function (sender, array) {
      this.deferreds[sender].resolve(array);
    }.bind(null, sender));
  },
  ...
  jiff_share_array_single_sender : function (jiff, secrets, threshold, receivers_list, sender, Zp, share_id) {
    if (secrets != null && secrets.length === 0) {
      ...
      for (var j = 0; j < secrets.length; j++) {
        promised_array.push(jiff.share_array_recursive(secrets[j], share_id + ':' + j));
      }
  ...
  jiff_share_ND_array : function (jiff, secrets, skeletons, threshold, receivers_list, senders_list, Zp, share_id) {
    ...
    // }
    jiff.share_from_skeleton = jiff.share_from_skeleton.bind({
    ...
    if (skeletons == null) {
      ...
      if (isReceiving) {
        jiff.listen(share_id + 'skeleton', jiff.share_from_skeleton);  // Share when dimensions are known
      }
    } else {
      ...
      for (i in senders_list) {  // Share each party's array
        let p_id = senders_list[sender];
        let skeleton = skeletons[p_id];
        jiff.share_from_skeleton(p_id, skeleton);
      }
    }
