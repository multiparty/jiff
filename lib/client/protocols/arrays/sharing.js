...
const share_from_skeleton = function () {
...
const share_ND_array_deferred = function (jiff, secrets, skeletons, threshold, receivers_list, senders_list, Zp, share_id) {
  ...
  [receivers_list, senders_list, threshold, Zp, share_id] = sanitize_array_params(jiff, ...parameters);
  ...
  // }
  share_from_skeleton = share_from_skeleton_unbound.bind(jiff).bind(null, {
  ...
  if (skeletons == null) {
    if (isSending) {  // Send the shape and lengths of this party's array to all receivers
      ...
      share_from_skeleton(jiff, jiff.id, secrets);  // Share the real values matching the emitted skeleton
    }
    if (isReceiving) {
    ...
    match_skeletons(jiff, skeletons, senders_list);  // Saftey check array dimention presets
    for (i in senders_list) {  // Share each party's array
      p_id = senders_list[i];
      var myself = p_id === jiff.id;
      skeleton = skeletons[p_id];
      share_from_skeleton(jiff, p_id, myself ? secrets : skeleton);
    }
  }
...
const receive_open_ND_array = function (jiff, parties, threshold, Zp, op_ids) {
  return open_ND_array(jiff, null, parties, null, op_ids);
};


module.exports = {
  jiff_share_array: share_array,
  jiff_share_2D_array: share_2D_array,
  jiff_share_ND_array: share_ND_array,
  jiff_share_ND_array_static: share_ND_array_static,
  jiff_share_ND_array_deferred: share_ND_array_deferred,
  jiff_skeleton_of: skeleton_of,
  jiff_open_array: open_array,
  jiff_open_ND_array: open_ND_array,
  jiff_receive_open_ND_array: receive_open_ND_array
};
