module.exports = {
  ...
  jiff_open_ND_array : function (jiff, shares, receivers_list, senders_list, op_ids) {
    ...
    console.log('ndarray', receivers_list, senders_list, jiff.id);

    // If this party is not a sender, then the variable shares is a skeleton
    if (senders_list.indexOf(jiff.id) === -1) {
      // Populate skeleton with imitation shares
      const share = new jiff.SecretShare({}, senders_list, jiff.threshold, jiff.Zp);
      shares = (function __wipe(nd_array, replace) {
    ...
    })(shares, senders_list, op_ids);  // hmm with receivers_list it hangs but didn't a minute ago
  },
  //TODO: documentation
  receive_open_ND_array : function (parties, threshold, Zp, op_id) {
