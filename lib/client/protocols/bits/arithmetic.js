class BitsArithmetic {
  ...
  cadd = (jiff, bits, constant, op_id) => {
    ...
    const constant_bits = jiff.helpers.number_to_bits(constant, bits.length); // pads with zeros to bits.length

    // initialize results
    let result = jiff.utils.many_secret_shares(Math.max(constant_bits.length, bits.length), bits[0].holders, bits[0].threshold, bits[0].Zp);
    const deferreds = result.deferreds;
    result = result.shares;

    const sum = bits[0].icxor_bit(constant_bits[0]);
    let carry = bits[0].icmult(constant_bits[0]);
    ...
    jiff.utils.bit_combinator(deferreds[deferreds.length - 1], 1, deferreds.length - 1, carry, function (i, carry) {
      let sum;
      if (i < bits.length) {
        const and = bits[i].icmult(constant_bits[i]);
        const xor = bits[i].icxor_bit(constant_bits[i]);
        const xorAndCarry = xor.ismult(carry, op_id + ':smult:' + (i - 1));
   ...
   */
  csubl = (jiff, bits, constant, op_id) => {
    if (!bits[0].isConstant(constant)) {
    ...
    const diff = bits[0].icxor_bit(constant_bits[0]);
    const borrow = bits[0].inot().icmult(constant_bits[0]);
    ...
    jiff.utils.bit_combinator(deferreds[deferreds.length - 1], 1, deferreds.length - 1, borrow, function (i, borrow) {
      ...
      if (i < bits.length) {
        const xor = bits[i].icxor_bit(constant_bits[i]);
        const andNot = bits[i].inot().icmult(constant_bits[i]);
   ...
   *                          and the bit at index |bits| is 1 if the result overflows, or 0 otherwise
   */
  csubr = (jiff, constant, bits, op_id) => {
    if (!bits[0].isConstant(constant)) {
    ...
    const diff = bits[0].icxor_bit(constant_bits[0]);
    const borrow = bits[0].issub(bits[0].icmult(constant_bits[0]));
    ...
    jiff.utils.bit_combinator(deferreds[deferreds.length - 1], 1, deferreds.length - 1, borrow, function (i, borrow) {
      ...
      if (i < bits.length) {
        const xor = bits[i].icxor_bit(constant_bits[i]);
        const andNot = bits[i].issub(bits[i].icmult(constant_bits[i]));
  ...
  sadd = (jiff, bits1, bits2, op_id) => {
    ...
    const tmp = bits1.length > bits2.length ? bits1 : bits2;
    bits2 = bits1.length > bits2.length ? bits2 : bits1; // shortest array
    bits1 = tmp; // longest array

    // initialize results
    let result = jiff.utils.many_secret_shares(bits1.length, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp);
    const deferreds = result.deferreds;
    result = result.shares;

    const sum = bits1[0].isxor_bit(bits2[0], op_id + ':sxor_bit:initial');
    const carry = bits1[0].ismult(bits2[0], op_id + ':smult:initial');
    ...
    jiff.utils.bit_combinator(deferreds[deferreds.length - 1], 1, deferreds.length - 1, carry, function (i, carry) {
      let sum;
      if (i < bits2.length) {
        const and = bits1[i].ismult(bits2[i], op_id + ':smult1:' + (i - 1));
        const xor = bits1[i].isxor_bit(bits2[i], op_id + ':sxor_bit1:' + (i - 1));
        const xorAndCarry = xor.ismult(carry, op_id + ':smult2:' + (i - 1));
  ...
  ssub = (jiff, bits1, bits2, op_id) => {
    ...
    // initialize results
    let result = jiff.utils.many_secret_shares(Math.max(bits1.length, bits2.length), bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp);
    const deferreds = result.deferreds;
    result = result.shares;

    const diff = bits1[0].isxor_bit(bits2[0], op_id + ':sxor_bit:initial');
    const borrow = bits1[0].inot().ismult(bits2[0], op_id + ':smult:initial');
    ...
    jiff.utils.bit_combinator(deferreds[deferreds.length - 1], 1, deferreds.length - 1, borrow, function (i, borrow) {
      ...
      if (i < bits1.length && i < bits2.length) {
        const xor = bits1[i].isxor_bit(bits2[i], op_id + ':sxor_bit1:' + (i - 1));
        const andNot = bits1[i].inot().ismult(bits2[i], op_id + ':smult1:' + (i - 1));
   ...
   */
  cmult = (jiff, bits, constant, op_id) => {
    if (!bits[0].isConstant(constant)) {
    ...
    // decompose constant into bits
    const constant_bits = jiff.helpers.number_to_bits(constant); // do not pad

    // Initialize the result
    let result = jiff.utils.many_secret_shares(bits.length + constant_bits.length, bits[0].holders, bits[0].threshold, bits[0].Zp);
    const deferreds = result.deferreds;
    result = result.shares;
    ...
    const zero = new jiff.SecretShare(0, bits[0].holders, bits[0].threshold, bits[0].Zp);
    const initial = [zero];
    ...
    // main function
    jiff.utils.bit_combinator(
      final_deferred,
      0,
      constant_bits.length,
      initial,
      function (i, intermediate) {
  ...
  smult = (jiff, bits1, bits2, op_id) => {
    ...
    // Initialize the result
    const offset = bits2.length === 1 ? -1 : 0;
    let result = jiff.utils.many_secret_shares(bits1.length + bits2.length + offset, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp);
    const deferreds = result.deferreds;
    result = result.shares;
    ...
    jiff.utils.bit_combinator(
      ...
      bits2,
      function (i, intermediate) {
        const this_bit = bits2[i];
        const bit_mult = []; // add bits1 or 0 to the result according to this bit
  ...
  sdiv = (jiff, bits1, bits2, op_id) => {
    ...
    // Initialize the result
    let quotient = jiff.utils.many_secret_shares(bits1.length, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp);
    const quotientDeferreds = quotient.deferreds;
    quotient = quotient.shares;

    let remainder = jiff.utils.many_secret_shares(Math.min(bits1.length, bits2.length), bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp);
    const remainderDeferreds = remainder.deferreds;
    ...
    jiff.utils.bit_combinator(
      ...
      -1,
      initial,
      function (i, _remainder) {
        const iterationCounter = bits1.length - i - 1;
        ...
        const sub = jiff.protocols.bits.ssub(_remainder, bits2, op_id + ':bits.ssub:' + iterationCounter);
        const noUnderflow = sub.pop().inot(); // get the overflow bit, sub is now the result of subtraction

        // Get next bit of quotient
        noUnderflow.wThen(quotientDeferreds[i].resolve);

        // Update remainder
        for (let j = 0; j < _remainder.length; j++) {
   ...
   */
  cdivl = (jiff, bits, constant, op_id) => {
    if (!bits[0].isConstant(constant)) {
    ...
    // Initialize the result
    let quotient = jiff.utils.many_secret_shares(bits.length, bits[0].holders, bits[0].threshold, bits[0].Zp);
    const quotientDeferreds = quotient.deferreds;
    quotient = quotient.shares;

    let constantLessBits = jiff.helpers.ceil(jiff.helpers.bLog(constant, 2));
    constantLessBits = parseInt(constantLessBits.toString(), 10);
    let remainder = jiff.utils.many_secret_shares(Math.min(constantLessBits, bits.length), bits[0].holders, bits[0].threshold, bits[0].Zp);
    const remainderDeferreds = remainder.deferreds;
    remainder = remainder.shares;
    ...
    jiff.utils.bit_combinator(
      ...
      initial,
      function (i, _remainder) {
        const iterationCounter = bits.length - i - 1;
        ...
        // and conditionally subtract b from the
        // intermediate remainder to continue
        const sub = jiff.protocols.bits.csubl(_remainder, constant, op_id + ':bits.csubl:' + iterationCounter);
        const noUnderflow = sub.pop().inot(); // get the overflow bit, sub is now the result of subtraction
   ...
   */
  cdivr = (jiff, constant, bits, op_id) => {
    if (!bits[0].isConstant(constant)) {
    ...
    // do not pad
    const constant_bits = jiff.helpers.number_to_bits(constant);

    // Initialize the result
    let quotient = jiff.utils.many_secret_shares(constant_bits.length, bits[0].holders, bits[0].threshold, bits[0].Zp);
    const quotientDeferreds = quotient.deferreds;
    quotient = quotient.shares;

    let remainder = jiff.utils.many_secret_shares(Math.min(constant_bits.length, bits.length), bits[0].holders, bits[0].threshold, bits[0].Zp);
    const remainderDeferreds = remainder.deferreds;
    remainder = remainder.shares;
    ...
    jiff.utils.bit_combinator(
      final_deferred,
      constant_bits.length - 1,
      -1,
      initial,
      function (i, _remainder) {
        const iterationCounter = constant_bits.length - i - 1;

        // add bit i to the head of remainder (least significant bit)
        // turn into a secret without communication, just for typing
        const cbit_share = new jiff.SecretShare(constant_bits[i], bits[0].holders, bits[0].threshold, bits[0].Zp);
        _remainder.unshift(cbit_share);

        // Get the next bit of the quotient
        // and conditionally subtract b from the
        // intermediate remainder to continue
        const sub = jiff.protocols.bits.ssub(_remainder, bits, op_id + ':bits.ssub:' + iterationCounter);
        const noUnderflow = sub.pop().inot(); // get the overflow bit, sub is now the result of subtraction
