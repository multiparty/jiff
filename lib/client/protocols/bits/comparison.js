class BitComparison {
  ...
  ceq = (jiff, bits, constant, op_id) => {
    ...
    const result = jiff.protocols.bits.cneq(bits, constant, op_id);
  ...
  cneq = (jiff, bits, constant, op_id) => {
    ...
    const constant_bits = jiff.helpers.number_to_bits(constant, bits.length);
    ...
    const initial = bits[0].icxor_bit(constant_bits[0]);
    jiff.utils.bit_combinator(deferred, 1, bits.length, initial, function (i, prev) {
      let xor = bits[i].icxor_bit(constant_bits[i]);
   ...
   *                                returned immediately as a boolean
   */
  cgteq = (jiff, bits, constant, op_id) => {
    if (!bits[0].isConstant(constant)) {
  ...
  clt = (jiff, bits, constant, op_id) => {
    ...
    const result = jiff.protocols.bits.cgteq(bits, constant, op_id);
    if (result === true || result === false) {
      return !result;
    }
    return result.inot();
   ...
   *                                returned immediately as a boolean
   */
  clteq = (jiff, bits, constant, op_id) => {
    if (!bits[0].isConstant(constant)) {
    ...
    const result = jiff.protocols.bits.cgt(bits, constant, op_id);
    if (result === true || result === false) {
      return !result;
    }
    return result.inot();
  ...
  sneq = (jiff, bits1, bits2, op_id) => {
    ...
    // big or of bitwise XORs
    const initial = bits1[0].isxor_bit(bits2[0], op_id + ':sxor_bit:initial');
    jiff.utils.bit_combinator(deferred, 1, bits1.length, initial, function (i, prev) {
      let next;
      if (i < bits2.length) {
        const xor = bits1[i].isxor_bit(bits2[i], op_id + ':sxor_bit:' + (i - 1));
   ...
   */
  sgt = (jiff, bits1, bits2, op_id) => {
    if (op_id == null) {
      op_id = jiff.counters.gen_op_id('bits.sgt', bits1[0].holders);
    }

    const gteq = jiff.protocols.bits.sgteq(bits1, bits2, op_id + ':bits.sgteq');
    const neq = jiff.protocols.bits.sneq(bits1, bits2, op_id + ':bits.sneq');
   ...
   */
  slt = (jiff, bits1, bits2, op_id) => {
    if (op_id == null) {
      op_id = jiff.counters.gen_op_id('bits.slt', bits1[0].holders);
    }
    const result = jiff.protocols.bits.sgteq(bits1, bits2, op_id);
    return result.inot();
  };
  /**
   ...
   */
  slteq = (jiff, bits1, bits2, op_id) => {
    if (op_id == null) {
      op_id = jiff.counters.gen_op_id('bits.slteq', bits1[0].holders);
    }
    const result = jiff.protocols.bits.sgt(bits1, bits2, op_id);
    return result.inot();
  };
}
