class BitProtocols {
   ...
   */
  rejection_sampling = (jiff, lower_bound, upper_bound, threshold, parties, Zp, op_id) => {
    // defaults
    if (parties == null) {
      parties = [];
    ...
    const result = jiff.preprocess.api.get_preprocessing(op_id);
    ...
    const many_shares = jiff.utils.many_secret_shares(finalLength, parties, threshold, Zp);
    const final_deferreds = many_shares.deferreds;

    // Crypto provider
    if (result == null) {
      const promise = jiff.from_crypto_provider('numbers', parties, threshold, Zp, op_id, {
        max: upper_bound,
        min: lower_bound,
      ...
      jiff.preprocess.api.preprocessing('rejection_sampling', 1, null, threshold, parties, parties, Zp, [op_id], {
        lower_bound: lower_bound,
        upper_bound: upper_bound
      });
      jiff.preprocess.api.executePreprocessing(function () {
        jiff.utils.resolve_many_secrets(final_deferreds, jiff.preprocess.api.get_preprocessing(op_id));
      });
    }
