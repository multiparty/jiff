module.exports = {
  ...
  rejection_sampling: function (jiff, lower_bound, upper_bound, threshold, parties, Zp, op_id) {
    ...
    var protocols = {generate_random_bit: jiff.protocols.generate_random_bit_smult};
    var params = {
    ...
    (function sample(reject_count) {
      params.reject_count = reject_count;

      result = jiff.protocols.rejection_sampling(threshold, parties, parties, Zp, params, protocols);
      result.promise.then(function () {
        if (result.share === 'RETRY' || result.share[0].value === 'RETRY') {
          sample(reject_count+1);
        } else {
          jiff.utils.resolve_many_secrets(final_deferreds, result.share);
        }
      });
    })(0);
