class BitShare {
   ...
   */
  share_bits = (jiff, secret, bit_length, threshold, receivers_list, senders_list, Zp, share_id) => {
    if (Zp == null) {
      Zp = jiff.Zp;
    }
    ...
    for (let i = 0; i < bit_length; i++) {
      const round = jiff.internal_share(local_bits[i], threshold, receivers_list, senders_list, Zp, share_id + ':' + i);
      for (let si = 0; si < senders_list.length; si++) {
        const pid = senders_list[si];
   ...
   */
  open_bits = (jiff, bits, parties, op_id) => {
    // Default values
    if (parties == null) {
      parties = [];
      for (let p = 1; p <= jiff.party_count; p++) {
        parties.push(p);
      }
    } else {
   ...
   */
  receive_open_bits = (jiff, senders, receivers, count, threshold, Zp, op_id) => {
    if (senders == null) {
