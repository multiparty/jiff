...
module.exports = function (SecretShare) {
  ...
  SecretShare.prototype.cdivfac = function (cst) {
    ...
    const inv = this.jiff.helpers.extended_gcd(cst, this.Zp)[0];
   ...
   * let shares = jiff_instance.share(x);
   * let sum = shares[1];
  ...
  SecretShare.prototype.smult = function (o, op_id) {
    ...
    const self = this;
    const ready_triplet = function (triplet) {
      const a = triplet[0];
      const b = triplet[1];
      const c = triplet[2];

      // d = s - a. e = o - b.
      const d = self.isadd(a.icmult(-1));
      const e = o.isadd(b.icmult(-1));

      // Open d and e.
      // The only communication cost.
      const e_promise = self.jiff.internal_open(e, e.holders, op_id + ':open1');
      const d_promise = self.jiff.internal_open(d, d.holders, op_id + ':open2');
      Promise.all([e_promise, d_promise]).then(function (arr) {
        const e_open = arr[0];
        const d_open = arr[1];

        // result_share = d_open * e_open + d_open * b_share + e_open * a_share + c.
        const t1 = self.jiff.helpers.mod(self.jiff.share_helpers['*'](d_open, e_open), self.Zp);
        const t2 = b.icmult(d_open);
        const t3 = a.icmult(e_open);

        // All this happens locally.
        let final_result = t2.icadd(t1);
    ...
    const triplet = this.jiff.preprocess.api.get_preprocessing(op_id + ':triplet');
    if (triplet == null) {
      const promise = this.jiff.from_crypto_provider('triplet', this.holders, Math.max(this.threshold, o.threshold), this.Zp, op_id + ':triplet');
  ...
  SecretShare.prototype.smult_bgw = function (o, op_id) {
    ...
    const new_threshold = this.threshold - 1 + (o.threshold - 1) + 1;
    if (new_threshold > this.holders) {
      const errorMsg = 'Threshold too large for smult_bgw: ' + new_threshold;
  ...
  SecretShare.prototype.sdiv = function (o, l, op_id) {
    ...
    const lZp = this.jiff.share_helpers['ceil'](this.jiff.helpers.bLog(this.Zp, 2));
    l = l != null && l < lZp ? l : lZp;

    // Convert to bits
    const dividend_bits = this.bit_decomposition(op_id + ':decomposition1').slice(0, l);
    const divisor_bits = o.bit_decomposition(op_id + ':decomposition2').slice(0, l);

    // Compute by long division
    const quotient_bits = this.jiff.protocols.bits.sdiv(dividend_bits, divisor_bits, op_id + ':bits.sdiv').quotient;
  ...
  SecretShare.prototype.cdiv = function (cst, op_id) {
    ...
    const self = this;
    const ready_quotient = function (noise, nOVERc) {
      // Use noise
      const noisyX = self.isadd(noise);
      self.jiff.internal_open(noisyX, noisyX.holders, op_id + ':open').then(function (noisyX) {
        const wrapped = self.icgt(noisyX, op_id + ':wrap_cgt'); // 1 => x + noise wrapped around Zp, 0 otherwise

        // if we did not wrap
        const noWrapDiv = self.jiff.share_helpers['floor/'](noisyX, cst);
        let unCorrectedQuotient = nOVERc.icmult(-1).icadd(noWrapDiv).icsub(1);
        let verify = self.issub(unCorrectedQuotient.icmult(cst));
        let isNotCorrect = verify.icgteq(cst, op_id + ':cor1');
        const noWrapAnswer = unCorrectedQuotient.isadd(isNotCorrect); // if incorrect => isNotCorrect = 1 => quotient = unCorrectedQuotient - 1

        // if we wrapped
        const wrapDiv = self.jiff.share_helpers['floor/'](self.jiff.share_helpers['+'](noisyX, self.Zp), cst);
        ...
        const wrapAnswer = unCorrectedQuotient.isadd(isNotCorrect); // if incorrect => isNotCorrect = 1 => quotient = unCorrectedQuotient - 1

        const answer = noWrapAnswer.isadd(wrapped.ismult(wrapAnswer.issub(noWrapAnswer), op_id + ':smult'));
        answer.wThen(final_deferred.resolve);
      });
    };

    // Preprocessing cases
    const quotient = this.jiff.preprocess.api.get_preprocessing(op_id + ':quotient');
    if (quotient == null) {
      // case 1: no preprocessing with crypto provider!
      const promise = this.jiff.from_crypto_provider('quotient', this.holders, this.threshold, this.Zp, op_id + ':quotient', { constant: cst });
      promise.then(function (msg) {
        ready_quotient(msg['shares'][0], msg['shares'][1]);
      });
    } else if (quotient.ondemand === true) {
      // case 2: constant was not available at preprocessing time, must do it now!
      this.jiff.preprocess.api.preprocessing('quotient', 1, null, this.threshold, this.holders, this.holders, this.Zp, [op_id + ':quotient'], { constant: cst, namespace: 'base' });
      this.jiff.preprocess.api.executePreprocessing(function () {
        const quotient = self.jiff.preprocess.api.get_preprocessing(op_id + ':quotient');
        ready_quotient(quotient.r, quotient.q);
      });
    } else {
      // case 3: preprocessing is completed!
      ready_quotient(quotient.r, quotient.q);
    }
  ...
  SecretShare.prototype.smod = function (o, l, op_id) {
    ...
    // Compute by long division
    const remainder_bits = this.jiff.protocols.bits.sdiv(dividend_bits, divisor_bits, op_id + ':bits.sdiv').remainder;
  ...
  SecretShare.prototype.cpow = function (cst, op_id) {
    ...
    // begin protocol
    let evens = this;
    let odds = new this.jiff.SecretShare(one, this.holders, this.threshold, this.Zp);
    ...
    let i = 0;
    for (i; this.jiff.share_helpers['<'](1, cst); i++) {
