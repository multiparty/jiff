...
module.exports = function (SecretShare) {
  ...
  SecretShare.prototype.slt = function (o, op_id) {
    ...
    let final_deferred = new this.jiff.helpers.Deferred();
    let final_promise = final_deferred.promise;
    let result = new this.jiff.SecretShare(final_promise, this.holders, Math.max(this.threshold, o.threshold), this.Zp);

    let w = this.ilt_halfprime(op_id + ':halfprime:1');
    ...
    w.wThen(function () {
      ...
      x.wThen(function () {
        ...
        y.wThen(function () {
          let xy = x.ismult(y, op_id + ':smult1');
          let answer = x
            .icmult(-1)
            .icadd(1)
            .issub(y)
