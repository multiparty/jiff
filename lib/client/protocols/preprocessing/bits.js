module.exports = {
  ...
  generate_random_bit_smult: function (jiff, threshold, receivers_list, compute_list, Zp, params) {
    ...
    // Generate random bit
    let random_bit, promise;
    if (compute_list.indexOf(jiff.id) > -1) {
      const bit = jiff.helpers.random(2);
      const bit_shares = jiff.internal_share(bit, params.compute_threshold, compute_list, compute_list, Zp, op_id + ':share');

      random_bit = bit_shares[compute_list[0]];
      for (var i = 1; i < compute_list.length; i++) {
        const party_id = compute_list[i];
        const obit = bit_shares[party_id];
  ...
  generate_random_bit_bgw: function (jiff, threshold, receivers_list, compute_list, Zp, params) {
    ...
    if (compute_list.indexOf(jiff.id) > -1) {
      ...
      random_bit = bit_shares[compute_list[0]];
      for (let i = 1; i < compute_list.length; i++) {
        const party_id = compute_list[i];
        const obit = bit_shares[party_id];
  ...
  generate_random_bits: function (jiff, threshold, receivers_list, compute_list, Zp, params, protocols) {
    ...
    for (var i = 0; i < params.count; i++) {
      ...
      const bit = protocols.generate_random_bit(threshold, receivers_list, compute_list, Zp, params, protocols);
