 ...
 */
const jiff_share_all_number = function (jiff, n, threshold, receivers_list, compute_list, Zp, params) {
  const isSender = compute_list.indexOf(jiff.id) > -1;
  ...
  let result, promise;
  if (isSender) {
    const shares = jiff.internal_share(n, params.compute_threshold, compute_list, compute_list, Zp, params.op_id + ':share');
