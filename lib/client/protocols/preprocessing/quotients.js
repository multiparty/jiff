...
module.exports = function (jiff, threshold, receivers_list, compute_list, Zp, params, protocols) {
  ...
  if (compute_list.indexOf(jiff.id) > -1) {
    const largestQuotient = jiff.share_helpers['floor'](jiff.share_helpers['/'](Zp, constant));
    const largestMultiple = jiff.share_helpers['*'](largestQuotient, constant);

    // Uniform random number between [0, Zp)
    const r_bits = jiff.protocols.bits.rejection_sampling(0, Zp, params.compute_threshold, compute_list, Zp, params.output_op_id + ':rejection1');
    const cmp = jiff.protocols.bits.cgteq(r_bits, largestMultiple, params.output_op_id + ':bits_cgteq');
    const r1 = jiff.protocols.bits.bit_composition(r_bits);

    // Uniform random number between [0, Math.floor(Zp / constant))
    let quotient = jiff.protocols.bits.rejection_sampling(0, largestQuotient, params.compute_threshold, compute_list, Zp, params.output_op_id + ':rejection2');
    quotient = jiff.protocols.bits.bit_composition(quotient);

    // Uniform random number between [0, constant)
    let remainder = jiff.protocols.bits.rejection_sampling(0, constant, params.compute_threshold, compute_list, Zp, params.output_op_id + ':rejection3');
    remainder = jiff.protocols.bits.bit_composition(remainder);
    const r2 = quotient.icmult(constant).isadd(remainder);
