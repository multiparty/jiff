const sample = function (jiff, range, compute_list, Zp, params, protocols, reject_count) {
  // Transform sampling range into bit size
  let bitLength = jiff.helpers.ceil(jiff.helpers.bLog(range, 2));
...
const one_round_sampling = function (jiff, lower_bound, upper_bound, compute_list, Zp, params, protocols, finalLength, reject_count) {
  ...
  if (range.toString() === '1') {
    const zero = protocols.generate_zero(params.compute_threshold, compute_list, compute_list, Zp, params, protocols).share;
    // special case: cadd can be performed locally on bit arrays of length 1!
    const resultOne = jiff.protocols.bits.cadd([zero], lower_bound);
  ...
  // Rejection protocol
  const bits = sample(jiff, range, compute_list, Zp, params, protocols, reject_count);
  const cmp = jiff.protocols.bits.clt(bits, range, params.output_op_id + ':bits.clt:' + reject_count);
  const bits_add = jiff.protocols.bits.cadd(bits, lower_bound, params.output_op_id + ':bits.cadd:' + reject_count);
  ...
  const promise = jiff.internal_open(cmp, compute_list, params.output_op_id + ':open:' + reject_count);
  return {
    share: bits_add,
    promise: promise.then(function (cmp) {
...
var computeParty = function (jiff, lower_bound, upper_bound, threshold, receivers_list, compute_list, Zp, params, protocols, op_id, finalLength, reject_count) {
  const result = one_round_sampling(jiff, lower_bound, upper_bound, compute_list, Zp, params, protocols, finalLength, reject_count);
  ...
  const many_shares = jiff.utils.many_secret_shares(finalLength, compute_list, threshold, Zp);
  const final_deferreds = many_shares.deferreds;

  result.promise.then(function (promiseVal) {
    // RETRY and PLACEHOLDER shares are cleaned up later in the preprocessing pipeline
    if (promiseVal === false) {
      for (let i = 0; i < final_deferreds.length; i++) {
    ...
    const reshared = reshareResult(jiff, upper_bound, threshold, receivers_list, compute_list, Zp, op_id, finalLength, result.share);
...
const reshareResult = function (jiff, upper_bound, threshold, receivers_list, compute_list, Zp, op_id, finalLength, shares) {
...
module.exports = function (jiff, threshold, receivers_list, compute_list, Zp, params, protocols) {
  ...
  const lower_bound = params.lower_bound != null ? params.lower_bound : 0;
  const upper_bound = params.upper_bound != null ? params.upper_bound : Zp;
  ...
  let finalLength = jiff.helpers.ceil(jiff.helpers.bLog(upper_bound, 2));
  ...
  // Rejection count
  const reject_count = params.reject_count || 0;
