 ...
 *
 */
const jiff_broadcast = function (jiff, share, parties, op_id) {
  for (let index = 0; index < parties.length; index++) {
    let i = parties[index]; // Party id
    if (i === jiff.id) {
    ...
    let msg = { party_id: i, share: share.value, op_id: op_id, Zp: share.Zp };
...
module.exports = {
  ...
  jiff_open: function (jiff, share, parties, op_id) {
    ...
    if (parties.indexOf(jiff.id) > -1) {
      ...
      return final_promise.then(function () {
        const shares = jiff.deferreds[op_id].shares;
  ...
  jiff_lagrange: function (jiff, shares) {
    let lagrange_coeff = []; // will contain shares.length many elements.
    ...
    for (let i = 0; i < shares.length; i++) {
      ...
      for (let j = 0; j < shares.length; j++) {
        ...
        if (pj !== pi) {
          let inv = jiff.helpers.extended_gcd(pi - pj, shares[i].Zp)[0];
    ...
    let recons_secret = 0;
    for (let p = 0; p < shares.length; p++) {
      const party = jiff.helpers.get_party_number(shares[p].sender_id);
      const tmp = jiff.helpers.mod(shares[p].value * lagrange_coeff[party], shares[p].Zp);
