const genericProtocols = require('./protocols/generic.js');
...
module.exports = function (jiff) {
  ...
  SecretShare.prototype.toString = function () {
    const val = this.ready ? this.value : '<promise>';
  ...
  SecretShare.prototype.logLEAK = function (tag, parties, op_id) {
    ...
    let promise = this.open(parties, tag, op_id);
    if (promise != null) {
  ...
  Object.getOwnPropertyNames(arithmetic.prototype).forEach((method) => {
    SecretShare.prototype[String(method)] = arithmetic.prototype[String(method)];
  });
  Object.getOwnPropertyNames(comparison.prototype).forEach((method) => {
    SecretShare.prototype[String(method)] = comparison.prototype[String(method)];
  });
  Object.getOwnPropertyNames(protocols.prototype).forEach((method) => {
    SecretShare.prototype[String(method)] = protocols.prototype[String(method)];
  });
