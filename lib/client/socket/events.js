class SocketEvent {
  ...
  initSocket = function () {
    ...
    this.jiffClient.socket.on('error', function (_msg) {
      try {
        const msg = JSON.parse(_msg);
        thisSocket.jiffClient.handlers.error(msg['label'], msg['error']);
      } catch (error) {
        thisSocket.jiffClient.handlers.error('socket.io', _msg);
      }
    });
