module.exports = function (JIFFClient) {
  ...
  JIFFClient.prototype.initSocket = function () {
    ...
    jiffClient.socket.on('public_keys', function (msg, callback) {
      ...
      msg = JSON.parse(msg);
      msg = jiffClient.execute_array_hooks('afterOperation', [jiffClient, 'public_keys', msg], 2);
    ...
    this.socket.on('disconnect', function (reason) {
      ...
      jiffClient.execute_array_hooks('afterOperation', [jiffClient, 'disconnect', reason], -1);
    });
  };
  ...
  JIFFClient.prototype.execute_wait_callbacks = function () {
    var new_waits = [];
    for (var i = 0; i < this.wait_callbacks.length; i++) {
      var wait = this.wait_callbacks[i];
      ...
      if (initialization) {
        parties_satisfied = parties_satisfied && this.__initialized;
      }
