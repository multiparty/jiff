...
function guardedSocket(jiffClient) {
  // TODO reintegrate socketOptions
  ...
  var socket = new WebSocket(jiffClient.hostname) //, jiffClient.options.socketOptions);
...
var safe_emit = function (label, msg) {
  ...
  if (this.readyState == 1) {
    ...
    this.send(JSON.stringify( { socketProtocol: label, data: msg } ), function (status) {
      if (status) {
