...
function guardedSocket(jiffClient) {
  ...
  const socket = io(jiffClient.hostname, jiffClient.options.socketOptions);
 ...
 */
const safe_emit = function (label, msg) {
 ...
 */
const safe_disconnect = function (free, callback) {
  if (this.is_empty()) {
    if (free) {
