...
class GuardedSocket {
  constructor(jiffClient) {
    ...
    this.options = {};
    const options = Object.assign({}, defaultSocketOptions, jiffClient.options.socketOptions);

    // Create plain socket io object which we will wrap in this
    this.socket = io(jiffClient.hostname, options);
    this.mailbox = new LinkedList(); // for outgoing messages
    this.empty_deferred = null; // gets resolved whenever the mailbox is empty
    this.old_disconnect = this.socket.disconnect;
  }

  on(instr, callback) {
    this.socket.on(instr, callback);
  }
  ...
  safe_emit(label, msg) {
    ...
    const mailbox_pointer = this.mailbox.add({ label: label, msg: msg });
    if (this.socket.connected) {
      // emit the message, if an acknowledgment is received, remove it from mailbox
      this.socket.emit(label, msg, (status) => {
        if (status) {
          this.mailbox.remove(mailbox_pointer);
          if (this.is_empty() && this.empty_deferred != null) {
            this.empty_deferred.resolve();
          }
  ...
  resend_mailbox() {
    ...
    const old_mailbox = this.mailbox;
    this.mailbox = new LinkedList();

    // loop over all stored messages and emit them
    let current_node = old_mailbox.head;
    while (current_node != null) {
      const label = current_node.object.label;
      const msg = current_node.object.msg;
  ...
  disconnect() {
    ...
    this.old_disconnect.apply(this.socket, arguments);
  }
  ...
  safe_disconnect(free, callback) {
    ...
    this.empty_deferred.promise.then(this.safe_disconnect.bind(this.socket, free, callback));
  }
