  ...
  function make_jiff(base_instance, options) {
    ...
    jiff.hooks.afterOperation[0] = function (jiff, label, msg) {
      if (label === 'share' || label === 'open') {
        ...
        msg['triplet'] = {
          a: jiff.helpers.BigNumber(msg['triplet']['a']),
          b: jiff.helpers.BigNumber(msg['triplet']['b']),
          c: jiff.helpers.BigNumber(msg['triplet']['c'])
        };
      } else if (label === 'number') {
        var numbers = msg['numbers'];
        for (var i = 0; i < numbers.length; i++) {
          var numberShares = numbers[i]['number'];
          if (typeof(numberShares) === 'object' && numberShares instanceof Array) {
            for (var b = 0; b < numberShares.length; b++) {
              numberShares[b] = jiff.helpers.BigNumber(numbers[i]['number'][b]);
            }
          } else {
            numbers[i]['number'] = jiff.helpers.BigNumber(numberShares);
          }
        }
        msg['numbers'] = numbers;
      }
