...
(function (exports, node) {
  ...
  function is_prime(p) {
    ...
    let i = new BigNumber_(5);
    let n = new BigNumber_(2);
    const six6 = new BigNumber_(6);
  ...
  function jiff_compute_shares(jiff, secret, parties_list, threshold, Zp) {
    ...
    for (let i = 0; i < parties_list.length; i++) {
      ...
      let power = jiff.helpers.BigNumber(jiff.helpers.get_party_number(p_id));

      for (let j = 1; j < polynomial.length; j++) {
        let tmp = jiff.helpers.mod(polynomial[j].times(power), Zp);
  ...
  function jiff_lagrange(jiff, shares) {
    ...
    for (let i = 0; i < shares.length; i++) {
      ...
      const pi = jiff.helpers.get_party_number(shares[i].sender_id);
      lagrange_coeff[pi] = jiff.helpers.BigNumber(1);

      for (let j = 0; j < shares.length; j++) {
        const pj = jiff.helpers.get_party_number(shares[j].sender_id);
        if (pj !== pi) {
          const inv = jiff.helpers.extended_gcd(pi - pj, shares[i].Zp)[0];
    ...
    let recons_secret = jiff.helpers.BigNumber(0);
    for (let i = 0; i < shares.length; i++) {
      const pi = jiff.helpers.get_party_number(shares[i].sender_id);
      const tmp = jiff.helpers.mod(shares[i].value.times(lagrange_coeff[pi]), shares[i].Zp);
  ...
  function createSecretShare(jiff, share) {
    ...
    const oldIsConstant = self.isConstant;
  ...
  function customizeShareHelpers(jiff) {
    ...
    jiff.share_helpers['floor/'] = function (v1, v2) {
      return v1.div(v2).decimalPlaces(0, 3);
    };
  ...
  function make_jiff(base_instance, options) {
    ...
    jiff.helpers.extended_gcd = function (a, b) {
      ...
      return (function recursive_helper(a, b) {
        ...
        const temp = recursive_helper(b, jiff.helpers.mod(a, b));
        const x = temp[0];
        const y = temp[1];
        const d = temp[2];
        return [y, x.minus(y.times(a.div(b).decimalPlaces(0, 3))), d];
      })(a, b);
    };
    ...
    jiff.helpers.bLog = function (value, base) {
      ...
      const blog = value.toString(base).length;
      const test = jiff.helpers.BigNumber(base).pow(blog - 1);
      if (test.eq(value)) {
        return blog - 1;
      }
      return blog - 0.5;
    ...
    jiff.helpers.random = function (max) {
      ...
      // eslint-disable-next-line no-undef
      const magnitude = jiff.helpers.BigNumber(10).pow(precision);
      const multiple = magnitude.div(max).decimalPlaces(0, 3).times(max);

      let rand;
      do {
        // eslint-disable-next-line no-undef
        rand = jiff.helpers._BigNumber.random(precision).times(magnitude).decimalPlaces(0, 3);
    ...
    jiff.protocols.bits.bit_composition = function (bits) {
      let result = bits[0];
      let pow = jiff.helpers.BigNumber(1);
    ...
    jiff.share = function (secret, threshold, receivers_list, senders_list, Zp, share_id) {
      ...
      if (secret != null && (!secret.decimalPlaces(0, 3).eq(secret) || secret.lt(0))) {
    ...
    jiff.hooks.afterOperation[0] = function (jiff, label, msg) {
      if (label === 'share' || label === 'open') {
        ...
        for (let i = 0; i < msg['shares'].length; i++) {
