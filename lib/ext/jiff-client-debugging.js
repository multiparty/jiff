(function (exports, node) {
  ...
  exports.make_jiff = function (jiff, options) {
    var inspectorInterval = 1000 * 60 * 2; // 2 minutes
    var printThreshold = inspectorInterval; // 2 minutes

    if (options != null) {
      if (options.inspectorInterval != null) {
        inspectorInterval = options.inspectorInterval;
      }
      if (options.printThreshold != null) {
        printThreshold = options.printThreshold;
      }
    }
    ...
    jiff.inspectDebug = function (ts) {
      ...
      var f = function (obj) {
        if (ts - obj.ts > printThreshold) {
          console.log(Math.round((ts - obj.ts) / 1000), obj.stack);
        }
      };
      stacks.iter(f);
      console.log('Done!', jiff.id);
    };
    var interval = setInterval(jiff.inspectDebug, inspectorInterval);
