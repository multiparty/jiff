(function (exports, node) {
  ...
  exports.make_jiff = function (jiff, options) {
    ...
    jiff.inspectDebugMailbox = function (ts) {
      ...
      console.log('Inspector Mailbox!', jiff.id);
      var node = jiff.socket.mailbox.head;
      while (node != null) {
        var delta = ts - node.object.time;
        if (delta > inspectorMailboxInterval) {
          console.log('Message in mailbox:', node.object.label, node.object.message, delta / 1000);
        }
        node = node.next;
      }
      console.log('Done!', jiff.id);
