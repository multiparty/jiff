...
(function (exports, node) {
  ...
  function createFixedpointSecretShare(jiff_instance, share) {
    ...
    share.cmult = function (cst, op_id, div) {
      ...
      cst = share.jiff.helpers.BigNumber(cst);
      const isInteger = cst.toString().indexOf('.') === -1;
      const cstDecimal = share.jiff.share_helpers['floor'](share.jiff.share_helpers['*'](cst, magnitude));
      ...
      if (!share.jiff.has_extension('negativenumber')) {
        ...
        let result = share.legacy.cmult(cstDecimal);
        if (div !== false) {
        ...
        const mOffset = magnitude.times(share.jiff.offset);
        if (isInteger && div !== false) {
          let tmp = share.legacy.cmult(cst);
          tmp = tmp.legacy.csub(mOffset.times(cst));
          tmp = tmp.legacy.cadd(mOffset);
          return tmp;
        }

        let resultNegative = share.legacy.cmult(cstDecimal);
    ...
    share.cdivfac = function (cst) {
      ...
      const result = share.legacy.cdivfac(cst);
    ...
    share.smult = function (o, op_id, div) {
      ...
      let result = share.legacy.smult(o, op_id);

      // Negative number composing
      if (share.jiff.has_extension('negativenumber')) {
        const offset = share.jiff.share_helpers['*'](magnitude, share.jiff.offset);

        // Subtract x*offset*mag^2, y*offset*mag^2, offset^2*mag^2
        const subshare1 = share.icsub(offset).icmult(offset);
        const subshare2 = o.icsub(offset).icmult(offset);
    ...
    share.smult_bgw = function (o, op_id, reshare, div) {
      ...
      let result = share.legacy.smult_bgw(o, op_id, reshare);
    ...
    share.cxor_bit = function (cst) {
      ...
      const reduced_share = share.legacy.cdivfac(magnitude);
      const reduced_xor = reduced_share.legacy.cadd(cst).legacy.ssub(reduced_share.legacy.cmult(cst).legacy.cmult(2));
    ...
    share.cor_bit = function (cst) {
      ...
      // In both cases, multiplying by the inverse of 10^{decimal_digits} should be enough to reduce it to either 0 or 1.
      const reduced_share = share.legacy.cdivfac(magnitude);
      const reduced_xor = reduced_share.legacy.cadd(cst).legacy.ssub(reduced_share.legacy.cmult(cst));
      return reduced_xor.legacy.cmult(magnitude);
    };
    share.sxor_bit = function (o, op_id) {
      ...
      // In both cases, multiplying by the inverse of 10^{decimal_digits} should be enough to reduce it to either 0 or 1.
      const reduced_share = share.legacy.cdivfac(magnitude);
      const reduced_o = o.legacy.cdivfac(magnitude);
      const reduced_xor = reduced_share.legacy.sadd(reduced_o).legacy.ssub(reduced_share.legacy.smult(reduced_o, op_id + ':smult1').legacy.cmult(2));
      return reduced_xor.legacy.cmult(magnitude);
    };
    share.sor_bit = function (o, op_id) {
      ...
      const reduced_share = share.legacy.cdivfac(magnitude);
      const reduced_o = o.legacy.cdivfac(magnitude);
      const reduced_or = reduced_share.legacy.sadd(reduced_o).legacy.ssub(reduced_share.legacy.smult(reduced_o, op_id + ':smult1'));
    ...
    for (let i = 0; i < comparisons.length; i++) {
      ...
      share[key] = (function (key, i) {
        return function () {
          ...
          const result = share.legacy[key].apply(share, arguments);
          return result.legacy.cmult(magnitude);
        };
      })(key, i);
    }
    ...
    share.sdiv = function (o, l, op_id) {
      ...
      const increased = share.legacy.cmult(magnitude);
    ...
    share.if_else = function (val1, val2, op_id) {
      if (share.isConstant(val1)) {
        val1 = magnitude.times(val1).decimalPlaces(0, 3);
      }
      if (share.isConstant(val2)) {
        val2 = magnitude.times(val2).decimalPlaces(0, 3);
      }

      const reduce_share = share.legacy.cdivfac(magnitude);
  ...
  function make_jiff(base_instance, options) {
    ...
    if (base_instance.free_digits < options.decimal_digits) {
      if (!(options.warn === false)) {
        console.log(
          'Warning: Fixedpoint extension: not enough free_digits to perform secret multiplications/divisions/mod or constant multiplications/divisions against non-integer constants safely. Need ' +
            (options.decimal_digits - base_instance.free_digits) +
            ' more digits in Zp.'
        );
      }
    ...
    base_instance.helpers.magnitude = function (m) {
      return base_instance.helpers.BigNumber(10).pow(m).decimalPlaces(0, 3);
    };
    base_instance.helpers.fits_in_digits = function (num) {
      let magnitude = base_instance.helpers.magnitude(base_instance.decimal_digits + base_instance.integer_digits);
    ...
    base_instance.helpers.format_as_float = function (v) {
      ...
      const magnitude = base_instance.helpers.magnitude(base_instance.decimal_digits);
    ...
    base_instance.helpers.format_as_fixed = function (v) {
      ...
      let magnitude = base_instance.helpers.magnitude(base_instance.decimal_digits);
      return magnitude.times(v).decimalPlaces(0, 3);
    };
    base_instance.helpers.to_fixed = function (v) {
      v = base_instance.helpers.BigNumber(v);
      const str = v.toFixed(base_instance.decimal_digits, base_instance.helpers._BigNumber.ROUND_FLOOR);
      return base_instance.helpers.BigNumber(str);
    };

    // Speed up calculating certain popular inverses by pre-computing
    let stored_magnitude = base_instance.helpers.magnitude(base_instance.decimal_digits);
    const stored_maginv = base_instance.helpers.extended_gcd(stored_magnitude, base_instance.Zp);
    const old_extended_gcd = base_instance.helpers.extended_gcd;
    ...
    /* OPEN */
    const old_open = base_instance.open;
    base_instance.open = function () {
      const promise = old_open.apply(base_instance, arguments);
      if (promise == null) {
        return null;
      }
    ...
    /* SHARE */
    const old_share = base_instance.share;
    ...
    const previous_namespace = base_instance.extensions[base_instance.extensions.indexOf(exports.name) - 1];
    const optional_div = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      if (params.div === false) {
        params.ignore = true;
      } else {
    ...
    const cdiv_constant = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
    ...
    base_instance.preprocessing_function_map[exports.name] = {
      cmult: [{ op: 'cdiv', op_id: '', namespace: previous_namespace, handler: optional_div }],
      ...
      smult_bgw: [{ op: 'cdiv', op_id: ':reduce', namespace: previous_namespace, handler: optional_div }],

      floor: [{ op: 'cdiv', op_id: '', namespace: previous_namespace, handler: cdiv_constant }]
    };
