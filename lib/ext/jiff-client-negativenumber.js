...
(function (exports, node) {
  ...
  function createNegativeNumberSecretShare(jiff, share) {
    const hasBigNumber = jiff.has_extension('bignumber');
    const hasFixedPoint = jiff.has_extension('fixedpoint');
    const offset = share.jiff.offset;
    ...
    share.cmult = function (cst, op_id, div) {
      ...
      let result = share.negative_legacy.cmult(cst, op_id, div);
      if (hasFixedPoint) {
        return result;
      }
    ...
    share.sadd = function (o) {
      let result = share.negative_legacy.sadd(o);
    ...
    share.ssub = function (o) {
      ...
      let result = share.negative_legacy.ssub(o);
      return result.negative_legacy.cadd(offset);
    };
    ...
    share.cxor_bit = function (cst) {
      let result = share.negative_legacy.csub(offset);
    ...
    // not operator
    share.not = function () {
      let reduced = share.negative_legacy.csub(offset);
      reduced = reduced.icmult(-1).negative_legacy.cadd(1);
      return reduced.negative_legacy.cadd(offset);
    };

    // secret and constant comparisons
    const comparisons = ['slt', 'slteq', 'sgt', 'sgteq', 'seq', 'sneq', 'clt', 'clteq', 'cgt', 'cgteq', 'ceq', 'cneq', 'lt_halfprime'];
    for (let i = 0; i < comparisons.length; i++) {
      ...
      share[key] = (function (key, i) {
        return function () {
          ...
          const result = share.negative_legacy[key].apply(share, arguments);
          return result.negative_legacy.cadd(offset);
        };
      })(key, i);
    }
    ...
    function reduce_representation(share) {
      ...
      if (hasFixedPoint) {
        const magnitude = share.jiff.helpers.magnitude(share.jiff.decimal_digits);
    ...
    share.cdivfac = function (cst, op_id) {
      ...
      const signUnit = selfNegative.icmult(-2).icadd(1); // if negative this is -1, if positive this is 1
      ...
      if (!otherNegative) {
        ...
        // o is negative, correct if share is positive
    ...
    share.cdiv = function (cst, op_id, floor_down) {
      ...
      cst = hasBigNumber ? share.jiff.helpers.BigNumber(cst) : cst;
      const otherNegative = jiff.share_helpers['<'](cst, 0);
      let cstAbs = jiff.share_helpers['abs'](cst);

      // This is the result assuming both share and o are non-negative
      const resultAbs = shareAbs.negative_legacy.cdiv(cstAbs, op_id + ':cdiv');
      ...
      if (floor_down === false) {
        ...
        // Round down
        let magnitude = hasFixedPoint ? share.jiff.helpers.magnitude(share.jiff.decimal_digits) : 1;
        ...
        const need_round = resultAbs.icmult(cstAbs).isneq(shareAbs.icmult(magnitude), op_id + ':floor:sneq');
        const xorNegative = selfNegative.icxor_bit(otherNegative ? 1 : 0);

        // we must correct if (1) need rounding (i.e. cst does not divide share) (2) xorNegative (i.e. result is negative)
        const and = need_round.ismult(xorNegative, op_id + ':floor:and');
    ...
    share.smod = function (o, l, op_id) {
      ...
      let shareAbs = share.iabs(op_id + ':abs1', true);
      let negative = shareAbs.isNegative;
      shareAbs = shareAbs.result.negative_legacy.csub(offset);
      const oAbs = o.iabs(op_id + ':abs2').negative_legacy.csub(offset);

      // |share| % |o|
      let result = shareAbs.negative_legacy.smod(oAbs, l, op_id + ':smod');
    ...
    share.sdiv = function (o, l, op_id, floor_down) {
      ...
      let otherAbs = o.iabs(op_id + ':abs2', true);
      let otherNegative = otherAbs.isNegative;
      otherAbs = otherAbs.result.negative_legacy.csub(offset);

      // This is the result assuming both share and o are non-negative
      const resultAbs = shareAbs.negative_legacy.sdiv(otherAbs, l, op_id + ':sdiv');

      // Sign correction: if either share or o are negative but not both.
      selfNegative = reduce_representation(selfNegative);
      otherNegative = reduce_representation(otherNegative);
      const xorNegative = selfNegative.isxor_bit(otherNegative, op_id + ':sxor');
      const sign = xorNegative.icmult(-2).icadd(1); // if negative = -1, else = 1
      let result = resultAbs.ismult(sign, op_id + ':cor1');
      result = result.negative_legacy.cadd(offset);
      ...
      if (floor_down === false) {
        ...
        // Round down
        const magnitude = hasFixedPoint ? share.jiff.helpers.magnitude(share.jiff.decimal_digits) : 1;
        const need_round = resultAbs.ismult(otherAbs, op_id + ':floor:smult').isneq(shareAbs.icmult(magnitude), op_id + ':floor:sneq');

        // we must correct if (1) need rounding (i.e. cst does not divide share) (2) xorNegative (i.e. result is negative)
        const and = xorNegative.ismult(need_round, op_id + ':floor:and');
        return result.issub(and);
      }
    };
    ...
    share.abs = function (op_id, return_intermediate) {
      ...
      let isNegative = share.iclt(_offset, op_id + ':clt'); // 0 or 1, no offset or magnitude
      const correction = share.icsub(_offset).icmult(2);
      const result = share.issub(correction.ismult(isNegative, op_id + ':smult'));
    ...
    share.if_else = function (val1, val2, op_id) {
      ...
      const reduce_share = share.negative_legacy.csub(offset);
  ...
  function make_jiff(base_instance, options) {
    ...
    if (jiff.offset == null) {
      if (jiff.has_extension('fixedpoint')) {
        ...
        // Sanity Checks
        const maxNoMult = jiff.helpers.magnitude(jiff.integer_digits + jiff.decimal_digits).times(2);
        if (!maxNoMult.lte(jiff.Zp)) {
          let msg = 'Fixedpoint|NegativeNumber: Zp is not large enough to fit given integer and decimal parts with negative numbers. ';
          msg += 'Need Zp >= ' + maxNoMult.toString() + ' to fit parameters.';
          throw msg;
        }
        const maxWithMult = jiff.helpers.magnitude(jiff.integer_digits + 2 * jiff.decimal_digits).times(2);
        if (!maxWithMult.lte(jiff.Zp) && !(options.warn === false)) {
          const warnMsg =
            'Warning: Fixedpoint|NegativeNumber extension: not enough free_digits to perform secret multiplications/divisions or constant multiplications/divisions against non-integer constants safely. ';
    ...
    const old_open = jiff.open;
    jiff.open = function () {
      const promise = old_open.apply(jiff, arguments);
      if (promise == null) {
        return null;
      } else {
    ...
    const previous_namespace = jiff.extensions[jiff.extensions.indexOf(exports.name) - 1];
    const should_floor_down_preprocessing = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
