...
(function (exports, node) {
  ...
  function createSecretShare(jiff, share, share_helpers) {
    ...
    // added negative legacy tag 7/25
    share.cadd = function (cst) {
      try {
        var result = share.negative_legacy.cadd(cst);
        return result;
    ...
    share.smult = function (o, op_id) {
      ...
      if (hasFixedPoint) {
        ...
        var subshare1 = share.cmult(offset.times(magnitude));
        var subshare2 = o.cmult(offset.times(magnitude));
        var subconst = offset.pow(2);
        result = result.ssub(subshare1).ssub(subshare2).csub(subconst);

        // Divide by magnitude
        // Result: x*y*mag
        result = result.negative_legacy.cdiv(magnitude);
        ...
        return promise.then(
          function (v) {
            if (jiff.modules.indexOf('bignumber') > -1) {
              return v.mod(share.Zp).minus(jiff.offset);
            } else {
