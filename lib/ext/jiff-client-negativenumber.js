...
(function(exports, node) {
  ...
  function negativenumber_secret_share(negativenumber_jiff, int_part, fraction_part) {
    ...
    this.cadd = function(cst) {
      ...
      var cst = this.jiff.BigNumber(cst).plus(this.jiff.options.offset); 
    ...
    this.smult = function(o) {
      ...
      console.log("i'm in smult 2");

      // offset = new BigNumber(this.jiff.options.offset);  
      // int_part = offset.floor();
      // fraction_part = offset.minus(int_part).times(this.jiff.magnitude).floor();

      // offset_share = new negativenumber_secret_share(this.jiff, int_part, fraction_part);

      // console.log(offset_share);
      

      

      var rx_times_ry = this.smult_wrapper(o).cadd(2.0);

      // var a_2_plus_a = this.jiff.options.offset.times(this.jiff.options.offset).plus(this.jiff.options.offset)
      // console.log(a_2_plus_a);
      var rx_plus_ry = this.sadd(o);

      return rx_times_ry;
      
    };

    this.smult_wrapper = function(o){
  ...
  function make_jiff(base_instance, options) {
    ...
    if(options.offset == null) {
      if(jiff.hasBigNumber()) {
        options.offset = new BigNumber(10); 
      }
      else {
        options.offset = 10;
      }
