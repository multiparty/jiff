...
(function (exports, node) {
  ...
  function createSecretShare(jiff, share, share_helpers) {
    ...
    share.cxor_bit = function (cst) {
      var result = share.csub(share.jiff.offset);
      result = result.negative_legacy.cxor_bit(cst);
      return result.cadd(share.jiff.offset);
    };
    ...
    share.cdivfac = function (cst, op_id) {
      ...
      shareAbs = shareAbs.result.csub(share.jiff.offset);
      ...
      var result = shareAbs.negative_legacy.cdivfac(cst).cadd(share.jiff.offset);
    ...
    share.cdiv = function (cst, op_id, floor_down) {
      ...
      // This is the result assuming both share and o are non-negative
      var result = shareAbs.negative_legacy.cdiv(cstAbs, op_id + ':c/').cadd(share.jiff.offset);
      ...
      if (floor_down === false) {
        ...
        if (hasFixedPoint) {
          ...
          xorNegative = xorNegative.csub(share.jiff.offset).negative_legacy.cdivfac(magnitude);
          need_round = need_round.csub(share.jiff.offset).negative_legacy.cdivfac(magnitude);
    ...
    share.smod = function (o, l, op_id, floor_down) {
      ...
      // Absolute value of o guarantees that mod takes sign of share.
      o = o.abs(op_id+':abs');

      // By default, round to zero to get the same behavior as JavaScript.
      var quotient;
      if (floor_down == null) {
      ...
      // Finalize by computing remainder.
      return share.ssub(quotient.smult(o, op_id+':smult'));
    };
    ...
    share.sdiv = function (o, l, op_id, floor_down) {
      ...
      otherAbs = otherAbs.result.csub(o.jiff.offset);

      // This is the result assuming both share and o are non-negative
      var result = shareAbs.negative_legacy.sdiv(otherAbs, l, op_id + ':/').cadd(share.jiff.offset);
