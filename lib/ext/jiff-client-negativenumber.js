...
(function(exports, node) {
  ...
  function make_jiff(base_instance, options) {
    ...
    jiff.share = function(secret, threshold, receivers_list, senders_list, Zp) {
      ...
      if(jiff.hasBigNumber) { 
        secret = new jiff.BigNumber(secret);
        offset = new jiff.BigNumber(jiff.options.offset);  
    ...
    jiff.share_vec = function(secrets, threshold, receivers_list, senders_list, Zp) { 
      console.log("in share_vec");
      result = [];
      secrets.map(function(secret){
        result.push(jiff.share(secret, threshold, receivers_list, senders_list, Zp));
        console.log(secret);
      });
      return result;
    };
