...
(function(exports, node) {
    ...
    function createSecretShare(jiff, share, share_helpers) {
        ...
        var old_mult = share.cmult;
        share.cmult = function(c) {
            var result = old_mult(c);
            console.log("Original mult, containing c(x+o)" + result);
            var subvar = share.jiff.offset * c; // two const - can be open
            console.log("Number to subtract:" + subvar)
            return (result.cadd(-1 * subvar) + share.jiff.offset);
        }
