...
(function (exports, node) {
  ...
  function createSecretShare(jiff, share, share_helpers) {
    ...
    share.cdivfac = function (cst, op_id) {
      ...
      selfNegative = selfNegative.negative_legacy.csub(share.jiff.offset);
      if (hasFixedPoint) {
        var magnitude = share.jiff.helpers.magnitude(share.jiff.decimal_digits);
        selfNegative = selfNegative.negative_legacy.cdivfac(magnitude);
      }

      // o is positive
      if (!otherNegative) {
        // only need to correct for if share is negative
        return result.ismult(selfNegative.icmult(-2).icadd(1), op_id + ':cor1').negative_legacy.cadd(share.jiff.offset);
      } else { // o is negative
        // if share is positive, correct.
        return result.ismult(selfNegative.icmult(2).icadd(-1), op_id + ':cor1').negative_legacy.cadd(share.jiff.offset);
      }
    };
    ...
    share.smod = function (o, l, op_id) {
      ...
      var oAbs = o.abs(op_id+':abs2').negative_legacy.csub(o.jiff.offset);
