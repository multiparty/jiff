...
(function (exports, node) {
  ...
  function browserPOST(hostname, body, callback) {
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4) {
        const error = xhr.status === 200 ? null : xhr.status;
   ...
   */
  const POST = node ? nodePOST : browserPOST;
  ...
  function execute_listeners(jiff, event, msg) {
    const listeners = jiff.socket.listeners(event);
  ...
  function mailboxRestAPI(jiff) {
    ...
    jiff.socket.mailbox.merge_requests = function () {
      const mailbox = jiff.socket.mailbox;
  ...
  function socketToRestAPI(jiff) {
    ...
    jiff.socket.connect = function () {
      const connectListeners = jiff.socket.listeners('connect');
    ...
    jiff.socket.is_empty = function () {
      return jiff.socket.mailbox.pending == null && jiff.socket.mailbox.current.initialization == null && jiff.socket.mailbox.current.messages.length === 0 && jiff.counters.pending_opens === 0;
    };
  ...
  exports.make_jiff = function (jiff, options) {
    ...
    jiff.restFlush = function () {
      ...
      const messages = jiff.socket.mailbox.current.messages;
      const sliced = messages.slice(0, jiff.maxBatchSize);
      const tail = messages.slice(jiff.maxBatchSize);
      let body = {
