...
(function (exports, node) {
  ...
  function make_jiff(base_instance, options) {
    ...
    jiff.initSocket = function () {
      ...
      /* ws uses the 'open' protocl on connection. Should not conflict with the 
      ...
      this.socket.on('close', function (reason) {
        console.log("native protocol used");
        socketClose(reason);
      });
    ...
    if (options.__internal_socket == null) {
      ...
      jiff.options.__internal_socket.safe_emit = function (label, msg) {
        ...
        /* ws messages are sent with a string called 'socketProtocol' to replace the nice protocol syntax of socket.io */
