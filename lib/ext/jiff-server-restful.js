...
module.exports = {
  ...
  make_jiff: function (base_instance, options) {
    ...
    app.post('/public_key', function (req, res) {
      // TODO what if client does not receive reply to this? client side must keep trying
      var packet = req.body;
      var computation_id = packet['computation_id'];
      var from_id = packet['from_id'];
      var msg = packet['msg'];

      jiff.public_key(computation_id, from_id, msg);
      ...
      if (result.success) {
        ...
        jiff.restful.keys_changed[computation_id][from_id] = false;
