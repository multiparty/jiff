...
module.exports = {
  ...
  make_jiff: function (base_instance, options) {
    ...
    jiff.restful.handlers = {
      ...
      open: jiff.open,
      custom: jiff.custom,
      crypto_provider: jiff.crypto_provider,
      free: jiff.free
    };
    ...
    jiff.restful.poll_initialize = function (msg) {
      ...
      var output = jiff.initialize_party(msg['computation_id'], initialization['party_id'], initialization['party_count'], initialization, false);
      if (!output.success) {
        return output;
      }
