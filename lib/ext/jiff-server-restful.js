  ...
  make_jiff: function (base_instance, options) {
    ...
    jiff.restful.poll_initialize = function (msg) {
      ...
      if (initialization == null) {
        if (msg['from_id'] == null) {
          return { status: false, error: 'cannot determine party id' };
        }
        return { status: true, initialization: null, from_id: msg['from_id'] };
      }

      var output =  jiff.initialize_party(msg['computation_id'], initialization['party_id'], initialization['party_count'], initialization);
      output['from_id'] = output.message.party_id;
      return output;
    };
    jiff.restful.poll_handle_message = function (computation_id, from_id, messages) {
      for (var i = 0; i < messages.length; i++) {
        ...
        if (jiff.restful.handlers[label] == null) {
          return { status: false, error: 'Unrecognized label ' + label };
        }
        ...
        if (output.message != null) {
          jiff.hooks.put_in_mailbox(jiff, label, output.message, computation_id, from_id);
        }
      }
    ...
    app.post('/poll', function (req, res) {
      ...
      if (!output.success) { // failed to initialize
        return res.send(200).json({ success: false, error: output.error });
      }
      ...
      if (!output.success) {
        return res.send(200).json(output);
      }

      // Fourth: dump mailbox and encrypt.
      var dumped = jiff.restful.dump_mailbox(computation_id, output.message.party_id);
