...
module.exports = {
  ...
  make_jiff: function (base_instance, options) {
    ...
    const maxBatchSize = options.maxBatchSize == null ? 150 : options.maxBatchSize;
    ...
    const oldInitComputation = jiff.initComputation;
    const oldFreeComputation = jiff.freeComputation;
    ...
    jiff.restful.dumpMailbox = function (computation_id, party_id) {
      const mailbox = jiff.hooks.getFromMailbox(jiff, computation_id, party_id);
      if (mailbox.length === 0) {
        return { messages: [] };
      }

      const messages = [];
      const count = Math.min(mailbox.length, maxBatchSize);
      ...
      const tag = (jiff.restful.maps.tags[computation_id][party_id] + 1) % LARGE;
    ...
    jiff.restful.freeTag = function (computation_id, party_id, tag) {
      if (tag != null && jiff.restful.maps.pendingMessages[computation_id] != null && jiff.restful.maps.pendingMessages[computation_id][party_id] != null) {
        const pending = jiff.restful.maps.pendingMessages[computation_id][party_id];
        if (tag === pending.tag) {
    ...
    jiff.restful.initializeParty = function (msg) {
      const initialization = msg['initialization'];
      ...
      const output = jiff.handlers.initializeParty(msg['computation_id'], initialization['party_id'], initialization['party_count'], initialization, false);
      if (!output.success) {
        return output;
      }
    ...
    jiff.restful.handleMessage = function (computation_id, from_id, messages) {
      for (let i = 0; i < messages.length; i++) {
        ...
        // handle message
        const output = jiff.restful.handlers[label](computation_id, from_id, payload);
    ...
    app.post('/poll', function (req, res) {
      ...
      let output = jiff.restful.initializeParty(msg);
      if (!output.success) {
      ...
      let response = { success: true, initialization: output.message };
      const computation_id = msg['computation_id'];
      const from_id = output.party_id;
      ...
      const dumped = jiff.restful.dumpMailbox(computation_id, from_id);
