...
module.exports = {
  ...
  make_jiff: function (base_instance, options) {
    ...
    jiff.restful.dumpMailbox = async function (computation_id, party_id) {
      var mailbox = await jiff.hooks.getFromMailbox(jiff, computation_id, party_id);
      if (mailbox.length === 0) {
        return { messages: [] };
      }
    ...
    jiff.restful.initializeParty = async function (msg) {
      ...
      var output = await jiff.handlers.initializeParty(msg['computation_id'], initialization['party_id'], initialization['party_count'], initialization, false);
      if (!output.success) {
        return output;
      }
    ...
    jiff.restful.handleMessage = async function (computation_id, from_id, messages) {
    ...
    app.post('/poll', async function (req, res) {
      ...
      // Execute start hooks
      try {
        msg = await jiff.hooks.execute_array_hooks('beforeOperation', [jiff, 'poll', msg['computation_id'],  msg['from_id'], msg], 4);
      } catch (err) {
        return res.json({ success: false, label: 'poll', error: typeof(err) === 'string' ? err : err.message });
      }
      // First: attempt to initialize if needed.
      var output = await jiff.restful.initializeParty(msg);
      ...
      output = await jiff.restful.handleMessage(computation_id, from_id, msg.messages);
      if (!output.success) {
        return res.json(output);
      }
      // Fourth: dump mailbox and encrypt.
      var dumped = await jiff.restful.dumpMailbox(computation_id, from_id);
      response = Object.assign(response, dumped);
      // Execute end hooks
      response = await jiff.hooks.execute_array_hooks('afterOperation', [jiff, 'poll', computation_id, from_id, response], 4);
      // Respond back!
      res.json(response);
    });
  }
