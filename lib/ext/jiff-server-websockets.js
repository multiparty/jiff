...
(function (exports, node) {
  ...
  function make_jiff(base_instance, options) {
    ...
    jiff.initSocket = function () {
      ...
      const wss = new WebSocket.Server({ server: this.http, clientTracking: true });
      ...
      this.io.on('connection', function (socket, req) {
        ...
        function initialization(msg) {
          ...
          // read message
          const computation_id = msg['computation_id'];
          let party_id = msg['party_id'];
          const party_count = msg['party_count'];
          ...
          const output = jiff.handlers.initializeParty(computation_id, party_id, party_count, msg, false);
          // END OF COMPUTATION
        ...
        function share(msg, callback) {
          const computation_id = jiff.socketMaps.computationId[socket.id];
          const from_id = jiff.socketMaps.partyId[socket.id];

          const output = jiff.handlers.share(computation_id, from_id, msg);
          if (!output.success) {
            const errorMsg = JSON.stringify({ label: 'share', error: output.error });
        ...
        function socketOpen(msg, callback) {
          ...
          const output = jiff.handlers.open(computation_id, from_id, msg);
          if (!output.success) {
            const errorMsg = JSON.stringify({ label: 'open', error: output.error });
            jiff.emit('error', errorMsg, computation_id, from_id);
          }
        }
        ...
        function socketCustom(msg, callback) {
          ...
          const output = jiff.handlers.custom(computation_id, from_id, msg);
          if (!output.success) {
            const errorMsg = JSON.stringify({ label: 'custom', error: output.error });
            jiff.emit('error', errorMsg, computation_id, from_id);
          }
        }
        ...
        function crypto_provider(msg, callback) {
          ...
          const res = jiff.handlers.crypto_provider(computation_id, from_id, msg);
          if (!res.success) {
            const errorMsg = JSON.stringify({ label: 'crypto_provider', error: res.error });
            jiff.emit('error', errorMsg, computation_id, from_id);
          }
        }
        ...
        function free(msg, callback) {
          ...
          const output = jiff.handlers.free(computation_id, from_id, msg);
          if (!output.success) {
            const errorMsg = JSON.stringify({ label: 'free', error: output.error });
            jiff.emit('error', errorMsg, computation_id, from_id);
          }
    ...
    jiff.hooks.onInitializeUsedId = function (jiff, computation_id, party_id, party_count, msg) {
      ...
      const previous_socket = jiff.socketMaps.socketId[computation_id][party_id];
    ...
    jiff.emit = function (label, msg, computation_id, to_id, callback) {
      ...
      const socket_to_use = this.socketMaps.socketId[computation_id][to_id]; // socket to use
      if (socket_to_use == null) {
        return;
      }

      const socket = socket_to_use;
      const message = { socketProtocol: label, data: msg };
      if (socket != null) {
        if (callback == null) {
