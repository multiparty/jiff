  ...
  function make_jiff(base_instance, options) {
    ...
    jiff.initSocket = function () {
      ...
      this.io.on('connection', function (socket, req) {
        ...
        async function initialization(msg) {
          ...
          if (output.success) {
            ...
            await jiff.resend_mailbox(computation_id, party_id);
          } else {
        ...
        async function socketOpen(msg, callback) {
          var computation_id = jiff.socketMaps.computationId[socket.id];
          var from_id = jiff.socketMaps.partyId[socket.id];

          var output = await jiff.handlers.open(computation_id, from_id, msg);
          if (!output.success) {
        ...
        async function socketCustom(msg, callback) {
          var computation_id = jiff.socketMaps.computationId[socket.id];
          var from_id = jiff.socketMaps.partyId[socket.id];

          var output = await jiff.handlers.custom(computation_id, from_id, msg);
          if (!output.success) {
        ...
        async function crypto_provider(msg, callback) {
          var computation_id = jiff.socketMaps.computationId[socket.id];
          var from_id = jiff.socketMaps.partyId[socket.id];
        ...
        async function free(msg, callback) {
          var computation_id = jiff.socketMaps.computationId[socket.id];
          var from_id = jiff.socketMaps.partyId[socket.id];
