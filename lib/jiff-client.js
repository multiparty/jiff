...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
     ...
     *
     * @example
     * var shares = jiff_instance.share(input, null, [1,2], [1,2]);
     * // this will be 1 if party 1's input is greater tha party 2's, 0 otherwise
     * var comparison = shares[1].gt(shares[2]);
     * // max is set to the (secret shared) input from party 1 or 2
     * // based on the result of the comparison, which is still secret
     * var max = comparison.if_else(shares[1], shares[2])
     * // reveal the larger value
     * return jiff_instance.open(max);
     */
   ...
   * var jiff_instance = jiff.make_jiff('http://localhost:8080', 'first_compuation', {party_count: 2}
   */
