...
(function (exports, node) {
  ...
  */
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    //jiff.preprocessing_function_map['xor'] = jiff.preprocessing_function_map['sxor'];
    ...
    //jiff.preprocessing_function_map['or'] = jiff.preprocessing_function_map['sor'];
    ...
    self.smult = function (o, op_id) {
      ...
      if (triplet == null) {
        console.log('could not find preprocessed value');
    ...
    /*jiff.preprocessing_function_map['sxor'] = [
      { op:'smult', op_id : 'smult1'},
    ];*/

    /**
     * bitwise-XOR of two secret shares OF BITS. Relies on smult_bgw instead of smult in order to avoid the need for preprocessed values.
     * @method sxor_bgw
     * @param {SecretShare} o - the share to XOR with.
     * @param {string} [op_id=auto_gen()] - the operation id which is used to identify this operation.
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
     ...
     * @instance
     */
    self.sxor_bgw = function (o, op_id) {
      if (!(o.jiff === self.jiff)) {
        throw new Error('shares do not belong to the same instance (^)');
      }
      if (!self.jiff.helpers.Zp_equals(self, o)) {
      ...
      return self.isadd(o).issub(self.ismult_bgw(o, op_id).icmult(2));
    };
    /**
    ...
    /*jiff.preprocessing_function_map['sor'] = [
      { op:'smult', op_id : 'smult1'},
    ];*/

    /**
     * OR of two secret shares OF BITS. Relies on smult_bgw in order to avoid preprocessing.
     * @method sor_bgw
     * @param {SecretShare} o - the share to OR with.
     * @param {string} [op_id=auto_gen()] - the operation id which is used to identify this operation.
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
    ...
    self.sor_bgw = function (o, op_id) {
      ...
      return self.isadd(o).issub(self.ismult_bgw(o, op_id));
    };
    /**
    ...
    //jiff.preprocessing_function_map['>='] = jiff.preprocessing_function_map['<'];
    /**
    ...
    /*
    PHASE 1: [smult, slt,] slteq, sgt, sgteq, seq, sneq and their constant versions, cdiv, sxor_bit, sor_bit
    KINAN: rename all gen_op_ids('symbol') to gen_op_ids('func_name')
    preprocessing_bla['<'] = [
      { lt_halfprime, ':halfprime:1' },
      { lt_halfprime, ':halfprime:2' },
      { lt_halfprime, ':halfprime:3' },
      { 'smult', ':smult1' },
      { lt_halfprime, ':smult2' },
    ];
    preprocessing_bla['smult'] = [
      { 'beaver': '' }
    ]
    preprocessing_bla['>=']= preprocessing_bla['<'] USE THIS FOR ALIASING

    precpressong_bla['beaver'] = null; // BY DEFAULT
    // when preprocessing functions looks for 'beaver', it wont find it
    //  and then it will know it is a base case and it should look for it in the user-provided (or default) protocols
    */
    ...
    //jiff.preprocessing_function_map['c>='] = jiff.preprocessing_function_map['>='];
    ...
    // reference for pre_processing
    /*jiff.preprocessing_function_map['c>'] = [
      { op : 'lt_halfprime', op_id : ':halfprime:1'},
      { op : 'lt_halfprime', op_id : ':halfprime:2'},
      { op : 'smult', op_id : ':smult1'},
    ];*/
    ...
    //jiff.preprocessing_function_map['c<='] = jiff.preprocessing_function_map['c>'];
    ...
    //jiff.preprocessing_function_map['='] = jiff.preprocessing_function_map['c<='];
    ...
    self.sdiv = function (o, l, op_id) {
      ...
      var q = self.jiff.server_generate_and_share({count: 1, number: 0}, self.holders, max(self.threshold, o.threshold), self.Zp, op_id + ':number')[0];
      var a = self; // dividend
    ...
    /*
    PHASE 2: sdiv, smod, lt_halfprime, clt_bits
     preprocess_bla['sdiv'] = [
      { op: 'number', param: { 'number': 0 }, op_id: ':number' },
      { op: 'sdiv_per_bit', op_id: '', repeatBits: true }
    ]

    preprocess_bla['sdiv_per_bit'] = [
      { op: 'smult', op_id: ':smult1:' },
      { op: 'smult', op_id: ':smult1:0' },
      { op: 'smult', op_id: ':smult1:0' },
      { op: 'smult', op_id: ':smult1:0' },
      { op: 'smult', op_id: ':smult1:0' }
     ]
    ...
    self.lt_halfprime = function (op_id) {
      ...
      /*var bitLength = share_helpers['floor'](self.jiff.helpers.bLog(share.Zp, 2)); // TODO: this leaks one bit, fix it for mod 2^n (add 1)
...
*/
      ...
      share.jiff.internal_open(r.isadd(share), share.holders, op_id + ':open').then(function (result) {
        var wrapped = self.jiff.protocols.clt_bits(result, bits, op_id + ':clt_bits');
    ...
    var internals = ['cadd', 'csub', 'cmult', 'sadd', 'ssub', 'smult', 'smult_bgw',
      'cxor_bit', 'sxor_bit', 'sxor_bgw','cor_bit', 'sor_bit', 'sor_bgw',
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.generate_beaver_bgw = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      ...
      if (compute_list.indexOf(jiff.id) > -1) {
        ...
        c = a.smult_bgw(b, op_id + ':smult_bgw');
      }
     ...
     */
    jiff.protocols.generate_random_bit = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      if (params == null) {
        params = {};
      }
      if (op_id == null) {
      ...
      // sxor_bgw forces a threshold of n/2 or less
      var compute_threshold = params.compute_threshold;
      if (compute_threshold == null) {
      ...
      if (compute_list.indexOf(jiff.id) > -1) {
        ...
        // reduce the threshold to make bgw possible -> won't support n-1 security
        var bit_shares = jiff.internal_share(bit, compute_threshold/2, compute_list, compute_list, Zp, op_id + ':share');

        random_bit = bit_shares[compute_list[0]];
        for (var i = 1; i < compute_list.length; i++) {
          var party_id = compute_list[i];
          random_bit = random_bit.sxor_bgw(bit_shares[party_id], op_id + ':xor' + i);
        }
      }
    ...
    jiff.protocols.generate_bit_sequence = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      ...
      // super hack way of pre_processing this
      var promises = [];
      var bits = [];
      for (var i = 0; i < params.count; i++) {
        bits.push(jiff.protocols.generate_random_bit(threshold, receivers_list, compute_list, Zp, op_id + ':bitsequence:' + i));
        //console.log('promise check: ', bits[i].promise);
        promises.push(bits[i].promise); // don't know if this should wait on all promises to return
      }
      return { op_id: op_id + ':bitsequence', share: bits, promise: Promise.all(promises) };
    };
    /**
    ...
    jiff.protocols.clt_bits = function (constant, bits, op_id) {
      ...
      for (i = c.length - 2; i >= 0; i--) {
        d[i] = d[i + 1].isor_bit(c[i], op_id + ':sOR:' + i); //does the op_id need to increment?
        //other option is rely on sor_bit and find a way to preprocess for a dynamic number of sor_bit operations
      }
    ...
    jiff.preprocessing_function_map = {
      ...
      'sxor_bit' : [
        { op:'smult', op_id:''} // no op_id suffix according to sxor_bit implementation
      ],
      'sor_bit' : [
        { op:'smult', op_id:':smult1'}
      ],
      'lt_halfprime' : [
        { op:'generate_bit_sequence', op_id:':bitsequence', params: {
          bit: true,
          count: 24, // number of bits in default gZp, should be dynamic plz
        }},
        { op:'smult', op_id: ':smult1' },
      ...
      'clt_bits': [
        { op:'sor_bit', op_id: ':sOR:', count: 24} // count should be calculated dynamically
      ],
    };

    // inequality
    jiff.preprocessing_function_map['slteq'] = jiff.preprocessing_function_map['slt'];
    jiff.preprocessing_function_map['sgteq'] = jiff.preprocessing_function_map['slt'];
    jiff.preprocessing_function_map['sgt'] = jiff.preprocessing_function_map['slt'];
    // constant inequality
    jiff.preprocessing_function_map['clteq'] = jiff.preprocessing_function_map['cgt'];
    jiff.preprocessing_function_map['cgteq'] = jiff.preprocessing_function_map['clt'];
    // equality / constant equality
    jiff.preprocessing_function_map['seq'] = jiff.preprocessing_function_map['clteq'];
    jiff.preprocessing_function_map['sneq'] = jiff.preprocessing_function_map['seq'];
    jiff.preprocessing_function_map['ceq'] = jiff.preprocessing_function_map['clteq'];
    jiff.preprocessing_function_map['cneq'] = jiff.preprocessing_function_map['ceq'];

    jiff.preprocessing_function_map['or'] = jiff.preprocessing_function_map['sor']; // would not worry about secret/constant neutral versions for now
    jiff.preprocessing_function_map['xor'] = jiff.preprocessing_function_map['sxor'];
    ...
    jiff.get_preprocessing = function (op_id) {
      ...
      // TODO: do some fallback according to user.
      // return jiff.triplet(self.holders, max(self.threshold, o.threshold), self.Zp, op_id + ':triplet');
      // the problem w/ this is there is no good way to determine what sort of value/object you need from the op_id. parsing it for relevant text isn't possible because it may be a nested operation
      // need a map like if ':triplet' = jiff.triplet()
    };
     ...
     */
    jiff.preprocessing = function (dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, id_list, params) {
      // defaults
      if (receivers_list == null) {
        receivers_list = [];
        for (var p = 1; p <= jiff.party_count; p++) {
      ...
      } // TODO: put default protocols for generating bits

      console.log('dependent_op: ', dependent_op);
      ...
      for (var i = 0; i < count; i++) {
        ...
        var preprocessing_dependencies = jiff.preprocessing_function_map[dependent_op];
        if (preprocessing_dependencies == null) {
          ...
          for (var k = 0; k < preprocessing_dependencies.length; k++) {
            ...
            //console.log('recursively computing for ', dependent_op, 'recursive:', dependency['op']);
            ...
            jiff.preprocessing(next_op, next_count, protocols, threshold, receivers_list, compute_list, Zp, next_id_list, extra_params);
     ...
     * @instance
     */
    jiff.finish_preprocessing = function (op, protocol, count, id_list) {
      jiff.counters.reset();
    };
