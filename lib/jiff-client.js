...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.smod = function (o, l, op_id) {
      ...
      // Set the new threshold for the resulting share
      remainder.threshold = max(self.threshold, o.threshold);
    ...
    self.bit_decomposition = function (op_id) {
      ...
      for (var i = 0; i < bitLength; i++) {
        ...
        result[i] = self.jiff.secret_share(self.jiff, false, deferred[i].promise, undefined, self.holders, self.threshold, self.Zp, 'share:' + i + ':' + op_id);
      }
      ...
      self.jiff.internal_open(r.isadd(self), self.holders, op_id + ':open').then(function (result) {
        ...
        for (var i = 0; i < bitLength; i++) {
          var bit_i = didWrap.iif_else(withWrap[i], noWrap[i], op_id + ':if_else:' + i);
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.bits.rejection_sampling = function (lower_bound, upper_bound, threshold, compute_list, Zp, op_id) {
      ...
      for (var i = 0; i < finalLength; i++) {
        ...
        result[i] = jiff.secret_share(jiff, false, deferreds[i].promise, undefined, compute_list, threshold, Zp, 'share:' + i + ':' + op_id);
      }

      // when result is ready, use this to resolve the returned array
      var resolve_sampling = function (bits) {
        for (var i = 0; i < deferreds.length; i++) {
          if (bits[i].ready) {
            deferreds[i].resolve(bits[i].value);
          } else {
            bits[i].promise.then(function (i) {
              deferreds[i].resolve(bits[i].value);
            }.bind(null, i));
          }
        }
      ...
      (function resample(reject_count) {
        ...
        if (jiff.helpers.bLog(range, 2).toString().indexOf('.') === -1) {
          // special case: range is a power of 2, no need to reject!
          resolve_sampling(bits_add);
          return;
        }
