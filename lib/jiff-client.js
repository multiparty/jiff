...
(function (exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * @param {object} [protocols=defaults] -
     ...
     */
    jiff.protocols.bits.rejection_sampling = function (threshold, receivers_list, compute_list, Zp, op_id, params, protocols) {
      // Defaults
      if (compute_list == null) {
        compute_list = [];
        for (var p = 1; p <= jiff.party_count; p++) {
      ...
      var promises = [];
      if (compute_list.indexOf(jiff.id) > -1) {
        result = jiff.protocols.bits.__rejection_sampling(lower_bound, upper_bound, params.compute_threshold, compute_list, Zp, op_id, params, protocols);
    ...
    /**
     * internal helper for rejection_sampling, do not call directly.
     * @method __rejection_sampling
     * @memberof jiff-instance.protocols.bits
     * @instance
     * @param lower_bound
     * @param upper_bound
     ...
     * @private
     */
    jiff.protocols.bits.__rejection_sampling = function (lower_bound, upper_bound, threshold, compute_list, Zp, op_id, params, protocols) {
      ...
      if (protocols.bits == null) {
        protocols.bits = jiff.protocols.generate_bit_sequence;
      }
      if (protocols.sampling == null) {
        protocols.sampling = jiff.protocols.bits.rejection_sampling;
      }
      ...
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('__rejection_sampling', compute_list);
      }

      threshold = compute_list.length;
      Zp = Zp != null ? Zp : jiff.Zp;
      ...
      (function resample(reject_count) {
        var bits = jiff.protocols.generate_bit_sequence(threshold, compute_list, compute_list, Zp, op_id + ':sampling:' + reject_count, {count: bitLength}).share;
        ...
        params['bitLength'] = bits.length;
        params['namespace'] = 'base';
        var promise = jiff.__preprocessing('bits.clt', 1, protocols, threshold, compute_list, compute_list, Zp, [ op_id + ':bits.clt:' + reject_count ], params);
        promise.then(function () {
          var bits_add = bits;
          ...
          var cmp = jiff.protocols.bits.clt(bits, range, op_id + ':bits.clt:' + reject_count);
    ...
    var cgteq_bits_count = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      var bitLength = params.bitLength;
      if (bitLength == null) {
        bitLength = jiff.helpers.bLog(Zp, 2).toString(10);
        bitLength = Math.floor(Number(bitLength));
      }
      return bitLength;
    };
    ...
    jiff.preprocessing_function_map = {
      base: {
        ...
        lt_halfprime : [
          { op: 'bits',  op_id: ':bitsequence', handler: lt_halfprime_handler },
          { op: 'smult', op_id: ':smult1' },
        ...
        'bits.cgteq': [
          { op: 'smult', op_id: ':smult:', count: cgteq_bits_count }
        ]
      }
    };
    ...
    jiff.get_preprocessing = function (op_id) {
      ...
      if (true || jiff.crypto_provider === true) {
        return null;
      }
    ...
    jiff.__preprocessing = function (dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, id_list, params) {
      ...
      for (var i = 0; i < count; i++) {
        ...
        if (namespace == null) {
          ...
          var result = protocol(threshold, receivers_list, compute_list, Zp, id, params, protocols);
