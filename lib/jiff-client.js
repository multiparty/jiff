...
(function (exports, node) {
  ...
  */
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
     ...
     *                         is generated by increasing a local counter, default ids suffice when all parties execute the
     *                         instructions in the same order.
     * @param {Object} triplet_set -  [Optional] an object mapping op_ids to beaver tripets that were generated during pre-processing. If no object is provided, the party will ask the server to generate a beaver triplet for the multiplication. Additionally, if there is no triplet mapped to the operation's op_id, the client will fall back on server-provided triplets;
     * @return {SecretShare} this party's share of the result.
     * @memberof SecretShare
     * @instance
     */
    self.smult = function (o, op_id) {
      ...
      var triplet;
      if (self.jiff.preprocessing_table.triplets['preprocess' + op_id] == null) {

        triplet = jiff.triplet(self.holders, max(self.threshold, o.threshold), self.Zp, op_id + ':triplet');
      } else {
        triplet = self.jiff.preprocessing_table.triplets['preprocess' + op_id];
      }
     ...
     * @method reshare
     * @param {number} threshold - the new threshold. Defaults to the length of receivers_list param
     * @param {Array} [receivers_list] - array of party ids to receive from, by default, this includes all parties.
     * @param {string} [op_id=auto_gen()] - the operation id which is used to identify this multiplication (and internally, the corresponding beaver triplet).
     *                         This id must be unique, and must be passed by all parties to the same instruction.
     *                         this ensures that every party gets a share from the same triplet for every matching instruction. An automatic id
     *                         is generated by increasing a local counter, default ids suffice when all parties execute the
     ...
     * @instance
     */
    self.reshare = function (threshold, receivers_list, op_id) {
      if (op_id == null) {
      ...
      var ready_threshold = function () {
        var intermediate_shares = self.jiff.internal_share(self.value, threshold, receivers_list, self.holders, self.Zp, op_id);
        var promises = [];
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.generate_beaver_bgw = function (compute_threshold, compute_list, receiving_threshold, receivers_list, Zp, op_id) {
      ...
      // Should this happen within generate_beaver_bgw or within preprocessing? my feeling is within pre_processing bc you may run this protocol with all parties and don't want to do unnecessary work, or not during pre_processing etc
      return [a, b, c];
    };
     ...
     *
     */
    jiff.protocols.generate_random_bit_sequence = function (compute_threshold, compute_list, receiving_threshold, receivers_list, Zp, op_id) {
     ...
     * @param {string} label - location generated values will be stored in the pre_processing table. Pre-defined are {triplets, tand_bits, bit_sequences, rand_nums}.
     ...
     */
    jiff.preprocessing = function (label, op, protocol, count, id_list) {
      //iterate count times through op and put results in jiff.preprocessing_table.label. for the first id_list.length ids provided
      var holders = [];
      for (var i = 1; i <= jiff.party_count; i++) {
        holders.push(i);
      }
      ...
      for (i = 0; i < count; i++) {
        var id = id_list[i];
        if (id == null) {
          id = jiff.counters.gen_op_id(op, holders);
        }
        jiff.preprocessing_table[label][id] = protocol(null, null, null, null, null, 'preprocess' + id);
