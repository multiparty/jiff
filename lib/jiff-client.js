...
(function (exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     *
     */
    jiff.protocols.generate_random_bit = function (bit, threshold, receivers_list, compute_list, Zp, triplet_id) {
      var i;
      ...
      var bit_shares = jiff.internal_share(bit, threshold, compute_list, compute_list, Zp, triplet_id + ':share');

      var random_bit = bit_shares[compute_list[0]];
      for (i = 1; i < compute_list.length; i++) {
        var party_id = compute_list[i];
        random_bit = random_bit.sxor_bit(bit_shares[party_id], triplet_id + ':' + i);
      }
