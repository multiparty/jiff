...
(function(exports, node) {
  ...
  function jiff_broadcast(jiff, share, parties, op_ids) {
    ...
    for(var index = 0; index < parties.length; index++) {
      ...
      if(i == jiff.id) { send_to_self = true; continue; }
    ...
    if(send_to_self) receive_open(jiff, jiff.id, share.value, op_ids[jiff.id], share.Zp);
  }
  ...
  function jiff_server_share_number(jiff, options, receivers_list, threshold, Zp) {
    ...
    var share = jiff.secret_share(jiff, false, deferred.promise(), undefined, receivers_list, threshold, Zp);
    return share;
  }
  ...
  function jiff_coerce_to_share(jiff, number, holders, Zp) {
    ...
    return jiff.secret_share(jiff, true, null, number, holders, 1, Zp);
  }
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    // Setup parties to wait for
    jiff.wait_for = options.wait_for == null ? options.wait_for : options.wait_for.slice(0); // slice(0) = shallow copy
    ...
    jiff.connect = function() {
      ...
      // identification
      jiff.socket.connect();
      jiff.socket.emit("computation_id", JSON.stringify({ "computation_id": jiff.computation_id, "party_id": jiff.id, "party_count": jiff.party_count }));
    }
    if(!(options.autoConnect === false)) jiff.connect();
    
    /**
     * Reconnect to the server if it was previously disconnected
     * @method reconnect
     * @memberof jiff.jiff-instance
     */
    jiff.reconnect = function() {
      if(!jiff.socket.connected) jiff.socket.connect();
      jiff.socket.emit("reconnect1", JSON.stringify({ "computation_id": computation_id, "party_id": jiff.id }));
    }
    ...
    jiff.emit = function(tag, receivers, message) {
      ...
      jiff.socket.emit("custom", JSON.stringify( {'tag': tag, 'receivers': receivers, 'message': message } ));
    };

    // custom message that have no listeners are stored here, then passed to the given listener.
    var custom_mailbox = {}; // tag -> array of senders and messages.
    ...
    jiff.listen = function(tag, handler) {
      ...
      // handle previously received messages
      var stored_messages = custom_mailbox[tag];
      if(stored_messages == null) return;
      while(stored_messages.length > 0) {
        var msg = stored_messages[0];
        if(jiff.listeners[tag] != null) { // This is needed since the listener called on the previous message may remove/change the listener
          jiff.listeners[tag](msg["sender_id"], msg["message"]);
          stored_messages.splice(0, 1); // remove first element
        }
      }
    }
     ...
     */
    jiff.helpers.pow_mod = function(base, pow, m) {
      if(pow == 0) return 1;

      var value = base;
      while(pow > 0) {
        if(pow % 2 == 0) {
          value = jiff.helpers.mod(value * value, m);
          pow = pow / 2;
        } else {
          value = jiff.helpers.mod(value * base, m);
          pow = pow - 1;
        }        
      }
    ...
    jiff.receive_open = function(parties, threshold, Zp) {
      if(Zp == null) Zp = jiff.Zp;
      return jiff_open(jiff, jiff.secret_share(jiff, true, null, null, parties, (threshold == null ? parties.length : threshold), Zp), [ jiff.id ]);
    };
    ...
    jiff.socket.on('public_key', function(msg) {
      sodium_promise.then(function() {
        // Make sure to have a wait for array
        if(jiff.wait_for == null) {
          jiff.wait_for = [ "s1" ];
          for(var i = 1; i <= jiff.party_count; i++) jiff.wait_for.push(i);
        }
        ...
        for(var party_id in msg) {
          if(msg.hasOwnProperty(party_id)) {
            jiff.keymap[party_id] = new Uint8Array(JSON.parse(msg[party_id]));
            var tmp = [];
            for(var i = 0; i < jiff.wait_for.length; i++) 
              if(party_id != jiff.wait_for[i]) tmp.push(jiff.wait_for[i]);
            jiff.wait_for = tmp;
          }
        }

        // Nothing to wait for anymore
        if(jiff.wait_for.length == 0 && !jiff.__ready) {
          jiff.__ready = true;
          jiff.listen();
    ...
    jiff.listen = function() {
      ...
      jiff.socket.on('custom', function(msg) {
        var json_msg = JSON.parse(msg);
        ...
        else { // no listener, put in mailbox
          if(custom_mailbox[tag] == null) custom_mailbox[tag] = [];
          custom_mailbox[tag].push({"sender_id": sender_id, "message": message});
        }
      });
