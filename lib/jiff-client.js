...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.sdiv = function(o, l, op_id) {
      ...
      //call div_helper, first array entry is the quotient
      return self.div_helper(o, l, op_id)[0];
    };

    /**
     * Integer modulus with two shares (self / o)
     * @method smod
     * @param {secret-share} o - the modulus to apply
     * @param {number} l - the maximum bit length of the answer. [optional]
     * @param {string} op_id - the operation id which is used to identify this operation.
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
     *                         ensure that corresponding instructions accross different parties are matched correctly [optional].
    ...
    self.smod= function(o, l, op_id) {
      ...
      //call div_helper, second array entry is the remainder
      return self.div_helper(o, l, op_id)[1];
    };

    /**
     * helper function which performs integer division and returns both quotient and remainder
     * @method div_helper
     * @param {secret-share} o - the share to divide by.
     * @param {number} l - the maximum bit length of the answer. [optional]
     * @param {string} op_id - the operation id which is used to identify this operation.
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
     *                         ensure that corresponding instructions accross different parties are matched correctly [optional].
     * @return {secret-share[]} array this party's share of the quotient and the remainder, in that order.
     * @memberof secret-share
     */
    self.div_helper = function(o, l, op_id) {
      if(op_id == null)
        op_id = self.jiff.counters.gen_op_id("/", self.holders);
      ...
      var final_deferred_remainder = $.Deferred();
      var final_promise_remainder = final_deferred_remainder.promise();
      var remainder = self.jiff.secret_share(self.jiff, false, final_promise_remainder, undefined, self.holders, max(self.threshold, o.threshold), self.Zp, "share:"+op_id);
      ...
      (function one_bit(i) {
        if(i >= l) {
          // we did this for all bits, q has the answer (if operation == 'mod' || not 'div' then return a)
          /*if(q.ready) final_deferred.resolve(q.value);
          else q.promise.then(function() { final_deferred.resolve(q.value); });

          if(a.ready) final_deferred_remainder.resolve(a.value);
          else a.promise.then(function() { final_deferred_remainder.resolve(a.value); });*/
          Promise.all([q.promise, a.promise]).then( function() {
            final_deferred_remainder.resolve(a.value)
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * The returned op_id will be unique with respect to other operations, and identifies the same
     * operation accross all parties, as long as all parties are executing instructions in the same order.
