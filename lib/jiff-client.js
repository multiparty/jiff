...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.cdiv = function (cst, op_id) {
      ...
      if (share_helpers['<='](cst, 0)) {
        throw 'divisor must be > 0 (cst/): ' + cst;
      }

      if (share_helpers['<='](self.Zp, cst)) {
        throw 'divisor must be < share.Zp (' + self.Zp + ') in (cst/): ' + cst;
      }
     ...
     */
    self.if_else = function (trueVal, falseVal) {
      ...
      if (const1 && const2) {
        ...
        return self.inot().ismult(falseVal.icsub(trueVal)).icadd(trueVal);
      } else if (const2) {
        return self.ismult(trueVal.icsub(falseVal)).icadd(falseVal);
      } else {
        return self.ismult(trueVal.issub(falseVal)).isadd(falseVal);
      }
    };
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    var guard_socket = function (socket) {
      ...
      socket.safe_disconnect = function (callback) {
    ...
    jiff.disconnect = function (safe, free, callback) {
      if (free === true) {
        jiff.socket.safe_emit('free', '');
      }

      if (safe) {
        jiff.socket.safe_disconnect(callback);
        if (jiff.triplets_socket !== jiff.socket) {
          jiff.triplets_socket.safe_disconnect();
        }
        if (jiff.numbers_socket !== jiff.socket) {
          jiff.numbers_socket.safe_disconnect();
        }
      } else {
