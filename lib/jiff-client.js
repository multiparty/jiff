...
(function(exports, node) {
   ...
   */
  function jiff_share_array(jiff, array, callback, padding, /*parameters for the 
    ...
    // compute the maximum length of the shared arrays secretly.
    var maximumLengthShare = jiff.share(array.length); console.log("sharing", array.length); console.log(maximumLengthShare);
    var max = maximumLengthShare[1];
    for(var i = 2; i <= jiff.party_count; i++) {
      // if(!max[i])
      //   continue;
      var comparison = max.gt(maximumLengthShare[i]);
      max = maximumLengthShare[i].add((max.sub(maximumLengthShare[i])).mult(comparison));
    }
    max.open(function(max_opened, failure) { console.log("max opened"); console.log(max_opened);
      ...
      // jiff.share any needed empty shares
      if(difference > 0) {
      ...
      share_array_promise.then(arrayOfShares, failure);
    });
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * @returns {promise} the promise of the array of shares.
     */
    jiff.share_array = function(array, callback, padding, threshold, receivers_list, senders_list, Zp, share_id) { return jiff_share_array(jiff, array, callback, padding, threshold, receivers_list, senders_list, Zp, share_id); }
