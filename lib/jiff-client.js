...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.sdiv = function (o, l, op_id) {
      ...
      var final_deferred = new Deferred();
      var result = self.jiff.secret_share(self.jiff, false, final_deferred.promise, undefined, self.holders, max(self.threshold, o.threshold), self.Zp, 'share:' + op_id);
      var promise_bits = [self.bit_decomposition(op_id + ':decomposition1'), o.bit_decomposition(op_id + ':decomposition2')];
      Promise.all(promise_bits).then(function (decomposed_bits) {
        var dividend_bits = decomposed_bits[0].splice(-l);
        var divisor_bits = decomposed_bits[1].splice(-l);
        ...
        if (quotient.ready) {
          final_deferred.resolve(quotient.value);
        } else {
          quotient.promise.then(function () {
            final_deferred.resolve(quotient.value);
          });
        }
      });
    ...
    self.smod = function (o, l, op_id) {
      ...
      Promise.all(promise_bits).then(function (decomposed_bits) {
        ...
        if (remainder.ready) {
          final_deferred.resolve(remainder.value);
        } else {
          remainder.promise.then(function () {
            final_deferred.resolve(remainder.value);
          });
        }
      });
    ...
    self.bit_decomposition = function (op_id) {
      ...
      var bitsPromise = self.jiff.protocols.bits.rejection_sampling(0, self.Zp, self.threshold, self.holders, self.Zp, op_id + ':sampling');
      bitsPromise.then(function (bits) {
        var r = self.jiff.protocols.bits.bit_composition(bits);
        ...
        self.jiff.internal_open(r.isadd(self), self.holders, op_id + ':open').then(function (result) {
          ...
          for (var i = 0; i < bitLength; i++) {
            result[i] = didWrap.iif_else(withWrap[i], noWrap[i], op_id + ':if_else:' + i);
          }
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     */
    jiff.share_bits = function (secret, threshold, receivers_list, senders_list, Zp, share_id) {
      if (Zp == null) {
        Zp = jiff.Zp;
      }
      if (share_id == null) {
      ...
      var local_bits = [];
      if (secret != null) {
        var bitLength = jiff.helpers.ceil(jiff.helpers.bLog(Zp, 2));
        bitLength = parseInt(bitLength.toString(), 10);
        local_bits = jiff.helpers.number_to_bits(secret, bitLength);
      }

      var shared_bits = [];
      for (var i = 0; i < local_bits.length; i++) {
        shared_bits[i] = jiff.internal_share(local_bits[i], threshold, receivers_list, senders_list, Zp, share_id + ':' + i);
      }
      return shared_bits;
    };
     ...
     * @instance
     * @param {SecretShare[]} bits - an array of the secret shares of bits, starting from least to most significant bits.
     * @param {number[]} party_ids - parties to open (same as jiff.open)
     ...
     */
    jiff.protocols.bits.open_bits = function (bits, party_ids, op_id) {
      if (op_id == null) {
      ...
      for (var i = 0; i < bits.length; i++) {
        opened_bits[i] = jiff.open(bits[i], party_ids, op_id + ':' + i);
      }
     ...
     * @returns {promise} a (JQuery) promise to the open value of the secret.
     */
    jiff.protocols.bits.open_bits = function (count, parties, threshold, Zp, op_id) {
      if (op_id == null) {
      ...
      if (count == null) {
        ...
        count = jiff.helpers.ceil(jiff.helpers.bLog(Zp, 2));
    ...
    jiff.protocols.bits.cneq = function (bits, constant, op_id) {
      ...
      var notEqual = bits[0].icxor(constant_bits[0]);
      for (var i = 1; i < bits.length; i++) {
        var xor = bits[i].icxor_bit(constant_bits[i]);
        notEqual = notEqual.isor_bit(xor, op_id + ':sor_bit:' + (i - 1));
      }
    ...
    jiff.protocols.bits.cgt = function (bits, constant, op_id) {
      ...
      if (constant.toString().startsWith('-')) {
        return true;
      }
      ...
      for (i = c.length - 2; i >= 0; i--) {
        d[i] = d[i + 1].isor_bit(c[i], op_id + ':sor_bit:' + i);
      }
    ...
    jiff.protocols.bits.cgteq = function (bits, constant, op_id) {
      ...
      return jiff.protocols.bits.cgt(bits, constant - 1, op_id);
    };
    ...
    jiff.protocols.bits.rejection_sampling = function (lower_bound, upper_bound, threshold, compute_list, Zp, op_id) {
      ...
      (function resample(reject_count) {
        var bitLength = jiff.helpers.ceil(jiff.helpers.bLog(range));
        bitLength = parseInt(bitLength.toString(), 2);

        var bits = jiff.server_generate_and_share({bit: true, count: bitLength}, compute_list, threshold, Zp);
        var bits_add = lower_bound === 0 ? bits : jiff.protocols.bits.cadd(bits, lower_bound, op_id + ':bits.cadd:' + reject_count);

        if (jiff.helpers.bLog(range).toString().indexOf('.') === -1) {
          deferred.resolve(bits_add);
          return;
        }

        var cmp = jiff.protocols.bits.cgt(bits, range, op_id + ':bits.cgt').inot();
        var promise = jiff.internal_open(cmp, compute_list, op_id + ':open');
    ...
    jiff.protocols.bits.cadd = function (bits, constant, op_id) {
      ...
      for (var i = 1; i < bits.length; i++) {
        ...
        sum[i] = xor.isxor_bit(carry, op_id + ':sxor_bit:' + (i - 1));
        carry = and.isadd(xorAndCarry); // cheap or, xor and and cannot both be true!
      }

      // Remaining bits [if any]
      for (; i < constant_bits.length; i++) {
        // and is always zero, xor is equal to the PUBLIC bit value
        sum[i] = carry.icxor_bit(constant_bits[i]);
    ...
    jiff.protocols.bits.csubl = function (bits, constant, op_id) {
      ...
      for (var i = 1; i < bits.length; i++) {
        ...
        diff[i] = xor.isxor_bit(borrow, op_id + ':sxor_bit:' + (i - 1));
      ...
      for (; i < constant_bits.length; i++) {
        ...
        diff[i] = borrow.icxor_bit(constant_bits[i]);
    ...
    jiff.protocols.bits.csubr = function (constant, bits, op_id) {
      ...
      for (; i < constant_bits.length; i++) {
        ...
        diff[i] = borrow.icxor_bit(constant_bits[i]);
        borrow = diff[i].icmult(constant_bits[i]);
      }
    ...
    /**
     *  Sums an array of secret bit-arrays by divide-and-conquer recursion
     */
    jiff.protocols.bits.sadd_as_tree = function (addends, op_id, length = addends.length) {
      return (length == 1) ? addends[0]
      : (length == 2) ? jiff.protocols.bits.sadd(addends[0], addends[1], "sadd_as_tree:sadd:2:" + op_id)
      : jiff.protocols.bits.sadd(
        jiff.protocols.bits.sadd_as_tree(addends.splice(0, Math.floor(length/2)), "sadd_as_tree:left:" + op_id),
        jiff.protocols.bits.sadd_as_tree(addends, "sadd_as_tree:right:" + op_id), "sadd_as_tree:sadd:" + length + ":" + op_id
      );
    };

    /**
     *  Safe parallel bitwise addition through carry-save adding
     *  with a fallback to addition in sequence when Zp is too small
     *  TODO: FIX THIS becasue it currently isn't worth keeping
     */
    jiff.protocols.bits.sadd_parallel = function (addends, op_id) {
      // Check assumptions, and fallback if neccessary
      if (Math.min(jiff.party_count, addends.length) >= jiff.Zp) {
        addends[0][0].jiff.protocols.bits.sadd_sequence(addends, op_id);
      }

      var sum_bits = addends[0];
      var carry_bits = [];
      for (var i = 0; i < sum_bits.length; i++) {
        for (var j = 1; j < addends.length; j++) {
          /* if (s[i] == 2) {
             c[i] = s[i]-1;
             s[i] %= 2;
          } */
          sum_bits[i] = sum_bits[i].sadd(addends[j][i]).sadd(carry_bits[ (i == 0) ? 0 : i-1 ]);
          let sum_mod = sum_bits[i].cmod(2);
          carry_bits[i] = sum_mod.smult(sum_bits[i].csub(1));
          sum_bits[i] = sum_mod;
        }
      }

      // Postproccess back into binary format
      jiff.protocols.bits.sadd(sum_bits, carry_bits);

      return sum_bits;
    }

    /**
     *  Fallback parallel bitwise addition that sequentially sums an array of sectret bit-arrays
     */
    jiff.protocols.bits.sadd_sequence = function (addends, op_id) {
        var sum = addends[0];
        for (var i = 1; i < addends.length; i++) {
          sum = sum[0].jiff.protocols.bits.sadd(sum, addends[i], "bits.sadd_parallel:" + i +":" + op_id);
        }
        return sum;
    }
    ...
    jiff.protocols.bits.sadd = function (bits1, bits2, op_id) {
      ...
      for (var i = 1; i < bits2.length; i++) {
        ...
        sum[i] = xor.isxor_bit(carry, op_id + ':sxor_bit2:' + (i - 1));
        carry = and.isadd(xorAndCarry); // cheap or, xor and and cannot both be true!
      }
      ...
      for (; i < bits1.length; i++) {
        ...
        sum[i] = bits1[i].isxor_bit(carry, op_id + 'sxor_bit1:' + (i - 1));
    ...
    jiff.protocols.bits.ssub = function (bits1, bits2, op_id) {
      ...
      // loop over the remaining bits
      var n = Math.min(bits1.length, bits2.length);
      for (var i = 1; i < n; i++) {
        var xor = bits1[i].isxor_bit(bits2[i], op_id + ':sxor_bit1:' + (i - 1));
        var andNot = bits1[i].inot().ismult(bits2[i], op_id + ':smult1:' + (i - 1));

        // save and update borrow
        diff[i] = xor.isxor_bit(borrow, op_id + ':sxor_bit2:' + (i - 1));
      ...
      for (; i < bits1.length; i++) {
        ...
        diff[i] = bits1[i].isxor_bit(borrow, op_id + ':sxor_bit1:' + (i - 1));
        borrow = bits1[i].inot().ismult(borrow, op_id + ':smult1:' + (i - 1));
      }
      for (; i < bits2.length; i++) {
        // xor and andNot are equal to the value of bits2[i]
        diff[i] = bits2[i].isxor_bit(borrow, op_id + ':sxor_bit1:' + (i - 1));
     ...
     *                                              default value should suffice when the code of all parties executes all instructions
     *                                              in the same exact order, otherwise, a unique base name is needed here.
     * @returns {SecretShare[]} bitwise sharing of the result, the length of the result will be between 1 to bits.length + ceil(log2(constant)) inclusive,
     *                          depending on how big constant is.
     */
    jiff.protocols.bits.cmult = function (bits, constant, op_id) {
      ...
      var zero = jiff.secret_share(jiff, true, null, 0, bits[0].holders, 1, bits[0].Zp, op_id + ':zero_share');
      ...
      for (var i = 0; i < constant_bits.length; i++) {
        if (constant_bits[i].toString() === '1') {
          result = jiff.protocols.bits.sadd(result, bits, op_id + ':bits.sadd:' + i);
        }
    ...
    jiff.protocols.bits.smult = function (bits1, bits2, op_id) {
      ...
      // get useless share of zero (just for padding)
      var zero = jiff.secret_share(jiff, true, null, 0, bits1[0].holders, 1, bits1[0].Zp, op_id + ':zero_share');

      // This will be the result
      var result = null;

      // intermediate products
      var bits_intermediate = [];

      // Compute and set the ith intermediate product
      var intermediate_bit = function(i) {
        // Get current bit of 'seed' multiplicand
        var this_bit = bits2[i];

        // Initialize intermediate bits
        // bit_mult[] = bits2_i * bits1[]
        bits_interm = [];
        for (var j = 0; j < bits1.length; j++) {
          bits_interm[j] = this_bit.iif_else(bits1[j], zero);
        }

        // Right-shift bits1 to muliply by two
        for (var j = 0; j < i; j++) {
            bits_interm.unshift(zero);
        }

        bits_intermediate[i] = bits_interm;
      }

      // Compute intermediate products over *shortest* array
      for (var i = 0; i <= bits2.length/2 - 1; i++) {
        intermediate_bit(i);
        intermediate_bit(bits2.length-i-1);
      }
      if (bits2.length % 2 == 1) {
        intermediate_bit(Math.floor(bits2.length/2));
      }

      // Sum an array of secret bit-arrays by divide-and-conquer recursion
      result = (function sadd_as_tree(addends, op_id, length = addends.length) {
       return (length == 1) ? addends[0]
       : (length == 2) ? jiff.protocols.bits.sadd(addends[0], addends[1], 'sadd_as_tree:sadd:2:' + op_id)
       : jiff.protocols.bits.sadd(
         sadd_as_tree(addends.splice(0, Math.floor(length/2)), 'sadd_as_tree:left:' + op_id),
         sadd_as_tree(addends, 'sadd_as_tree:right:' + op_id), 'sadd_as_tree:sadd:' + length + ':' + op_id
       );
      })(bits_intermediate, 'bits.smult:' + op_id);
    ...
    jiff.protocols.bits.seq = function (bits1, bits2, op_id) {
      ...
      return jiff.protocols.bits.sneq(bits1, bits2, op_id);
    };
    ...
    jiff.protocols.bits.sneq = function (bits1, bits2, op_id) {
      ...
      // big or of bitwise xors
      var notEqual = bits1[0].isxor_bit(bits2[0], op_id + ':sxor_bit:initial');
      for (var i = 1; i < bits2.length; i++) {
      ...
      // remaining bits [if any]
      for (; i < bits1.length; i++) {
        // xor is bits1[i] since bits2[i] is zero
        notEqual = notEqual.isor_bit(bits2[i], op_id + ':sor_bit:' + (i - 1));
      }
     ...
     *                                                                and the remainder array has the same length as bits2.
    ...
    jiff.protocols.bits.sdiv = function (bits1, bits2, op_id) {
      ...
      for (var i = bits1.length - 1; i >= 0; i--) {
        ...
        remainder.unshift(bits1[i]);

        // Get the next bit of the quotient
        // and conditionally subtract b from the
        // intermediate remainder to continue
        var sub = jiff.protocols.bits.ssub(remainder, bits2, op_id + ':bits.ssub:' + iterationCounter);
        var noUnderflow = sub.pop().inot(); // get the overflow bit, sub is now the result of subtraction

        // Get next bit of quotient
        quotient.unshift(noUnderflow);

        // Update remainder
        for (var j = 0; j < remainder.length; j++) {
          // note, if noUnderflow, then |# bits in sub| <= |# bits in remainder|
          var ifElseCount = iterationCounter * (iterationCounter + 1) / 2;
          remainder[j] = noUnderflow.iif_else(sub[j], remainder[j], op_id + ':if_else:' + (ifElseCount + j));
        }
      }

      while (remainder.length > bits2.length) {
        remainder.pop();
      }
     ...
     *                                                                and the remainder array has the same length as ceil(log2(constant)).
    ...
    jiff.protocols.bits.cdivl = function (bits, constant, op_id) {
      ...
      for (var i = bits.length - 1; i >= 0; i--) {
        ...
        // add bit i to the head of remainder (least significant bit)
        remainder.unshift(bits[i]);

        // Get the next bit of the quotient
        // and conditionally subtract b from the
        // intermediate remainder to continue
        var sub = jiff.protocols.bits.csubl(remainder, constant, op_id + ':bits.csub:' + iterationCounter);
        var noUnderflow = sub.pop().inot(); // get the overflow bit, sub is now the result of subtraction
      ...
      while (remainder.length > constant.toString(2).length) {
        remainder.pop();
      }
     ...
     *                                              in the same exact order, otherwise, a unique base name is needed here.
     * @returns {{quotient: SecretShare[], remainder: SecretShare[]}} the quotient and remainder bits arrays, note that
     *                                                                the quotient array has the same length as ceil(log2(constant)),
     *                                                                and the remainder array has the same length as bits.
    ...
    jiff.protocols.bits.cdivr = function (constant, bits, op_id) {
      ...
      // get useless share of zero (just for bootstrapping)
      var zero = jiff.secret_share(jiff, true, null, 0, bits[0].holders, 1, bits[0].Zp, op_id + ':zero_share');
      ...
      // loop over each bit
      for (var i = constant_bits.length - 1; i >= 0; i--) {
        var iterationCounter = (constant_bits.length - i - 1);

        // add bit i to the head of remainder (least significant bit)
        remainder.unshift(zero.icadd(constant_bits[i]));

        // Get the next bit of the quotient
        // and conditionally subtract b from the
        // intermediate remainder to continue
        var sub = jiff.protocols.bits.ssub(remainder, bits, op_id + ':bits.ssub:' + iterationCounter);
        var noUnderflow = sub.pop().inot(); // get the overflow bit, sub is now the result of subtraction
      ...
      while (remainder.length > bits.length) {
        remainder.pop();
      }
