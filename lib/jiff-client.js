...
(function (exports, node) {
  ...
  function encrypt_and_sign(message, encryption_public_key, signing_private_key, operation_type) {
    ...
    if (operation_type === 'triplet' || operation_type === 'number') {
  ...
  function jiff_share(jiff, secret, threshold, receivers_list, senders_list, Zp, share_id) {
    ...
    if (senders_list.indexOf(jiff.id) > -1) {
      ...
      for (i = 0; i < receivers_list.length; i++) {
        ...
        msg = jiff.execute_array_hooks('beforeOperation', ['share', msg], 1);

        msg['share'] = jiff.hooks.encryptSign(msg['share'], jiff.keymap[msg['party_id']], jiff.secret_key, 'share');
  ...
  function receive_share(jiff, json_msg) {
    // Decrypt share
    json_msg['share'] = jiff.hooks.decryptSign(json_msg['share'], jiff.secret_key, jiff.keymap[json_msg['party_id']], 'share');

    json_msg = jiff.execute_array_hooks('afterOperation', [ 'share', json_msg ], 1);
  ...
  function jiff_broadcast(jiff, share, parties, op_ids) {
    for (var index = 0; index < parties.length; index++) {
      ...
      msg = jiff.execute_array_hooks('beforeOperation', ['open', msg], 1);

      msg['share'] = jiff.hooks.encryptSign(msg['share'], jiff.keymap[msg['party_id']], jiff.secret_key, 'open');
  ...
  function receive_open(jiff, json_msg) {
    ...
    if (json_msg['party_id'] !== jiff.id) {
      json_msg['share'] = jiff.hooks.decryptSign(json_msg['share'], jiff.secret_key, jiff.keymap[json_msg['party_id']], 'open');
      json_msg = jiff.execute_array_hooks('afterOperation', [ 'open', json_msg ], 1);
    }
  ...
  function jiff_triplet(jiff, receivers_list, threshold, Zp, triplet_id) {
    ...
    msg = jiff.execute_array_hooks('beforeOperation', ['triplet', msg], 1);
  ...
  function receive_triplet(jiff, json_msg) {
    json_msg = jiff.execute_array_hooks('afterOperation', [ 'triplet', json_msg ], 1);
  ...
  function jiff_server_share_number(jiff, options, receivers_list, threshold, Zp, number_id) {
    ...
    msg = jiff.execute_array_hooks('beforeOperation', ['number', msg], 1);
    msg = JSON.stringify(msg);
  ...
  function receive_server_share_number(jiff, json_msg) {
    json_msg = jiff.execute_array_hooks('afterOperation', [ 'number', json_msg ], 1);
  ...
  function receive_custom(jiff, json_msg) {
    if (json_msg['encrypted'] === true) {
      json_msg['message'] = jiff.hooks.decryptSign(json_msg['message'], jiff.secret_key, jiff.keymap[json_msg['party_id']], 'custom');
    }

    if (json_msg['party_id'] !== jiff.id) {
      json_msg = jiff.execute_array_hooks('afterOperation', ['custom', json_msg], 1);
    }
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    var guard_socket = function (socket) {
          ...
          socket.emit(label, msg, function (status) {
            if (status) {
              ...
              if (label === 'free' && socket === jiff.socket) {
                jiff.execute_array_hooks('afterOperation', ['free', msg], 1);
              }
            }
          });
        }
      ...
      socket.disconnect = function () {
        if (socket === jiff.socket) {
          jiff.execute_array_hooks('beforeOperation', ['disconnect', {}], -1);
        }
    ...
    if (jiff.hooks.generateKeyPair == null) {
      if (options.sodium !== false) {
        jiff.hooks.generateKeyPair = function () {
          // eslint-disable-next-line no-undef
          var key = sodium.crypto_box_keypair(); // this party's public and secret key
    ...
    jiff.connect = function () {
      var ready_connect = function () {
        ...
        msg = jiff.execute_array_hooks('beforeOperation', [ 'initialization', msg ], 1);
        msg = JSON.stringify(msg);
      ...
      if (options.sodium === false) {
        ...
        // eslint-disable-next-line no-undef
        sodium.ready.then(ready_connect);
      }
    };
    ...
    jiff.emit = function (tag, receivers, message, encrypt) {
      ...
      for (var p = 0; p < receivers.length; p++) {
        ...
        message_to_send = jiff.execute_array_hooks('beforeOperation', ['custom', message_to_send], 1);

        if (message_to_send['encrypted'] !== false) {
          message_to_send['message'] = jiff.hooks.encryptSign(message_to_send['message'], jiff.keymap[message_to_send['party_id']], jiff.secret_key, 'custom');
    ...
    jiff.disconnect = function (safe, free, callback) {
      if (safe) {
        ...
        if (free) {
          jiff.free(jiff.socket);
        }
        jiff.socket.disconnect();
        if (jiff.triplets_socket !== jiff.socket) {
          if (free) {
            jiff.triplets_socket.emit('free', JSON.stringify({}));
          }
          jiff.triplets_socket.disconnect();
        }
        if (jiff.numbers_socket !== jiff.socket) {
          if (free) {
            jiff.numbers_socket.emit('free', JSON.stringify({}));
          }
    ...
    jiff.free = function (socket) {
      ...
      var msg = {};
      if (socket === jiff.socket) {
        msg = jiff.execute_array_hooks('beforeOperation', ['free', msg], 1);
      }
    ...
    jiff.socket.on('init', function (msg) {
      msg = JSON.parse(msg);
      msg = jiff.execute_array_hooks('afterOperation', [ 'initialization', msg ], 1);
      ...
      if (jiff.secret_key == null || jiff.public_key == null) {
        var key = jiff.hooks.generateKeyPair();
        jiff.secret_key = key.secret_key;
        jiff.public_key = key.public_key;
      }

      msg = { public_key: jiff.hooks.dumpKey(jiff.public_key) };
      msg = jiff.execute_array_hooks('beforeOperation', [ 'public_key', msg ], 1);

      jiff.socket.emit('public_key', JSON.stringify(msg));
    ...
    jiff.socket.on('public_key', function (msg) {
      msg = JSON.parse(msg);
      msg = jiff.execute_array_hooks('afterOperation', [ 'public_key', msg ], 1);

      msg = msg.public_keys;

      var i;
      for (i in msg) {
        if (msg.hasOwnProperty(i) && jiff.keymap[i] == null) {
          jiff.keymap[i] = jiff.hooks.parseKey(msg[i]);
        }
      }
    ...
    jiff.socket.on('disconnect', function (reason) {
      ...
      jiff.execute_array_hooks('afterOperation', [ 'disconnect', reason ], -1);
    });
