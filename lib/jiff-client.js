...
(function(exports, node) {
  ...
  function jiff_open(jiff, share, parties, op_id) {
    ...
    if(parties.indexOf(jiff.id) > -1) {
      ...
      // remove random holders - threshold elements      
      while(promises.length > share.threshold)
        promises.splice(Math.floor(Math.random() *promises.length), 1);
      ...
      return Promise.all(promises).then(function(shares) {
        ...
        return jiff_lagrange(shares, jiff.party_count, jiff);
      });
    }
