...
(function(exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.smult_bgw = function(jiff, x, y, op_id) {
      if (!(x.jiff === self.jiff || !(x.jiff === self.jiff))) throw "shares do not belong to the same instance (*)";
      if (!self.jiff.helpers.Zp_equals(self, x)) throw "shares must belong to the same field (*)";
      if (!self.jiff.helpers.array_equals(self.holders, x.holders)) throw "shares must be held by the same parties (*)";
    ...
    self.sgteq = function(o, l, op_id) {
      ...
      function compare_online(preprocess) {
        ...
        for(var i = 0; i < self.holders.length; i++)
          ...
          else nested_op_ids[self.holders[i]] = op_id + ":" + self.holders[i] + ":" + holders_label;
