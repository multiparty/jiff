...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.sdiv = function (o, l, op_id) {
      ...
      var final_deferred = new Deferred();
      var final_promise = final_deferred.promise;
      var result = self.jiff.secret_share(self.jiff, false, final_promise, undefined, self.holders, max(self.threshold, o.threshold), self.Zp, 'share:quotient:' + op_id);
      var promise_bits = [self.bit_decomposition(op_id + ":sdiv:dividend"), o.bit_decomposition(op_id + ":sdiv:divisor")];
    ...
    self.smod = function (o, l, op_id) {
      ...
      var final_deferred = new Deferred();
      var final_promise = final_deferred.promise;
      var result = self.jiff.secret_share(self.jiff, false, final_promise, undefined, self.holders, max(self.threshold, o.threshold), self.Zp, 'share:remainder:' + op_id);
      var promise_bits = [self.bit_decomposition(op_id + ":sdiv:mod:dividend"), o.bit_decomposition(op_id + ":sdiv:mod:modulus")];
      Promise.all(promise_bits).then(function (decomposed_bits) {
        var dividend_bits = decomposed_bits[0].splice(-l);
        var divisor_bits = decomposed_bits[1].splice(-l);

        var remainder_bits = self.jiff.protocols.bits.sdiv(dividend_bits, divisor_bits, op_id + ":sdiv:mod").remainder;
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.bits.open_bits = function (bits, party_ids, op_id) {
        ...
        var deferred_number = new Deferred();
        Promise.all(opened_bits).then(function (bits) {
            deferred_number.resolve(jiff.helpers.bits_to_number(bits, bits.length));
        });
    ...
    jiff.protocols.bits.csubl = function (bits, constant, op_id) {
      ...
      // initialize results
      var diff = [ bits[0].icxor_bit(constant[0]) ];
      var borrow = bits[0].inot().icmult(constant[0]);

      // loop over the remaining bits
      for (var i = 1; i < bits.length; i++) {
        var xor = bits[i].icxor_bit(constant[i]);
        var andNot = bits[i].inot().icmult(constant[i]);
      ...
      for (; i < constant_bits.length; i++) {
        ...
        diff[i] = borrow.isxor_bit(constant_bits[i], op_id + ':sxor_bit:' + (i - 1));
    ...
    jiff.protocols.bits.smult = function (bits1, bits2, op_id) {
      ...
      for (var i = 0; i < bits2.length; i++) {
        ...
        for (var j = 0; j < bits1.length; j++) {
          bit_mult[j] = this_bit.iif_else(bits1[j], zero);
        }

        if (result == null) {
          result = bit_mult;
        } else {
          result = jiff.protocols.bits.sadd(result, bit_mult);
        }
