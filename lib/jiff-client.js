...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.sdiv = function (o, l, op_id) {
      ...
      // Convert to bits
      var dividend_bits = self.bit_decomposition(op_id + ':decomposition1').splice(-l);
      var divisor_bits = o.bit_decomposition(op_id + ':decomposition2').splice(-l);
      ...
      // Set the new threshold for the resulting share
      quotient.threshold = max(self.threshold, o.threshold);
    ...
    self.bit_decomposition = function (op_id) {
      ...
      for (var i = 0; i < bitLength; i++) {
        deferred[i] = new Deferred();
        result[i] = self.jiff.secret_share(self.jiff, false, deferred[i].promise, undefined, self.holders, self.threshold, self.Zp, 'share:bit' + i + ':' + op_id)
      }
      ...
      bitsPromise.then(function (bits) {
        ...
        self.jiff.internal_open(r.isadd(self), self.holders, op_id + ':open').then(function (result) {
          ...
          for (var i = 0; i < bitLength; i++) {
            let bit_i = didWrap.iif_else(withWrap[i], noWrap[i], op_id + ':if_else:' + i);

            if (bit_i.ready) {
              deferred[i].resolve(bit_i.value);
            } else {
              bit_i.promise.then(function (index, share_value) {
                deferred[index].resolve(share_value);
            }.bind(null, i));
            }
          }
        });
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.bits.share_bits = function (secret, bit_length, threshold, receivers_list, senders_list, Zp, share_id) {
      ...
      if (bit_length == null) {
        bit_length = jiff.helpers.ceil(jiff.helpers.bLog(Zp, 2));
        bit_length = parseInt(bit_length.toString(), 10);
      }
    ...
    jiff.protocols.bits.open_bits = function (bits, parties, op_id) {
      ...
      for (var i = 0; i < bits.length; i++) {
        opened_bits[i] = jiff.open(bits[i], parties, op_id + ':' + i);
      }
    ...
    jiff.protocols.bits.cneq = function (bits, constant, op_id) {
      ...
      var notEqual = bits[0].icxor_bit(constant_bits[0]);
      for (var i = 1; i < bits.length; i++) {
