...
(function(exports, node) {
   ...
   * @param {string/number/object} - the operation id (or a map from receiving party to operation id) to be used to tag outgoing messages [optional].
  ...
  function jiff_open(jiff, share, parties, op_ids) {
    ...
    if(share.holders.indexOf(jiff.id) > -1) {
      ...
      share = share.refresh(op_ids[share.holders[0]]+":refresh");
  ...
  function jiff_triplet(jiff, receivers_list, threshold, Zp, triplet_id) {
    ...
    if(triplet_id == null) {
      ...
      triplet_id = "triplet:" + label + ":" + (jiff.triplet_op_count[label]++);
    }
  ...
  function jiff_server_share_number(jiff, options, receivers_list, threshold, Zp, number_id) {
    ...
    if(number_id == null) {
      ...
      number_id = "number:" + label + ":" + (jiff.number_op_count[label]++);
    }
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp) {
     ...
     * @memberof jiff.secret-share
     */
    self.refresh = function(op_id) { // TODO add jsdoc here and in jiff.server_generate_and_share etc..
     ...
     * @memberof jiff.secret-share
     */
    self.smult = function(o, triplet_id) { // TODO add jsdoc for triplet_id in here, jiff.triplet, and jiff_triplet
      if (!(o.jiff === self.jiff)) throw "shares do not belong to the same instance (*)";
      if (!(o.Zp === self.Zp)) throw "shares must belong to the same field (*)";
      if (!self.jiff.helpers.array_equals(self.holders, o.holders)) throw "shares must be held by the same parties (*)";
      ...
      var e_promise = self.jiff.open(e, e.holders, triplet_id == null ? null : "open_nested:"+triplet_id+":1");
      var d_promise = self.jiff.open(d, d.holders, triplet_id == null ? null : "open_nested:"+triplet_id+":2");
    ...
    self.sgteq = function(o, l) {
      ...
      function compare_online(preprocess) {
        ...
        for(var i = 0; i < self.holders.length; i++) {
          var label = self.holders[i] + ":" + holders_label;
          if(self.jiff.open_op_count[label] == null) self.jiff.open_op_count[label] = 0;
          nested_op_ids[self.holders[i]] = "open:" + label + ":" + (self.jiff.open_op_count[label]) + ":nested1";
        }
        
        // Unique and in order single triplet id for the nest secret multiplication that is executed inside finish_compare.
        var label = self.holders.join(",");
        if(self.jiff.triplet_op_count[label] == null) self.jiff.triplet_op_count[label] = 0;
        var nested_triplet_id = "triplet:" + label + ":" + (self.jiff.triplet_op_count[label]++);
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     */
    jiff.server_generate_and_share = function(options, receivers_list, threshold, Zp, op_id) { return jiff_server_share_number(jiff, options, receivers_list, threshold, Zp, op_id) };
