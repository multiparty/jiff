...
(function(exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.helpers.random = function(max) {
      ...
      // == 2^53 - 1
      if(max > 9007199254740991) throw new RangeError('Max value should be smaller than 2^53.');
      ...
      var bytesNeeded = Math.ceil((bitsNeeded + 1) / 8);
      var randomBytes = new Uint8Array(bytesNeeded);
      var maxBytes = Math.pow(256, bytesNeeded);
      ...
      while (true) {
        ...
        if (randomValue < maxBytes - maxBytes % max) {
