...
(function (exports, node) {
  var crypto_, io_, $_, sodium_;
  if (node) {
    io_ = require('socket.io-client');
    $_ = require('jquery-deferred');
   ...
   * @param {object} dependencies - contains any of these three attributes: "io", "jquery-deferred", "sodium"
  ...
  exports.dependencies = function (dependencies) {
    ...
    $_ = dependencies['jquery-deferred'] != null ? dependencies['jquery-deferred'] : $_;
   ...
   * @returns {string} the signed cipher, a JSON string that includes two properties: 'cipher' and 'nonce'.
   */
   ...
   * @memberof jiff.utils
   * @param {string} cipher_text - the cipher text to decrypt, a JSON string that includes two properties: 'cipher' and 'nonce'.
  ...
  function decrypt_and_sign(jiff, cipher_text, decryption_secret_key, signing_public_key) {
    cipher_text = JSON.parse(cipher_text);
    ...
    if (receivers_list.indexOf(jiff.id) > -1) {
      ...
      for (i = 0; i < senders_list.length; i++) {
        ...
        if (jiff.deferreds[share_id][p_id] == null) {
          ...
          jiff.deferreds[share_id][p_id] = $_.Deferred();
        }
  ...
  function receive_share(jiff, json_msg) {
    ...
    // Execute when share is decrypted and ready
    var ready = function (json_msg) {
      ...
      if (jiff.deferreds[op_id][sender_id] == null) {
        ...
        jiff.deferreds[op_id][sender_id] = $_.Deferred();
      }
    ...
    if (json_msg['share'].then) {
      json_msg['share'].then(function (msg) {
        json_msg['share'] = msg;
        ready(json_msg);
      });
    } else {
  ...
  function jiff_open(jiff, share, parties, op_ids) {
    ...
    if (parties.indexOf(jiff.id) > -1) {
      ...
      var final_deferred = $_.Deferred(); // will be resolved when the final value is reconstructed
      ...
      for (i = 0; i < share.holders.length; i++) {
        ...
        if (jiff.deferreds[op_ids[jiff.id]][p_id] == null) {
          jiff.deferreds[op_ids[jiff.id]][p_id] = $_.Deferred();
        }
  ...
  function receive_open(jiff, json_msg) {
    ...
    // Execute when message is decrypted and ready
    var ready = function (json_msg) {
      if (json_msg['party_id'] !== jiff.id) {
  ...
  function jiff_triplet(jiff, receivers_list, threshold, Zp, triplet_id) {
    ...
    var a_deferred = $_.Deferred();
    var b_deferred = $_.Deferred();
    var c_deferred = $_.Deferred();
    ...
    if (jiff.id === 's1') {
      ...
      jiff.socket.safe_emit('triplet', cipher);
    }
  ...
  function jiff_server_share_number(jiff, options, receivers_list, threshold, Zp, number_id) {
    ...
    if (jiff.id === 's1') {
      ...
      var cipher = jiff.hooks.encryptSign(jiff, msg, jiff.keymap['s1'], jiff.secret_key);
      jiff.socket.safe_emit('number', cipher);
    }
  ...
  function jiff_share_array(jiff, array, lengths, threshold, receivers_list, senders_list, Zp, share_id) {
    ...
    var share_array_deferred = $_.Deferred();
  ...
  function jiff_share_2D_array(jiff, array, lengths, threshold, receivers_list, senders_list, Zp, share_id) {
    ...
    // wrap around result of share_array
    var lengths_deferred = $_.Deferred();
  ...
  function receive_custom(jiff, json_msg) {
    ...
    if (json_msg['message'].then) {
      json_msg['message'].then(function (msg) {
  ...
  function make_jiff(hostname, computation_id, options) {
      ...
      socket.safe_disconnect = function (free, callback) {
        (function ready() {
          if (socket.mailbox.head == null && jiff.counters.pending_opens === 0) {
            ...
            socket.empty_deferred = $_.Deferred();
    ...
    if (options.__internal_socket == null) {
      jiff.socket = io_(hostname, { autoConnect: false });
      guard_socket(jiff.socket);
    } else {
      jiff.socket = options.__internal_socket;
      // similar to guard, but for internal_socket
      jiff.socket.safe_emit = function (label, msg) {
        jiff.socket.emit(label, msg);
        if (label === 'free') {
          jiff.execute_array_hooks('afterOperation', [jiff, 'free', msg], 2);
        }
      };
      ...
      // internal socket delivers messages immediately
      jiff.socket.empty_deferred = $_.Deferred();
    ...
    jiff.socket.on('initialization', function (msg) {
      ...
      jiff.computation_id = msg.computation_id;
...
}((typeof exports === 'undefined' ? window.jiff = {} : exports), typeof exports !== 'undefined'));
