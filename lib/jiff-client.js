...
(function (exports, node) {
  ...
  function bit_combinator(deferred, start, length, initial, func, promisify, valufy) {
    ...
    var __bit_combinator = function (start, val) {
      ...
      promisify(val).then(__bit_combinator.bind(null, start + next, val));
    };
    ...
    if (initial == null) {
      ...
      promisify(initial).then(__bit_combinator.bind(null, start, initial));
    }
  }
    ...
    if (parties.indexOf(jiff.id) > -1) {
      ...
      for (i = 0; i < share.holders.length; i++) {
        ...
        if (jiff.deferreds[op_id][p_id] == null) {
          jiff.deferreds[op_id][p_id] = new Deferred();
        }
        ...
        (function (promise, p_id) { // p_id is modified in a for loop, must do this to avoid scoping issues.
          promise.then(function (received_share) {
            jiff.deferreds[op_id][p_id] = null;
  ...
  function jiff_triplet(jiff, receivers_list, threshold, Zp, triplet_id) {
    ...
    var msg = { triplet_id: triplet_id, receivers: receivers_list, threshold: threshold, Zp: Zp };
    ...
    var a_share = jiff.secret_share(jiff, false, a_deferred.promise, undefined, receivers_list, threshold, Zp);
    var b_share = jiff.secret_share(jiff, false, b_deferred.promise, undefined, receivers_list, threshold, Zp);
    var c_share = jiff.secret_share(jiff, false, c_deferred.promise, undefined, receivers_list, threshold, Zp);
   ...
   * @param {jiff-instance} jiff - the jiff instance.
   * @param {object} json_msg - the parsed json message as received by the triplet event.
   *
   */
  function receive_triplet(jiff, json_msg) {
    json_msg = jiff.execute_array_hooks('afterOperation', [jiff, 'triplet', json_msg], 2);
  ...
  function jiff_share_all_number(jiff, n, threshold, receivers_list, senders_list, Zp, share_id) {
    ...
    if (isReceiver) {
      ...
      for (i = 1; i < senders_list.length; i++) {
        share = share.isadd(shares[senders_list[i]]);
      }
  ...
  function jiff_server_share_number(jiff, options, receivers_list, threshold, Zp, number_id) {
    ...
    for (i = 0; i < options.count; i++) {
      ...
      shares[i] = jiff.secret_share(jiff, false, deferred.promise, undefined, receivers_list, threshold, Zp);
    }
   ...
   * @param {jiff-instance} jiff - the jiff instance.
   * @param {object} json_msg - the parsed json message as received by the number event.
   */
  function receive_server_share_number(jiff, json_msg) {
    json_msg = jiff.execute_array_hooks('afterOperation', [jiff, 'number', json_msg], 2);
    var numbers = json_msg['numbers'];
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp) {
    ...
    self.smult = function (o, op_id) {
      ...
      if (triplet == null) {
        triplet = jiff.triplet(self.holders, max(self.threshold, o.threshold), self.Zp, op_id + ':triplet');
      }
      ...
      // if object was provided but no triplet exists for the op_id
      var a = triplet[0];
      var b = triplet[1];
      var c = triplet[2];
    ...
    self.cdiv = function (cst, op_id) {
      ...
      var nOVERc = self.jiff.server_generate_and_share({count: 1, max: ZpOVERc}, self.holders, self.threshold, self.Zp, op_id + ':nOVERc')[0];
      var nMODc = self.jiff.server_generate_and_share({count: 1, max: cst}, self.holders, self.threshold, self.Zp, op_id + ':nMODc')[0];
    ...
    self.lt_halfprime = function (op_id) {
      ...
      var bitLength = share_helpers['floor'](self.jiff.helpers.bLog(share.Zp, 2)); // TODO: this leaks one bit, fix it for mod 2^n (ceil)

      // Retrieve pre-processed bits
      var bits = jiff.get_preprocessing(op_id + ':bitsequence');
      if (bits == null) {
        bits = self.jiff.server_generate_and_share({bit: true, count: bitLength}, share.holders, share.threshold, share.Zp, op_id + ':number:bits');
      }

      if (bits.length !== bitLength) {
        throw new Error('Preprocessed bit sequence has incorrect length, expected: ' + bitLength + ' actual: ' + bits.length);
      }

      // remove this line when fixing TODO
      bits[bitLength] = self.jiff.server_generate_and_share({number: 0}, share.holders, share.threshold, share.Zp, op_id + ':number:' + bitLength)[0];
    ...
    self.bit_decomposition = function (op_id) {
      ...
      var bitLength = share_helpers['ceil'](self.jiff.helpers.bLog(self.Zp, 2));
      ...
      var bits = self.jiff.protocols.bits.rejection_sampling(0, self.Zp, self.threshold, self.holders, self.Zp, op_id + ':sampling');
      var r = self.jiff.protocols.bits.bit_composition(bits);
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    // setup main socket
    jiff.socket = (options.__internal_socket == null ? io_(hostname, { autoConnect: false }) : options.__internal_socket);
    ...
    jiff.connect = function () {
      var ready_connect = function () {
        ...
        var msg = {
          ...
          public_key: jiff.hooks.dumpKey(jiff, jiff.public_key)
        };
     ...
     */
    jiff.helpers.Deferred = Deferred;
     ...
     */
    jiff.receive_open = function (parties, threshold, Zp, op_id) {
      if (Zp == null) {
        Zp = jiff.Zp;
      }
      return jiff_open(jiff, jiff.secret_share(jiff, true, null, null, parties, (threshold == null ? parties.length : threshold), Zp), [jiff.id], op_id);
    };
    ...
    jiff.protocols.reshare = function (share, threshold, receivers_list, senders_list, Zp, op_id) {
      ...
      } else { // either a receiver of share is ready
        ready_share();
      }
     ...
     */
    jiff.protocols.generate_and_share_random = function (threshold, receivers_list, senders_list, Zp, share_id, params) {
      if (Zp == null) {
        Zp = jiff.Zp;
      }
     ...
     */
    jiff.protocols.generate_and_share_zero = function (threshold, receivers_list, senders_list, Zp, share_id, params) {
     ...
     * @memberof jiff-instance.protocols
     * @instance
     * @param {number} [threshold=receivers_list.length] - the threshold of the triplets when stored by receivers after generation.
     * @param {Array} [receivers_list=all_parties] - array of party ids that want to receive the triplet shares, by default, this includes all parties.
    ...
    jiff.protocols.generate_beaver_bgw = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      ...
      if (compute_threshold == null) {
        compute_threshold = Math.floor((compute_list.length + 1) / 2); // honest majority BGW
      }

      if (op_id == null) {
        op_id = jiff.counters.gen_op_id2('generate_beaver_bgw', receivers_list, compute_list);
      }

      var a, b, c;
      if (compute_list.indexOf(jiff.id) > -1) {
        a = jiff.protocols.generate_and_share_random(compute_threshold, compute_list, compute_list, Zp, op_id + ':share_a', null);
        b = jiff.protocols.generate_and_share_random(compute_threshold, compute_list, compute_list, Zp, op_id + ':share_b', null);
      ...
      return { op_id: op_id + ':triplet', share: [a, b, c], promise: Promise.all([a.promise, b.promise, c.promise]) };
    };
     ...
     * @param {Array} [receivers_list=all_parties] - array of party ids that want to receive the bit shares, by default, this includes all parties.
     * @param {Array} [compute_list=all_parties] - array of party ids that will perform this protocol, by default, this includes all parties.
     * @param {number} [Zp=jiff-instance.Zp] - the mod (if null then the default Zp for the instance is used).
     * @param {string} [op_id=auto_gen()] - the operation id which is used to identify this operation.
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
     ...
     *                               Expects:
     *                               - an optional number compute_threshold parameter, which specifies threshold used
     *                               during the protocol execution. By default, this is the length of the compute_list.
     * @return {SecretShare} this party's share of the generated bit
     */
    jiff.protocols.bits.generate_random_bit  = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      if (params == null) {
        params = {};
      }
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id2('generate_random_bit', receivers_list, compute_list);
      }
      if (Zp == null) {
        Zp = jiff.Zp;
      }
      ...
      // Figure out compute threshold and validate it
      var compute_threshold = params['compute_threshold'];
      if (compute_threshold == null) {
        compute_threshold = Math.floor((compute_list.length + 1) / 2); // honest majority BGW
      }
      ...
      return jiff.protocols.reshare(random_bit, threshold, receivers_list, compute_list, Zp, op_id+':reshare');
    };

    /**
     * generates a sequence of random bits under MPC, relies on jiff.protocols.generate_random_bit
     * @memberof jiff-instance.protocols
     * @instance
     * @param {number} [threshold=receivers_list.length] - the threshold of the bit when stored by receivers after generation.
     * @param {Array} [receivers_list=all_parties] - array of party ids that want to receive the bit shares, by default, this includes all parties.
     ...
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
     *                         ensure that corresponding instructions across different parties are matched correctly.
     * @param {object} [params={bit_length}] - an object containing extra parameters passed by the user.
     *                               Expects:
     *                               - count: how many random bits should be computed
     *                               - an optional number compute_threshold parameter, which specifies threshold used
     *                               during the protocol execution. By default, this is the length of the compute_list.
     * @return {Object} contains 'op_id', 'share' (array of secret shares bits) and 'promise'.
     */
    jiff.protocols.generate_bit_sequence = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      if (params == null) {
        params = {};
      }
      ...
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id2('generate_bit_sequence', receivers_list, compute_list);
      }
      var promises = [];
      var bits = [];
      for (var i = 0; i < params.count; i++) {
        bits.push(jiff.protocols.bits.generate_random_bit(threshold, receivers_list, compute_list, Zp, op_id + i));
        promises.push(bits[i].promise);
      }

      return {op_id: op_id, share: bits, promise: Promise.all(promises)};
    };
    /**
     ...
     */
    jiff.protocols.bits.open_bits = function (bits, parties, op_id) {
      // Default values
      if (parties == null || parties === []) {
        parties = [];
        for (var p = 1; p <= jiff.party_count; p++) {
      ...
      // Compute operation ids (one for each party that will receive a result
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id2('open_bits', parties, bits[0].holders);
      }
     ...
     * @returns {promise} a (JQuery) promise to the open value of the secret.
     */
    jiff.protocols.bits.receive_open = function (count, parties, threshold, Zp, op_id) {
      // Default values
      if (parties == null || parties === []) {
        parties = [];
        for (i = 1; i <= jiff.party_count; i++) {
      ...
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('bits.open', parties);
      }
      ...
      var opened_bits = [];
      for (var i = 0; i < count; i++) {
        opened_bits[i] = jiff.receive_open(parties, threshold, Zp, op_id + ':' + i);
      }
     ...
     * @method rejection_sampling_preprocessing
     * @memberof jiff-instance.protocols.bits.rejection_sampling_preprocessing
     * @instance
     ...
     * @returns {Object} an object containing keys: 'op_id', 'share', and 'promise'. The promise is resolved when the rejection sampling is completed.
     *                   The object is consumed by <jiff_instance>.preprocessing:
     *                        - The op_id is equal to the passed op_id (if not null) and is used to save the result.
     ...
     *                   If this is pre-processed as part of some builtin primitive, no further action by the user is neeeded.
     *                   If this is executed because the user wants to produce some sampled value in the preprocessing stage, then the result can be obtained by calling <jiff_instance>.get_preprocessing(op_id) after
     *                   the promise is resolved.
     */
    jiff.protocols.bits.rejection_sampling_preprocessing = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      // Defaults
      if (compute_list == null) {
        compute_list = [];
        for (var p = 1; p <= jiff.party_count; p++) {
      ...
      threshold = threshold != null ? threshold : compute_list.length;
      Zp = Zp != null ? Zp : jiff.Zp;

      // If not a compute nor receiver party, return null
      if (compute_list.indexOf(jiff.id) === -1 && receivers_list.indexOf(jiff.id) === -1) {
        return null;
      }

      // op_id must be unique to both compute and receivers
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id2('rejection_sampling', receivers_list, compute_list);
      }

      // bounds
      var lower_bound = params.lower_bound != null ? params.lower_bound : 0;
      var upper_bound = params.upper_bound != null ? params.upper_bound : jiff.Zp;
      var bitLength = jiff.helpers.ceil(jiff.helpers.bLog(upper_bound, 2));
      bitLength = parseInt(bitLength.toString(), 10); // for bignumber

      // execute protocol
      var result = [];
      var promises = [];
      if (compute_list.indexOf(jiff.id) > -1) {
        result = jiff.protocols.rejection_sampling(lower_bound, upper_bound, params.compute_threshold, compute_list, Zp, op_id);
        promises.push(result.promise);
      }

      // fix threshold
      for (var i = 0; i < bitLength; i++) {
        result[i] = jiff.protocols.reshare(result[i], threshold, receivers_list, compute_list, Zp, op_id + ':reshare');
        if (result[i] != null) {
      ...
      // return output
      return { op_id: op_id, share: result, promise: Promise.all(promises) };
    };

    /**
     * Samples a random secret shared number between lower and upper bound under MPC using rejection sampling.
     * No party knows what the number is.
     * @method rejection_sampling
     * @memberof jiff-instance.protocols.bits.rejection_sampling
     * @instance
     * @param {number|BigNumber} [lower_bound=0] - the lower bound.
     * @param {number|BigNumber} [upper_bound=Zp] - the upper bound.
     * @param {number} [threshold=floor((compute_list.length+1)/2)] - the threshold of the resulting shares after sampling.
     * @param {Array} [compute_list=all_parties] - array of party ids that will perform this protocol, by default, this includes all parties.
     * @param {number} [Zp=jiff-instance.Zp] - the mod (if null then the default Zp for the instance is used).
     * @param {string} [op_id=auto_gen()] - the operation id which is used to identify this operation.
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
     *                         ensure that corresponding instructions across different parties are matched correctly.
     * @returns {SecretShare[]} bitwise sharing of the number: an array of secret shared bits, with index 0 corresponding to the
     *                          least significant bit. You can use jiff_instance.protocols.bit_composition to transform the array into a
     *                          single numerical secret share.
     */
    jiff.protocols.rejection_sampling = function (lower_bound, upper_bound, threshold, compute_list, Zp, op_id) {
      lower_bound = lower_bound != null ? lower_bound : 0;
      upper_bound = upper_bound != null ? upper_bound : jiff.Zp;
      if (compute_list == null) {
        compute_list = [];
        for (var p = 1; p <= jiff.party_count; p++) {
          compute_list.push(p);
      ...
      threshold = threshold != null ? threshold : Math.floor((compute_list.length + 1) / 2); // honest majority BGW
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('rejection_sampling', compute_list);
      }
      ...
      var many_shares = many_secret_shares(jiff, finalLength, compute_list, threshold, Zp);
      var deferreds = many_shares.deferreds;
      var result = many_shares.shares;

      // Sample and resample output
      (function resample(reject_count) {
        var bits = jiff.server_generate_and_share({bit: true, count: bitLength}, compute_list, threshold, Zp);
        var bits_add = bits;
        if (lower_bound.toString() !== '0') {
          bits_add = jiff.protocols.bits.cadd(bits, lower_bound, op_id + ':bits.cadd:' + reject_count);
        }

        var cmp = jiff.protocols.bits.clt(bits, range, op_id + ':bits.clt');
        ...
        var promise = jiff.internal_open(cmp, compute_list, op_id + ':open:' + reject_count);
    ...
    jiff.protocols.bits.cmult = function (bits, constant, op_id) {
      ...
      var zero = jiff.secret_share(jiff, true, null, 0, bits[0].holders, 1, bits[0].Zp);
      var initial = [ zero ];
    ...
    jiff.protocols.bits.sgt = function (bits1, bits2, op_id) {
      ...
      var gteq = jiff.protocols.bits.sgteq(bits1, bits2, op_id + ':bits.sgt');
    ...
    jiff.protocols.bits.cdivl = function (bits, constant, op_id) {
      ...
      if (constant.toString() === '1') {
        return {
          quotient: bits,
          remainder: [jiff.secret_share(jiff, true, null, 0, bits[0].holders, 1, bits[0].Zp)]
        }
      }
      ...
      bit_combinator(final_deferred, bits.length-1, -1, initial, function (i, _remainder) {
        ...
        // and conditionally subtract b from the
        // intermediate remainder to continue
        var sub = jiff.protocols.bits.csubl(_remainder, constant, op_id + ':bits.csub:' + iterationCounter);
        var noUnderflow = sub.pop().inot(); // get the overflow bit, sub is now the result of subtraction
    ...
    jiff.protocols.bits.cdivr = function (constant, bits, op_id) {
      ...
      bit_combinator(final_deferred, constant_bits.length-1, -1, initial, function (i, _remainder) {
        ...
        var cbit_share = jiff.secret_share(jiff, true, null, constant_bits[i], bits[0].holders, 1, bits[0].Zp);
     ...
     * @instance
     */
    jiff.crypto_provider = false;
    if (options.crypto_provider === true) {
      jiff.crypto_provider = options.crypto_provider;
    }
    ...
    var sdiv_count = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      var bitLength = jiff.helpers.bLog(Zp, 2).toString(10);
      bitLength = Math.ceil(Number(bitLength));
      if (params.l != null) {
        bitLength = bitLength < params.l ? bitLength : params.l;
      }
      return bitLength;
    };
    var lt_halfprime_handler = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      var bitLength = jiff.helpers.bLog(Zp, 2).toString(10);
      bitLength = Math.floor(Number(bitLength));
      params.count = bitLength;
      params.bit = true;
      return params;
    };
    var clt_bits_count = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      var bitLength = jiff.helpers.bLog(Zp, 2).toString(10);
      bitLength = Math.floor(Number(bitLength));
      return bitLength;
    };
    ...
    jiff.preprocessing_function_map = {
      base: {
        ...
        lt_halfprime : [
          { op: 'generate_bit_sequence',  op_id: ':bitsequence', handler: lt_halfprime_handler },
          { op: 'smult', op_id: ':smult1' },
          { op: 'clt_bits', op_id: ':clt_bits' },
          { op: 'sxor_bit', op_id: ':sxor_bit' }
        ],
        clt_bits: [
          { op: 'sor_bit', op_id: ':sor_bit:', count: clt_bits_count }
        ],
        cdiv: [
        ...
        sdiv: [
          { op: 'clteq', op_id: ':clteq', count: sdiv_count },
          { op: 'slteq', op_id: ':slteq', count: sdiv_count },
          { op: 'smult', op_id: ':smult1:', count: sdiv_count },
          { op: 'smult', op_id: ':smult2:', count: sdiv_count }
        ],
        smod: [
          { op: 'sdiv', op_id: ':sdiv' },
          { op: 'smult', op_id: ':smult' }
     ...
     * @instance
     * @param {string} op_id - the op_id associated with the preprocessed value/share.
     * @param {secret_share} share - the share/value to store.
     */
    ...
    jiff.__preprocessing = function (dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, id_list, params) {
      var find_closest_namespace = function (op, starting_namespace) {
        ...
        while (namespace_index >= 0) {
          ...
          if (jiff.preprocessing_function_map[namespace][op] != null) {
            return namespace;
          }
          namespace_index--;
        }
      ...
      if (protocols.beaver == null) {
        protocols.beaver = jiff.protocols.generate_beaver_bgw;
        protocols.generate_bit_sequence = jiff.protocols.generate_bit_sequence;
      }
      ...
      for (var i = 0; i < count; i++) {
        ...
        if (namespace == null) {
          ...
          var result = protocol(threshold, receivers_list, compute_list, Zp, id, params);
          ...
          for (var k = 0; k < preprocessing_dependencies.length; k++) {
            ...
            var extra_params = Object.assign({}, params, dependency['params']); // TODO: IE11??
            ...
            var next_id_list = [];// compose ids similar to how the actual operation is implemented
    ...
    /**
     * Keeps track of the MOST RECENT pre-processing task [Do not modify]
     * Used to make sure that no two tasks can execute at the same time, and
     * that pre-processing tasks are executed in the order they are created.
     * @member {deferred} __preprocessingDeferred
     * @memberof jiff-instance
     * @instance
     */
    jiff.__preprocessingDeferred = new Deferred();
    jiff.__preprocessingDeferred.resolve();
    ...
    jiff.preprocessing = function (dependent_op, count, batch, protocols, threshold, receivers_list, compute_list, Zp, id_list, params) {
      ...
      jiff.__preprocessingDeferred.promise.then(function () {
        ...
        params['__namespace_index'] = jiff.extensions.indexOf(params['namespace']);

        (function batchPreprocess(start) {
          var thisBatch = Math.min(count - start, batch);
          var thisList = id_list;
          if (id_list != null) {
            thisList = id_list.slice(start, start + thisBatch);
          }

          var promise = jiff.__preprocessing(dependent_op, thisBatch, protocols, threshold, receivers_list, compute_list, Zp, thisList, params);
          promise.then(function () {
            if (start + batch < count) {
              batchPreprocess(start + batch);
            } else {
      ...
      jiff.__preprocessingDeferred = finalDeferred;
      return finalDeferred.promise;
    };

    /**
     * Call to indicate to JIFF that the preprocessing phase is finished.
     * @method finish_preprocessing
     * @memberof jiff-instance
     * @instance
     */
    jiff.finish_preprocessing = function () {
      jiff.counters.reset();
    };
    ...
    jiff.socket.on('public_keys', function (msg, callback) {
      ...
      msg = JSON.parse(msg);
      if (jiff.id !== 's1') {
        msg = jiff.hooks.decryptSign(jiff, msg, jiff.secret_key, jiff.keymap['s1']);
        msg = JSON.parse(msg);
      }
    ...
    jiff.counters.gen_triplet_id = function (holders) {
      var label = jiff.op_id_seed + 'triplet:' + holders.join(',');
      if (jiff.counters.triplet_op_count[label] == null) {
        jiff.counters.triplet_op_count[label] = 0;
      }
      return label + ':' + (jiff.counters.triplet_op_count[label]++);
    };
     ...
     * number for all parties, as long as all parties are executing instructions in the same order.
     * @param {Array} holders - an array containing the ids of all the holders of the number.
     * @return {string} the op_id for the open.
     */
    jiff.counters.gen_number_id = function (holders) {
      var label = jiff.op_id_seed + 'number:' + holders.join(',');
      if (jiff.counters.number_op_count[label] == null) {
        jiff.counters.number_op_count[label] = 0;
      }
      return label + ':' + (jiff.counters.number_op_count[label]++);
    };
    ...
    jiff.socket.on('triplet', function (msg, callback) {
      callback(true); // send ack to server
      ...
      // parse message
      receive_triplet(jiff, msg);
    });

    jiff.socket.on('number', function (msg, callback) {
      callback(true); // send ack to server
      ...
      // parse message
      receive_server_share_number(jiff, msg);
    });
