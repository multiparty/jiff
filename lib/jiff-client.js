  ...
  */
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.generate_beaver_bgw = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      ...
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('*', receivers_list);
      }
     ...
     * @memberof jiff-instance
     * @instance
     * @param {string} op - name of the operation that will later use the pre_processed values
     * @param {Function} protocol - function within jiff.protocols that generates the specified values (e.g. jiff.protocols.generate_beaver_bgw).
     * @param {Number} count - number of times the protocol should be performed, number of values that will be generated
     * @param {Array} [threshold=receivers_list.length] - the threshold of the preprocessed shares.
     * @param {Array} [threshold=receivers_list.length] - the threshold of the preprocessed shares.
     ...
     * @param {Array} [Zp=receivers_list.length] - the Zp of the preprocessed shares.
     * @param {Array} [id_list] - array of ids to be used sequentially to identify the pre_processed values. Optional.
     * @param {Array} [params={}] - any additional protocol-specific parameters.
     */
    jiff.preprocessing = function (dependent_op, protocols, count, threshold, receivers_list, compute_list, Zp, id_list, params) {
      if (receivers_list == null) {
        receivers_list = [];
        for (var p = 1; p <= jiff.party_count; p++) {
      ...
      if (protocols == null) {
        protocols = {
          beaver: jiff.protocols.generate_beaver_bgw,
        }
      }
      ...
      for (var i = 0; i < count; i++) {
        ...
        if (jiff.preprocessing_function_map[dependent_op] == null) {
          console.log('base case')
          var protocol = protocols[dependent_op];
          var result = protocol(threshold, receivers_list, compute_list, Zp, id, params);
          jiff.store_preprocessing(result.op_id, result.share);
          console.log('stored id: ', result.op_id);
          promises.push(result.promise);
        } else {
          var preprocessing_dependencies = jiff.preprocessing_function_map[dependent_op];
          console.log('recursively computing for ', dependent_op);

          for (var k = 0; k < preprocessing_dependencies.length; k++) {
            var next_op = preprocessing_dependencies[k]['op'];
            var extra_params = preprocessing_dependencies[k]['params'];
            //var nested_id = preprocessing_dependencies[k]['op_id'];
            jiff.preprocessing(next_op, null, 1, threshold, receivers_list, compute_list, Zp, [id], extra_params);
          }
        }
      }
      return Promise.all(promises);
