  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.share_ND_array = function (secrets, skeletons, threshold, receivers_list, senders_list, Zp, share_id) {
      ...
      Promise.all(all_promises).then(function (array) {
        ...
        for (i in senders_list) {
          let p_id = senders_list[i];
          shares[p_id] = array[p_id];
            // //console.log(jiff.id, 'all_promises shares['+p_id+']', Object.keys(shares[p_id]));
        }
    ...
    jiff.serialize = function (shares) {
      ...
      if (typeof(shares.length) === 'undefined' || shares.length === 0) {
        ...
        Promise.all(deferred_serials).then(function (serials) {
          final_deferred.resolve(JSON.stringify(serials));
        });
      }
    ...
    jiff.open_ND_array = function (shares, parties, op_id) {
      ...
      if (typeof(shares.length) === 'undefined') {
        return Promise.resolve(jiff.internal_open(shares, parties, op_id + ':open'));
