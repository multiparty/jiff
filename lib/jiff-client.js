...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.seq = function (o, op_id) {
      ...
      return self.issub(o).icpow(self.jiff.Zp-1, op_id).not();
    };
    ...
    self.ceq = function (cst, op_id) {
      ...
      return self.icsub(cst).icpow(self.jiff.Zp-1, op_id).not();
    };
    ...
    /**
     * Constant (modular) exponentiation by squaring
     * @method cpow
     * @memberof SecretShare
     * @param {number} - the exponent (index)
     * @param {string} [op_id=auto_gen()] - the operation id which is used to identify this operation.
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
     *                         ensure that corresponding instructions across different parties are matched correctly.
     * @return {SecretShare} this party's share of the power raised to the provided exponent.
     * @instance
     */
    self.cpow = function (exponent, op_id) {
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('c^', self.holders);
      }

      var exponent_bits = self.jiff.helpers.number_to_bits(exponent);
      var base = self;
      var power = exponent_bits[i] ? base : 1;
      for (var i = 1; i < exponent_bits.length; i++) {
        base = base.smult(base, op_id + ':' + i);
        if (exponent_bits[i]) {
          power = (power === 1)? base : base.mult(power, op_id + ':e' + i);
        }
      }
      return power;
    };
    ...
    var internals = ['cadd', 'csub', 'cmult', 'sadd', 'ssub', 'smult',
      ...
      'sdiv', 'cdiv', 'cpow', 'not', 'lt_halfprime', 'if_else'];
