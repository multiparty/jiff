...
(function(exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
     ...
     * @return {share-object} this party's share of the result.
     */
    this.add_vec = function(o) {
      if (!(o.jiff === self.jiff)) throw "shares do not belong to the same instance (+)";
      if (!(o.Zp === self.Zp)) throw "shares must belong to the same field (+)";

      //if (!(self.value.length !== o.value.length)) throw "shares must have the same length";
      console.log("I'm here");
      console.log(o);

      // add the two shares when ready locally
      var ready_add_vec = function() {
        var result = [];
        for(var i = 0; i < self.value.length; i++){
          result.push(mod(self.value[i] + o.value[i], self.Zp));
        }
        console.log("i computed a result");
        console.log(result);
        return result;
      }

      if(self.ready && o.ready) // both shares are ready
        return new secret_share(self.jiff, true, null, ready_add_vec(), self.Zp);

      // promise to execute ready_add when both are ready
      var promise = self.pick_promise(o).then(ready_add_vec, self.error);
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    /**
     * Flags whether this instance is connected and the server signaled the start of computation. [Do not use; ufse isReady() instead]
    ...
    jiff.wait_for = function(parties, callback) {
      ...
      // TODO: what if server was not connected? does sockets.io keep emits in a back log then send them or are they lost?
    }
    ...
    jiff.establish_server_side_connections = options.establish_server_side_connections === true;
    ...
    jiff.share = function(secret, threshold, receivers_list, senders_list, Zp) { 
      return jiff_share(jiff, secret, threshold, receivers_list, senders_list, Zp);
    };

    /**
     * Share multiple secret inputs.
     * @method share_vec
     * @memberof jiff.jiff-instance
     * @instance
     * @param {array} secrets - the numbers to share (this party's input).
     * @param {number} threshold - the minimimum number of parties needed to reconstruct the secret, defaults to all the recievers [optional].
     * @param {array} receivers_list - array of party ids to share with, by default, this includes all parties [optional].
     * @param {array} senders_list - array of party ids to receive from, by default, this includes all parties [optional].
     * @param {number} Zp - the modulos (if null then the default Zp for the instance is used) [optional].
     * @returns {array} a list of map (of size equal to the number of parties)
     *          where the key is the party id (from 1 to n)
     *          and the value is the share object that wraps
     *          the value sent from that party (the internal value maybe deferred).
     */
    jiff.share_vec = function(secrets, threshold, receivers_list, senders_list, Zp) { 
      result = [];
      secrets.map(function(secret){
        result.push(jiff_share(jiff, secret, threshold, receivers_list, senders_list, Zp));
      });
      return result;
    };
