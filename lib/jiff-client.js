...
(function (exports, node) {
  ...
  function jiff_open_array(jiff, shares, parties, op_ids) {
    ...
    for (var i = 0; i < shares.length; i++) {
      ...
      var ids = {};
      for (var p in op_ids) {
        if (op_ids.hasOwnProperty(p)) {
          ids[p] = op_ids[p] + ':' + i;
        }
      }

      promises.push(jiff.open(shares[i], party, ids));
    }
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    // setup main socket
    // eslint-disable-next-line no-undef
    jiff.socket = (options.__internal_socket == null ? io(hostname, {autoConnect: false, transports: ['polling']}) : options.__internal_socket);
