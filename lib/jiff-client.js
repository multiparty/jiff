...
(function (exports, node) {
   ...
   *
   */
  function many_secret_shares(jiff, count, holders, threshold, Zp, id, ratios) {
    var deferreds = [];
    var shares = [];
    for (var i = 0; i < count; i++) {
      var deferred = new Deferred();
      shares.push(jiff.secret_share(jiff, false, deferred.promise, undefined, holders, threshold, Zp, 'share:' + i + ':' + id, ratios));
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     */
    jiff.protocols.bits.share_bits = function (secret, bit_length, threshold, receivers_list, senders_list, Zp, share_id, receivers_ratios) {
      var i;
      if (Zp == null) {
        Zp = jiff.Zp;
      }
      ...
      for (i = 0; i < bit_length; i++) {
        var round = jiff.internal_share(local_bits[i], threshold, receivers_list, senders_list, Zp, share_id + ':' + i, receivers_ratios);
    ...
    jiff.protocols.bits.sadd = function (bits1, bits2, op_id) {
      ...
      // initialize results
      var result = many_secret_shares(jiff, bits1.length, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id, bits1[0].ratios);
      var deferreds = result.deferreds;
      result = result.shares;
    ...
    jiff.protocols.bits.ssub = function (bits1, bits2, op_id) {
      ...
      // initialize results
      var result = many_secret_shares(jiff, Math.max(bits1.length, bits2.length), bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id, bits1[0].ratios);
      var deferreds = result.deferreds;
      result = result.shares;
    ...
    jiff.protocols.bits.smult = function (bits1, bits2, op_id) {
      ...
      var result = many_secret_shares(jiff, bits1.length + bits2.length + offset, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id, bits1[0].ratios);
      var deferreds = result.deferreds;
      result = result.shares;
    ...
    jiff.protocols.bits.sdiv = function (bits1, bits2, op_id) {
      ...
      // Initialize the result
      var quotient = many_secret_shares(jiff, bits1.length, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id + ':quotient', bits1[0].ratios);
      var quotientDeferreds = quotient.deferreds;
      quotient = quotient.shares;

      var remainder = many_secret_shares(jiff, Math.min(bits1.length, bits2.length), bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id + ':remainder', bits1[0].ratios);
