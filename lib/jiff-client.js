...
(function (exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * @throws error if some shares does not belong to the passed jiff instance.
     */
    jiff.open_ND_array = function (shares, parties, op_ids) {
      var i;
      var final_deferred = new Deferred();
      ...
      // compute operation id
      if (op_ids == null) {
        op_ids = jiff.counters.gen_op_id('open_ND_array', parties);
      }
      ...
      console.log('send skeleton', jiff.id, op_ids, skeleton);
      jiff.emit(op_ids + 'skeleton', parties, JSON.stringify(skeleton));

      (function __open_ND_array(shares, parties, op_ids) {
        if (typeof(shares.length) === 'undefined') {
          return Promise.resolve(jiff.internal_open(shares, parties, op_ids + ':open'));
      ...
      })(shares, parties, op_ids).then(function (opened) {
    ...
    jiff.receive_open_ND_array = function (parties, threshold, Zp, op_id) {
      var final_deferred = new Deferred();
      if (Zp == null) {
        Zp = jiff.Zp;
      }
      ...
      jiff.listen(op_id + 'skeleton',  function (sender, skeleton) {
        ...
        if (typeof(skeleton) === 'string') {
          ...
          console.log('receive skeleton', jiff.id, op_id, skeleton);
        }
