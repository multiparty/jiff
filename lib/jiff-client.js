...
(function(exports, node) {
  ...
  function jiff_share(jiff, secret, receivers_list, senders_list, Zp, op_id) {
    ...
    if(receivers_list.indexOf(jiff.id) > -1) {
      ...
      for(var i = 0; i < senders_list.length; i++) {
        ...
        if(p_id == jiff.id) {
          result[p_id] = new secret_share(jiff, true, null, shares[p_id], receivers_list.join(","), Zp);
        ...
        result[p_id] = new secret_share(jiff, false, promise, undefined, receivers_list.join(","), Zp);
      }
    }
  ...
  function jiff_compute_shares(secret, party_count, parties_list, Zp) {
    ...
    for(var i = 0; i < parties_list.length; i++) {
      ...
      var power = get_party_number(party_count, parties_list[i]);

      for(var j = 1; j < polynomial.length; j++) {
        shares[p_id] = mod((shares[p_id] + polynomial[j] * power), Zp);
        power = power * get_party_number(party_count, parties_list[i]);
      }
    }
  ...
  function jiff_open(jiff, share, parties, op_id) {
    ...
    if(op_id == null) {
      if(jiff.open_op_count[share.holders] == null) jiff.open_op_count[share.holders] = 0;
      op_id = "open:" + share.holders + ":" + (jiff.open_op_count[share.holders]++);
    }
  ...
  function jiff_lagrange(shares, party_count, Zp) {
    var lagrange_coeff = Array(party_count+1 + 1); // + 1 for "s1"

    // Compute the Langrange coefficients at 0
    for(var i = 1; i <= party_count + 1; i++) {
      lagrange_coeff[i] = 1;
      for(var j = 1; j <= party_count + 1; j++) {
    ...
    recons_secret = mod((recons_secret + shares["s1"] * lagrange_coeff[party_count+1]), Zp);
   ...
   * @param {string} holders - the parties that hold all the corresponding shares.
