...
(function (exports, node) {
  ...
  */
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     */
    jiff.preprocessing = function (op, protocols, count, threshold, receivers_list, compute_list, Zp, id_list, params) {
      if (receivers_list == null) {
        receivers_list = [];
        for (var i = 1; i <= jiff.party_count; i++) {
          receivers_list.push(i);
      ...
      for (i = 0; i < count; i++) {
        var id = id_list[i];
        if (id == null) {
          id = jiff.counters.gen_op_id(op, receivers_list);
        }
        console.log(op, id);
        if (jiff.preprocessing_function_map[op] == null) {
          var protocol = protocols[op]
          ...
          var preprocessing_dependencies = jiff.preprocessing_function_map[op];
