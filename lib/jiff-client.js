...
(function (exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * This takes the same parameters as jiff-instance.share, but returns an array of secret bit shares.
     * @method share
     * @memberof jiff-instance
     * @instance
     * @param {number} secret - the number to share (this party's input)
     ...
     *          the value sent from that party (the internal value maybe deferred).
     */
    jiff.share_number_as_bits = function (secret, threshold, receivers_list, senders_list, Zp, share_id) {
        var local_bits = jiff.helpers.number_to_bits(secret, Math.ceil(Math.log2(number)));
        var shared_bits = [];
        for (var i = 0; i < local_bits.length; i++) {
            shared_bits[i] = jiff_share(jiff, local_bits[i], threshold, receivers_list, senders_list, Zp, share_id + ':bit' + i);
        }
        return shared_bits;
    };
     ...
     * This works regardless of whether the represented value fits inside the corresponding field.
     * @method open_bits
     * @memberof jiff-instance.protocols.bits
     * @instance
     * @param {SecretShare[]} bits - an array of the secret shares of bits, starting from least to most significant bits.
     * @param {number[]} party_ids - parties to open (same as jiff.open)
     * @param {String} op_id - op_id to id open operation (will be concatenated with a counter for each bit)
     * @returns {promise} a promise to the number represented by bits.
     */
    jiff.protocols.bits.open_bits = function (bits, party_ids, op_id) {
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('bits.open_bits', bits[0].holders);
      }

      var opened_bits = [];
      for (var i = 0; i < bits.length; i++) {
        opened_bits[i] = bits[0].jiff.open(bits[i], party_ids, op_id + ':' + i);
      }
