  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    var guard_socket = function (socket) {
      ...
      socket.safe_emit = function (label, msg) {
        ...
        var mailbox_pointer = socket.mailbox.add({label: label, msg: msg, time: Date.now()});
        if (socket.connected) {
    ...
    jiff.preprocessingDaemon = function () {
      ...
      (function () {
        ...
        promise.then(function () {
          ...
          if (jiff.inspectDebugPromises != null) { // TODO remove debugging code later
            jiff.inspectDebugPromises(Number.MAX_VALUE);
          }
