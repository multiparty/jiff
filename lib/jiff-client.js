...
function JIFFClient(hostname, computation_id, options) {
   ...
   * @type {number}
   */
  if (options.party_id != null && !(typeof(options.party_id) === 'number')) {
    throw new Error('"' + options.party_id + '" is not a number.  Please use a numerical party ID from [1, 2, ...].');
  }
  ...
  /**
   * A collection of useful protocols for manipulating GMW shares.
   * @member {!gmw} gmw
   * @memberof protocols
   */
  this.protocols.gmw = {};
  ...
  /**
   * A collection of useful protocols for manipulating GMW bit arrays, and transforming them from and to regular numeric shares.
   * @member {!bits} bits
   * @memberof protocols.gmw
   */
  this.protocols.gmw.bits = {};
