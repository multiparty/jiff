...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.sdiv = function (o, l, op_id) {
      ...
      if (l == null) {
        l = Math.ceil(Math.log2(self.Zp));  // maximum bit-length in Zp
      }
      ...
      Promise.all(promise_bits).then(function (decomposed_bits) {
        ...
        quotient.promise.then(function (share_value) {
            final_deferred.resolve(share_value);
        });
      });

      return self.jiff.secret_share(self.jiff, true, final_promise, undefined, self.holders, self.threshold, self.Zp, 'share:quotient:' + op_id);
    };
