...
(function(exports, node) {
        ...
        (function(promise, p_id) { // p_id is modified in a for loop, must do this to avoid scoping issues.
          promise.then(function(received_share) {
            ...
            if(final_deferred != null) {
              final_deferred.resolve(jiff_lagrange(shares, jiff.party_count));
              final_deferred = null;
            }
   ...
   *
   */
  function jiff_lagrange(shares, party_count, jiff) {
    ...
    for(var i = 0; i < shares.length; i++) {
      ...
      for(var j = 0; j < shares.length; j++) {
        ...
        if(pj != pi) {
          var inv = extended_gcd(pi - pj, shares[i].Zp)[0];
          lagrange_coeff[pi] = lagrange_coeff[pi] * (0 - pj) * inv;
          lagrange_coeff[pi] = mod(lagrange_coeff[pi], shares[i].Zp);
        }
      }
    }
    ...
    this.add_vec = function(o){
      if(o instanceof Array){
        if(typeof(o[0]) == "number"){
          sum_vec = [];
          o.map(function(item){
            var shares = this.share(item);
            var sum = shares[1];
            for(var i = 2; i <= this.party_count; i++){
              sum = sum.cadd(shares[i]);
            }
            sum_vec.push(sum.open_to_promise().then(success, failure)); 
          });
           
          Promise.all(sum_vec).then(function(results){
            return results;
          });
        }
        else {
          throw "object of type secret currently not supported";
        }
      }
      else {
        throw "object shared is not an array";
      }
    }
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.share = function(secret, threshold, receivers_list, senders_list, Zp) { 
      var test = jiff_share(jiff, secret, threshold, receivers_list, senders_list, Zp);
      return test;
    };
