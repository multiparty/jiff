...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
     ...
     * @memberof secret-share
     */
    self.beaver_bgw = function (x, y, receivers_list, threshold, Zp, triplet_id) {
      /**
       * 1. share x, y
      ...
      if (triplet_id == null) {
        triplet_id = self.counters.gen_triplet_id(receivers_list);
      }
      ...
      var x_shares = self.internal_share(zi, threshold, receivers_list, receivers_list, Zp, op_id);
      var y_shares = self.internal_share(y, threshold, receivers_list, receivers_list, Zp, op_id);
      ...
      Promise.all(promises).then( function (results) {
        for (var i = 2; i <= self.party_count; i++) {
          x_sum = x_sum.sadd(shares[i]);
          y_sum = y_sum.sadd(shares[i]);
        }
      },
        function (err) {
          console.log(err);
