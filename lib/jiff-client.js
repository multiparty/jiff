...
(function (exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * @param {Array} receivers_list = all_parties
     * @param {Array} senders_list = all_parties
     * @param {object} skeletons - If set, this will skip the listening step and share right away
     *                             using the structure specified for each party. This should be a
     *                             mapping with the senders_list as keys and skeletons as values.
     * @param {number} Zp = jiff_instance.Zp
     * @param {string|number} share_id = auto_gen()
     ...
     */
    jiff.share_ND_array = function (secrets, threshold, receivers_list, senders_list, skeletons, Zp, share_id) {
      var i, p_id;
      ...
      let share_from_skeleton = function (sender, __skeleton) {
        ...
        (function __share_ND_array(secrets, share_id) {
          if (typeof(secrets) === 'number' || secrets == null) {
            // Safety Checks
            if (secrets != null && (Math.floor(secrets) !== secrets || secrets < 0)) {
              console.log('secrets', secrets);
              throw new Error('secret must be a non-negative whole number');
            }
            if (secrets != null && (secrets >= Zp)) {
              console.log('secrets', secrets);
              throw new Error('secret must fit inside Zp');
            }

            return jiff.internal_share(secrets, threshold, receivers_list, [sender], Zp, share_id + ':share:' + secrets)[sender];
          } else if (typeof(secrets.length) === 'undefined') {
            throw new Error('Secret must be either type of number or array');
            ...
            var deferred_array = new Deferred();
        ...
        })(__skeleton, share_id).then(function (sender, array) {
      ...
      var all_promises = [null];  // There is no zeroth party
      for (p_id in final_deferreds) {
