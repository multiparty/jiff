...
(function (exports, node) {
   ...
   *                           all parameters are optional, However, private and public key must either be both provided or neither of them provided.
   <pre>
   {
     ...
     "sodium": boolean (indicates whether to use libsodium wrappers or not),
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * All parties must share arrays of the same shape
     * @method share_ND_array_symmetrical
     * @memberof jiff-instance
     * @instance
     * @param {Array} secrets - the n-dimensional array of numbers to share (this party's input)
     * @param {number} threshold = receivers_list.length
     * @param {Array} receivers_list = all_parties
     * @param {Array} senders_list = all_parties
     * @param {number} Zp = jiff_instance.Zp
     * @param {string|number} share_id = auto_gen()
     * @returns {object} a map (of size equal to the number of senders)
     *                   where the key is the party id (from 1 to n)
     *                   and the value are each an [n+1]-dimensional
     *                   array corresponding to the secrets in the
     *                   original passed array, but with the deepest
     *                   elements each being an array of SecretShares
     *                   indexed by the parameter receivers_list.
     */
    jiff.share_ND_array_symmetrical = function (secrets, threshold, receivers_list, senders_list, Zp, share_id) {
      var final_deferred = new Deferred();

      if (share_id == null) {
        share_id = jiff.counters.gen_share_id(receivers_list, senders_list) + ':share_ND_array_sym';
      }
      ...
      if (typeof(secrets.length) === 'undefined' || secrets.length === 0) {
        return jiff.internal_share(secrets, threshold, receivers_list, senders_list, Zp, share_id + ':share:' + secrets);
      } else {
        var deferred_array = [];

        for (var i = 0; i < secrets.length; i++) {
          deferred_array.push(jiff.share_ND_array_symmetrical(secrets[i], threshold, receivers_list, senders_list, Zp, share_id + ':' + i));
        }

        Promise.all(deferred_array).then(function (array) {
          final_deferred.resolve(array);
        });
     ...
     *                   original passed array, but with the deepest
     *                   elements each being an array of SecretShares
     *                   indexed by the parameter senders_list.
     */
    jiff.share_ND_array = function (secrets, threshold, receivers_list, senders_list, Zp, share_id) {
      var i;
      ...
      if (share_id == null) {
        share_id = jiff.counters.gen_share_id(receivers_list, senders_list) + ':share_ND_array';
      }
      if (threshold == null) {
        threshold = receivers_list.length;
      } else if (threshold < 0) {
      ...
      // All secrets are replaced by a preliminary senders_list while maintaing the array's orginial structure.
      let skeleton = (function __wipe(nd_array, replace) {
        if (!(typeof(nd_array.length) === 'undefined' || nd_array.length === 0)) {
          var wiped_array = [];
          for (i = 0; i < nd_array.length; i++) {
            wiped_array.push(__wipe(nd_array[i], replace));
          }
          return wiped_array;
        }
        return replace;
      })(secrets, (senders_list.indexOf(jiff.id) > -1) ? [jiff.id] : []);

      // Check a stringified index and get if set
      // TODO: Change to array based indexing instead of using strings
      let get = function (__var, index, __else = undefined) {
        var __get = __else;
        try {
            __get = eval('__var' + index);  // FIX THIS
        } catch {
            throw new Error('Unset index: ' + index);
        }
        return __get;
      }

      // Merge two multidimensional arrays of party-id arrays
      let __merge = function (nd_array1, nd_array2) {
        if (typeof(nd_array1.length) === 'undefined' && typeof(nd_array2.length) === 'undefined') {
          return [nd_array1, nd_array2].sort((i, j) => i - j);
        } else if (typeof(nd_array1.length) === 'undefined') {
          return [nd_array1].concat(nd_array2).sort((i, j) => i - j);
        } else if (typeof(nd_array2.length) === 'undefined') {
          return nd_array1.concat([nd_array2]).sort((i, j) => i - j);
        } else if (nd_array1.length <= 1 && nd_array2.length <= 1) {
          return nd_array1.concat(nd_array2).sort((x, y) => x - y);
        } else {
          let shorter_array = (nd_array1.length < nd_array2.length) ? nd_array1 : nd_array2;
          let longer_array = (nd_array1.length >= nd_array2.length) ? nd_array1 : nd_array2;
          var merged_array = longer_array;

          for (i = 0; i < shorter_array.length; i++) {
            merged_array[i] = __merge(shorter_array[i], longer_array[i]);
          }
          return merged_array;
        }
      }
      ...
      jiff.listen(share_id + 'skeleton', function (sender, message) {
        if (sender !== jiff.id) {
          merged_skeleton = __merge(merged_skeleton, JSON.parse(message));
        }
        ...
        if (total === senders_list.length) {
          ...
          (function __share_ND_array(id_skeleton, share_id, index) {
            var deferred = new Deferred();

            if (typeof(id_skeleton.length) === 'undefined' || id_skeleton.length === 0 || id_skeleton[0].length === 0) {
              // This should never happen, but catch anyway for future debugging
              throw new Error('Array skeleton has outliers');
            } else if (typeof(id_skeleton[0].length) === 'undefined') {
              var secret = get(secrets, index, null);
              var share_senders_list = id_skeleton;
              return jiff.internal_share(secret, threshold, receivers_list, share_senders_list, Zp, share_id + ':share:' + secret);
            } else {
              var promised_array = [];

              for (i = 0; i < id_skeleton.length; i++) {
                promised_array.push(__share_ND_array(id_skeleton[i], share_id + ':' + i, index + '[' + i + ']'));
              }
          ...
          })(merged_skeleton, share_id, '').then(function (array) {
            final_deferred.resolve(array);
          });
        }
      });
    ...
    /**
     * Export an array of secret shares of any dimention as a string in JSON
     * This takes the array of shares as its only parameter.
     * @method serialize
     * @memberof jiff-instance
     * @instance
     * @param {SecretShare|SecretShare[]} shares - the (multi)dimensional array of shares, or just a single share
     * @returns {promise} a (JQuery) promise to the JSON string representing and array of the secret shares' known values/coefficients
     */
    jiff.serialize = function (shares) {
      var final_deferred = new Deferred();

      if (typeof(shares.length) === 'undefined' || shares.length === 0) {
        if (shares.ready) {
          final_deferred.resolve(JSON.stringify(shares.value));
        } else {
          shares.promise.then(function () {
            final_deferred.resolve(JSON.stringify(shares.value));
          });
        }
      } else {
        var deferred_serials = [];

        for (var i = 0; i < shares.length; i++) {
          deferred_serials.push(jiff.serialize(shares[i]));
        }

        Promise.all(deferred_serials).then(function (serials) {
          var json = '[';
          for (var i = 0; i < serials.length - 1; i++) {
            json += serials[i] + ', ';
          }
          json += serials[serials.length - 1] + ']';

          final_deferred.resolve(json);
        });
      }
    ...
    /**
     * Recreate an array of secret shares from its JSON representation
     * This takes the JSON string of shares, and its new holders as the only required parameters.
     * @method parse
     * @memberof jiff-instance
     * @instance
     * @param {String} json the JSON string representing and array of the secret shares' known values/coefficients
     * @param {Array} holders - the parties that hold all the corresponding shares (must be sorted) - same as in jiff-instance.secret_share(..., holders, ...)
     * @param {number} threshold - the min number of parties needed to reconstruct the secrets - same as in jiff-instance.secret_share(..., threshold, ...)
     * @param {number} Zp - the mod under which this share should be created.
     * @param {number} op_id - the tag to identify new shares
     * @returns {SecretShare|SecretShare[]} shares - the (multi)dimensional array of shares, or just a single share
     */
    jiff.parse = function (json, holders, threshold, Zp, op_id) {
      // Parse JSON if neccessary
      if (typeof(json) === 'string') {
        json = JSON.parse(json);
      }

      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('parse', holders);
      }
      ...
      if (threshold == null) {
        threshold = jiff.party_count;
      }

      // Recursively recreate the shares
      var shares = [];
      if (Object.keys(json).length === 0) {
        shares = jiff.secret_share(jiff, true, null, json, holders, threshold, Zp, op_id + ':share:' + String(json));
      } else {
        for (var i = 0; i < json.length; i++) {
          shares[i] = jiff.parse(json[i], holders, threshold, Zp, op_id);
        }
      }
      return shares;
    }
     ...
     * @throws error if some shares does not belong to the passed jiff instance.
     */
    jiff.open_ND_array = function (shares, parties, op_id) {
      var i;
      var final_deferred = new Deferred();
      ...
      if (typeof(shares.length) === 'undefined' || shares.length === 0) {
        return jiff.internal_open(shares, parties, op_id + ':open');
      } else {
        var deferred_array = [];

        for (i = 0; i < shares.length; i++) {
          deferred_array.push(jiff.open_ND_array(shares[i], parties, op_id + ':' + i));
        }
     ...
     *
     */
    jiff.protocols.bits.generate_random_bit = function (compute_threshold, compute_list, receiving_threshold, receivers_list, Zp, op_id) {
      if (op_id == null) {
    ...
    jiff.protocols.bits.smult = function (bits1, bits2, op_id) {
      ...
      // Initialize the result
      var result = many_secret_shares(jiff, bits1.length + bits2.length, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id);
      var deferreds = result.deferreds;
      result = result.shares;
    ...
    jiff.protocols.bits.sneq = function (bits1, bits2, op_id) {
      ...
      bit_combinator(deferred, 1, bits1.length, initial, function (i, prev) {
        ...
        if (i < bits2.length) {
          ...
          next = prev.isor_bit(bits2[i], op_id + ':sor_bit:' + (i - 1));
        }
        return next;
      });
