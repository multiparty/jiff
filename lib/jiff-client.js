...
(function (exports, node) {
  ...
  function jiff_share_all_number(jiff, n, threshold, receivers_list, compute_list, Zp, params) {
    ...
    if (params.compute_threshold == null) {
      params.compute_threshold = compute_list.length;
    }
    ...
    if (isSender) {
      ...
      for (i = 1; i < compute_list.length; i++) {
    ...
    return {share: result, promise: result.promise};
  }
   ...
   *                           { "number": number, "bit": boolean, "min": number, "max": number, "count": number, "bitLength": number}
   *                           if bitLength is provided, each number is generated and shared bitwise, and padded to "bitLength" many bits.
   *                           in this case, returned result will be an array of bit shares per count (a 2D array).
   * @param {Array} [receivers_list=all_parties] - array of party ids that want to receive the triplet shares, by default, this includes all parties.
   ...
   *                             is generated by increasing a local counter, default ids suffice when all parties execute the
   *                             instructions in the same order.
   * @return {SecretShare[]|SecretShare[][]} - this party's share of the generated number.
   */
  function jiff_server_share_number(jiff, options, receivers_list, threshold, Zp, number_id) {
    ...
    for (i = 0; i < options.count; i++) {
      ...
      if (options.bitLength != null) {
        shares[i] = [];
        for (var b = 0; b < options.bitLength; b++) {
          var bitPromise = promise.then(function (b, bits) {
            return bits[b];
          }.bind(null, b));
          shares[i][b] = jiff.secret_share(jiff, false, bitPromise, undefined, receivers_list, threshold, Zp);
        }
      } else {
        shares[i] = jiff.secret_share(jiff, false, promise, undefined, receivers_list, threshold, Zp);
      }
    }
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp) {
    ...
    self.lt_halfprime = function (op_id) {
      ...
      if (bits == null) {
        bits = jiff.server_generate_and_share({bitLength: bitLength, max: self.Zp}, self.holders, self.threshold, self.Zp, op_id + ':sampling')[0];
      }
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     *                             is generated by increasing a local counter, default ids suffice when all parties execute the
     *                             instructions in the same order.
     * @returns {SecretShare[]|SecretShare[][]} an array of secret shares of shares of generated numbers according to options.
     */
     ...
     * @memberof jiff-instance.protocols
     * @instance
     * @param {number} [threshold=receivers_list.length] - the min number of parties needed to reconstruct the secret after it is computed, defaults to all the receivers.
     * @param {Array} [receivers_list=all_parties] - array of party ids to receive the result, by default, this includes all parties.
     * @param {Array} [compute_list=all_parties] - array of party ids to perform the protocol, by default, this includes all parties.
    ...
    jiff.protocols.generate_beaver_bgw = function (threshold, receivers_list, compute_list, Zp, params, protocols) {
      ...
      if (compute_list.indexOf(jiff.id) > -1) {
        ...
        a = protocols.generate_random_number(compute_threshold, compute_list, compute_list, Zp, params, protocols).share;
        params.op_id = op_id + ':share_b';
        b = protocols.generate_random_number(compute_threshold, compute_list, compute_list, Zp, params, protocols).share;
      ...
      return { op_id: op_id + ':triplet', share: [a, b, c], promise: Promise.all(promises) };
    };
    ...
    jiff.protocols.generate_random_bit_bgw  = function (threshold, receivers_list, compute_list, Zp, params) {
      ...
      var op_id = params.op_id;
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id2('generate_random_bit_bgw', receivers_list, compute_list);
      }
    ...
    jiff.protocols.generate_random_bits = function (threshold, receivers_list, compute_list, Zp, params, protocols) {
      ...
      var op_id = params.op_id;
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id2('generate_random_bits', receivers_list, compute_list);
      }
    ...
    var __rejection_sampling = function (lower_bound, upper_bound, compute_list, Zp, params, protocols) {
      ...
      (function resample(reject_count) {
        ...
        if (jiff.crypto_provider === true) {
          ...
          var promise = jiff.__preprocessing('bits.clt', 1, protocols, params.compute_threshold, compute_list, compute_list, Zp, [params.op_id + ':bits.clt:' + reject_count], paramsCopy);
          if (lower_bound.toString() !== '0') {
            paramsCopy['op_id'] = params.op_id + ':preprocessing:bits.cadd';
            var promise2 = jiff.__preprocessing('bits.cadd', 1, protocols, params.compute_threshold, compute_list, compute_list, Zp, [params.op_id + ':bits.cadd:' + reject_count], paramsCopy);
    ...
    jiff.protocols.bits.rejection_sampling = function (threshold, receivers_list, compute_list, Zp, params, protocols) {
      ...
      // fix threshold
      for (var i = 0; i < bitLength; i++) {
        result[i] = jiff.protocols.reshare(result[i], threshold, receivers_list, compute_list, Zp, params.op_id + ':reshare:' + i);
        if (result[i] != null) {
    ...
    jiff.__preprocessing = function (dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, id_list, params) {
      ...
      for (var i = 0; i < count; i++) {
        var paramsCopy = Object.assign({}, params);
        if (paramsCopy.op_id != null) {
          paramsCopy.op_id = paramsCopy.op_id + i;
        }
        ...
        var namespace = find_closest_namespace(dependent_op, paramsCopy['namespace']);
        if (namespace == null) {
          var protocol = protocols[dependent_op];
          var result = protocol(threshold, receivers_list, compute_list, Zp, paramsCopy, protocols);
          ...
          for (var k = 0; k < preprocessing_dependencies.length; k++) {
            ...
            var extra_params = Object.assign({}, paramsCopy, dependency['params']);
