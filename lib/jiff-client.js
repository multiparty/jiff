...
(function(exports, node) {
  ...
  /**
   * Encrypts a message with a symmetric key and adds a tag for validation (using crypto_aead_xchacha20poly1305_ietf_encrypt).
   * The function will execute message.toString(10) internally to ensure type of message is a string before encrypting.
   * @memberof jiff.utils
   * @instance
   * @param {number/String} message - the message to encrypt.
   * @param {Uint8Array/Array/String} symmetric_key - the symmetric key to be used for encryption.
   * @returns {object} - the signed cipher. {cipher:<cipher>,nonce:<nonce>}
   */
  function symmetric_key_encrypt(message, symmetric_key, operation_type) {
    if(operation_type == 'share' || operation_type == 'open') message = message.toString(10);
    var generatedNonce = sodium.randombytes_buf(sodium.crypto_secretbox_NONCEBYTES);

    // parsing the inputs to make sure they're Uint8Array
    try {
      symmetric_key = symmetric_key instanceof Uint8Array ? symmetric_key :
                      symmetric_key instanceof Array ? new Uint8Array(symmetric_key) :
                      new Uint8Array(JSON.parse(symmetric_key))
    } catch(e) {
      console.error('error parsing symmetric key');
      console.error(e);
      return;
    }

    var generatedCiphertext = sodium.crypto_aead_xchacha20poly1305_ietf_encrypt(
      message,
      null,
      null,
      generatedNonce,
    ...
    return {"cipher": '['+generatedCiphertext.toString()+']',"nonce": '['+generatedNonce.toString()+']'};
  }

  /**
   * Decrypts a message with a symmetric key and verifies it (using crypto_aead_xchacha20poly1305_ietf_decrypt).
   * The function will execute parseInt internally to ensure that the returned value is a number.
   * @memberof jiff.utils
   * @instance
   * @param {Uint8Array/Array/String} ciphertext - the ciphertext to decrypt.
   * @param {Uint8Array/Array/String} symmetric_key - the symmetric_key to decrypt with.
   * @param {Uint8Array/Array/String} nonce - the nonce used to decrypt with.
   * @returns {number/string} the decrypted message.
   */
  function symmetric_key_decrypt(ciphertext, symmetric_key, nonce, operation_type) {
    // parsing the inputs to make sure they're Uint8Array
    try {
      ciphertext = ciphertext instanceof Uint8Array ? ciphertext :
                   ciphertext instanceof Array ? new Uint8Array(ciphertext) :
                   new Uint8Array(JSON.parse(ciphertext));
      symmetric_key = symmetric_key instanceof Uint8Array ? symmetric_key :
                      symmetric_key instanceof Array ? new Uint8Array(symmetric_key) :
                      new Uint8Array(JSON.parse(symmetric_key));
      nonce = nonce instanceof Uint8Array ? nonce :
                      nonce instanceof Array ? new Uint8Array(nonce) :
                      new Uint8Array(JSON.parse(nonce));
    } catch(e) {
      console.error('error parsing an input to symmetric_key_decrypt');
      console.error(e);
      return;
    }

    try {
      var decryptedData = sodium.to_string(sodium.crypto_aead_xchacha20poly1305_ietf_decrypt(
        null,
        ciphertext,
        null,
        nonce,
      ...
      decryptedData = (operation_type == 'share' || operation_type == 'open') ? parseInt(decryptedData, 10) : decryptedData;
      return decryptedData;

    } catch(_) {
      throw "Error decrypting symmetric key encrypted data. Might be a bad signature or nonce.";
    }
  }
  ...
  function jiff_share(jiff, secret, threshold, receivers_list, senders_list, Zp, share_id) {
    ...
    if(senders_list.indexOf(jiff.id) > -1) {
      ...
      for(var i = 0; i < receivers_list.length; i++) {
        ...
        var cipher_share
        switch(jiff.encryption_type) {
          case 'none': cipher_share = jiff.hooks.encryptSign(shares[p_id], jiff.keymap[p_id], jiff.secret_key, 'share'); break; // TODO
          case 'asymmetric_keys': cipher_share = jiff.hooks.encryptSign(shares[p_id], jiff.keymap[p_id], jiff.secret_key, 'share'); break;
          case 'symmetric_key': cipher_share = jiff.hooks.symmetricEncrypt(shares[p_id], jiff.symmetric_key_map[p_id], 'share'); break;
        }
        var msg = { party_id: p_id, share: cipher_share, op_id: share_id, encryption_type: jiff.encryption_type };
   ...
   * @param {string} message_encryption_type - the method of encryption used.
   *
   */
  function receive_share(jiff, sender_id, share, op_id, message_encryption_type) {
    // Decrypt share
    switch (message_encryption_type) {
      case 'none': share = jiff.hooks.decryptSign(share, jiff.secret_key, jiff.keymap[sender_id], 'share'); break; // TODO
      case 'asymmetric_keys': share = jiff.hooks.decryptSign(share, jiff.secret_key, jiff.keymap[sender_id], 'share'); break;
      case 'symmetric_key': share = jiff.hooks.symmetricDecrypt(share.cipher, jiff.symmetric_key_map[sender_id], share.nonce, 'share'); break;
    }
  ...
  function jiff_broadcast(jiff, share, parties, op_ids) {
    for(var index = 0; index < parties.length; index++) {
      ...
      var cipher_share
      switch(jiff.encryption_type) {
        case 'none': cipher_share = jiff.hooks.encryptSign(share.value, jiff.keymap[i], jiff.secret_key, 'open'); break; // TODO
        case 'asymmetric_keys': cipher_share = jiff.hooks.encryptSign(share.value, jiff.keymap[i], jiff.secret_key, 'open'); break;
        case 'symmetric_key': cipher_share = jiff.hooks.symmetricEncrypt(share.value, jiff.symmetric_key_map[i], 'open'); break;
      }
      var msg = { party_id: i, share: cipher_share, op_id: op_ids[i], Zp: share.Zp, encryption_type: jiff.encryption_type };
   ...
   * @param {string} message_encryption_type - the method of encryption used.
   */
  function receive_open(jiff, sender_id, share, op_id, Zp, message_encryption_type) {
    // Decrypt share
    if(sender_id != jiff.id)
      switch(message_encryption_type) {
        case 'none': share = jiff.hooks.decryptSign(share, jiff.secret_key, jiff.keymap[sender_id], 'open'); break; //TODO
        case 'asymmetric_keys': share = jiff.hooks.decryptSign(share, jiff.secret_key, jiff.keymap[sender_id], 'open'); break;
        case 'symmetric_key':  share = jiff.hooks.symmetricDecrypt(share.cipher, jiff.symmetric_key_map[sender_id], share.nonce, 'open'); break;
      }
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.ssub = function(o) {
      ...
      // promise to execute ready_sub when both are ready
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    /**
     * They type of encryption to use.
     * @member {string} encryption_type
     * @memberof jiff.jiff-instance
     * @instance
     * @enum {('asymmetric_keys', 'symmetric_key', 'none')} The allowed types of encryption.
     * @default asymmetric_keys
     */
    jiff.encryption_type = (['asymmetric_keys', 'symmetric_key', 'none'].indexOf(options.encryption_type) > -1) ?
                            options.encryption_type : 'asymmetric_keys';
    ...
    var guard_socket = function(socket) {
      ...
      socket.safe_emit = function(label, msg) {
        jiff.log("Sending message.<br/>Label: " + label + "<br/>Message: " + msg);
    ...
    /**
     * A map from party id to symmetric key. Where key is the party id (number),
     * and value is the symmetric key (Uint8Array).
     *
     * { party_id -> [Symmetric Key] }
     */
    jiff.symmetric_key_map = options.symmetric_key_map ? options.symmetric_key_map : {};
    ...
    if(jiff.hooks.symmetricEncrypt == null) jiff.hooks.symmetricEncrypt = symmetric_key_encrypt;
    if(jiff.hooks.symmetricDecrypt == null) jiff.hooks.symmetricDecrypt = symmetric_key_decrypt;
    ...
    jiff.connect = function() {
      ...
      if(options.__internal_socket == null) {
        jiff.socket.on('connect', function() {
          jiff.log("Socket connected! Socket id: " + jiff.socket.id + ". Sending 'computation_id' emit to server.<br/>id: " + computation_id + "<br/>party_id: " + jiff.id + "<br/>party_count: " + jiff.party_count);
          if(jiff.socket_id_display)
            jiff.socket_id_display(jiff.socket.id);
        ...
        jiff.log("Sending 'computation_id' emit to server.<br/>id: " + computation_id + "<br/>party_id: " + jiff.id + "<br/>party_count: " + jiff.party_count);
        jiff.socket.emit("computation_id", JSON.stringify({ "computation_id": computation_id, "party_id": jiff.id, "party_count": jiff.party_count }));
      }
    }
    ...
    jiff.socket.on('init', function(msg) {
      sodium_promise.then(function() {
        jiff.log("Received 'init' emit from server.");
        msg = JSON.parse(msg);
        if(jiff.id == null) {
          jiff.id = msg.party_id;
          jiff.log("Our party_id is now = " + jiff.id);
          if(jiff.id_display)
            jiff.id_display(jiff.id);
        }
        ...
        if(jiff.secret_key == null || jiff.public_key == null) {
          ...
          jiff.log("Keypair generated!<br/>Public key = [" + jiff.public_key.toString() + "]. Private key = [" + jiff.secret_key.toString() + "].")
          if(jiff.public_key_display)
            jiff.public_key_display(jiff.public_key.toString());
          if(jiff.secret_key_display)
            jiff.secret_key_display(jiff.secret_key.toString());
        }

        jiff.log("Sending 'public_key' emit to server.<br/> Public key: [" + jiff.public_key.toString() + "].")
    ...
    jiff.socket.on('public_key', function(msg) {
      sodium_promise.then(function() {
        jiff.log("Public key emit received.");
        ...
        for(var i in jiff.keymap)
          if(jiff.keymap.hasOwnProperty(i)) {
            ...
            if(jiff.public_keys_display)
              jiff.public_keys_display(jiff.keymap);

            // BEGIN generate symmetric keys.
            // Generate symmetric keys for other parties in the computation.
            // Members with lower party id generate the key for the higher.
            if(jiff.encryption_type == 'symmetric_key' && !jiff.symmetric_key_map[i] && i > jiff.id) {
              // generate the key
              jiff.symmetric_key_map[i] = sodium.crypto_aead_xchacha20poly1305_ietf_keygen();
              if(jiff.symmetric_keys_display)
                jiff.symmetric_keys_display(jiff.symmetric_key_map);
              
              jiff.log("Generated symmetric key for (" + i + ") = [" + jiff.symmetric_key_map[i].toString() + "].");
              // encrypt the key with the receiver's public key
              var encryptedSymmetricKey = jiff.hooks.encryptSign(
                '[' + jiff.symmetric_key_map[i] + ']',
                jiff.keymap[i],
                jiff.secret_key,
                'symmetric_key'
              );
              // sending the key as a 'share' emit
              var encryptedSymmetricKeyMessage = { party_id: i, symmetric_key: encryptedSymmetricKey, symmetric_key_msg:true };
              jiff.socket.safe_emit('share', JSON.stringify(encryptedSymmetricKeyMessage));
            }
            // END generate symmetric keys.
          }
        ...
        if(jiff.encryption_type == 'symmetric_key')
          for(var i = 1; i < jiff.party_count; i++) {
            if(i == jiff.id) continue;
            if(jiff.symmetric_key_map[i] == null) return;
          }
        for(var i = 1; i <= jiff.party_count; i++)
    ...
    jiff.logs = [];
    jiff.logs_box = options.logs_box;
    jiff.log = function(log_line) {
      jiff.logs.push(log_line);
      if(jiff.logs_box) jiff.logs_box(log_line);
    }
    jiff.public_keys_display = options.public_keys_display;
    jiff.symmetric_keys_display = options.symmetric_keys_display;
    jiff.id_display = options.id_display;
    jiff.public_key_display = options.public_key_display;
    jiff.secret_key_display = options.secret_key_display;
    jiff.socket_id_display = options.socket_id_display;
    ...
    jiff.socket.on('share', function(msg, callback) {
      ...
      var message_encryption_type = json_msg["encryption_type"];
      jiff.log("Received 'share' emit from " + sender_id + ", share (object): " + share + ", op_id: " + op_id); console.log(share);

      // If it's a symmetric key, add it to the map, else receive_share
      if(json_msg.symmetric_key_msg) {
        var decryptedSymmetricKey = jiff.hooks.decryptSign(
          json_msg.symmetric_key,
          jiff.secret_key,
          jiff.keymap[sender_id],
          'symmetric_key'
        );
        jiff.symmetric_key_map[sender_id] = new Uint8Array(JSON.parse(decryptedSymmetricKey));
        if(jiff.symmetric_keys_display)
          jiff.symmetric_keys_display(jiff.symmetric_key_map);
    ...
    jiff.socket.on('open', function(msg, callback) {
      ...
      var Zp = json_msg["Zp"];
      var message_encryption_type = json_msg["encryption_type"];
      jiff.log("Received 'open' emit from " + sender_id + ", share (object): " + share + ", op_id: " + op_id + ", zp: " + Zp);

      receive_open(jiff, sender_id, share, op_id, Zp, message_encryption_type);
    });
  ...
  exports.utils = { 'encrypt_and_sign': encrypt_and_sign, 'decrypt_and_sign': decrypt_and_sign,
                    'symmetric_key_encrypt': symmetric_key_encrypt, 'symmetric_key_decrypt': symmetric_key_decrypt };
