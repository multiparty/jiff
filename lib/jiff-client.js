  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    var guard_socket = function(socket) {
      ...
      socket.safe_emit = function(label, msg) {
        // set up mailbox per label type
        if(socket.mailbox[label] == null) socket.mailbox[label] = {};
        
        // add message to mailbox
        socket.mailbox[label][msg] = true;
        if(socket.connected)
          // emit the message, if an acknowledgment is received, remove it from mailbox
          socket.emit(label, msg, function(status) {
            if(status) delete socket.mailbox[label][msg];
          });
      };
      ...
      socket.resend_mailbox = function() {
        ...
        for(var label in socket.mailbox) {
          if(!socket.mailbox.hasOwnProperty(label)) continue;

          // Get all messages per label
          for(var msg in socket.mailbox.label) {
            if(!socket.mailbox[label].hasOwnProperty(msg)) continue;
