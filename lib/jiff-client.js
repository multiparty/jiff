...
(function(exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
        ...
        for(var i in jiff.keymap)
          if(jiff.keymap.hasOwnProperty(i)) {
            ...
            if(jiff.encryption_type == 'symmetric_key' && !jiff.symmetric_key_map[i] && i > jiff.id) {
              ...
              var encryptedSymmetricKeyMessage = { party_id: i, symmetric_key: encryptedSymmetricKey};
              jiff.socket.safe_emit('symmetric_key', JSON.stringify(encryptedSymmetricKeyMessage));
            }
    ...
    jiff.socket.on('share', function(msg, callback) {
      ...
      jiff.log("Received 'share' emit from " + sender_id + ", share (object): " + share + ", op_id: " + op_id);

      receive_share(jiff, sender_id, share, op_id, message_encryption_type);
    });

    jiff.socket.on('symmetric_key', function(msg, callback) {
      callback(true);
