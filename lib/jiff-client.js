...
// use fs module to read/write preprocessing to json file 
var fs = require('fs'); 
...
function JIFFClient(hostname, computation_id, options) {
  ...
  // functions to load and store preprocessing table to json file 

  this.writeFilePreprocessing = function() {
    ...
    fs.writeFile('./preprocess.json', stringifyTable, err => {
      ...
      console.log("succesfully wrote preprocessing table to file"); 
      // clear preprocessing table
      // this.preprocessing_table = {}; 
    })
  }
  
  this.readFilePreprocessing = function() {
    try {
        ...
        if (preprocessData.hasOwnProperty(key)) {
          ...
          if (preprocessData[key][0]) {
            ...
            for (var i = 0; i < preprocessData[key].length; i++) {
              preprocessData[key][i] = new this.SecretShare(individualShare[i]['value'], individualShare[i]['holders'], individualShare[i]['threshold'], individualShare['Zp']);
            }
          }
          else {
