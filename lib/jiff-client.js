  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.share_ND_array = function (secrets, skeletons, threshold, receivers_list, senders_list, Zp, share_id) {
      ...
      Promise.all(all_promises).then(function (array) {
        var shares = {};
        //console.log('array', array.length, senders_list, jiff.party_count);
        for (var i = 0; i < senders_list.length; i++) {
          let p_id = senders_list[i];
          shares[p_id] = array[i];
        }
    ...
    jiff.open_ND_array = function (shares, parties, op_id) {
      ...
      jiff.emit(op_id + 'skeleton', parties, JSON.stringify(skeleton));
      ...
      })(shares, parties, op_id).then(function (opened) {
    ...
    jiff.receive_open_ND_array = function (parties, threshold, Zp, op_ids) {
      if (Zp == null) {
        Zp = jiff.Zp;
      }
      ...
      jiff.listen(op_ids + 'skeleton',  function (sender, skeleton) {
        jiff.remove_listener(op_ids + 'skeleton');
        if (typeof(skeleton) === String) {
          skeleton = JSON.parse(skeleton);
        }
        (function __open_ND_array(shares, parties, op_id) {
          if (typeof(shares.length) === 'undefined') {
            return Promise.resolve(jiff.receive_open(parties, threshold, Zp, op_id + ':open'));
          } else if (shares.length === 0) {
            return Promise.resolve([]);
          } else {
            var deferred_array = [];
            ...
            for (var i = 0; i < shares.length; i++) {
              deferred_array.push(jiff.__open_ND_array(shares[i], parties, op_id + ':' + i));
            }

            return deferred_array.promise;
          }
        })(skeleton, parties, op_ids).then(function (opened) {
          final_deferred.resolve(opened);
        });
