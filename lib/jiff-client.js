...
(function (exports, node) {
   ...
   *
   */
  function many_secret_shares(jiff, count, holders, threshold, Zp, id) {
    var deferreds = [];
    var shares = [];
    for (var i = 0; i < count; i++) {
      var deferred = new Deferred();
      shares.push(jiff.secret_share(jiff, false, deferred.promise, undefined, holders, threshold, Zp, 'share:' + i + ':' + id));
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.bit_decomposition = function (op_id) {
      ...
      var many_shares = many_secret_shares(jiff, bitLength, self.holders, self.threshold, self.Zp, 'share:' + op_id);
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.reshare = function (share, threshold, receivers_list, senders_list, Zp, op_id) {
      ...
      if (isReceiver) {
        ...
        result = jiff.secret_share(jiff, false, final_deferred.promise, undefined, receivers_list, threshold, Zp, 'share:' + op_id);
      }
    ...
    jiff.protocols.generate_beaver_bgw = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      ...
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('generate_beaver_bgw', receivers_list);
      }
    ...
    jiff.protocols.bits.generate_random_bit = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      ...
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('generate_random_bit', receivers_list);
      }
      if (Zp == null) {
        Zp = jiff.Zp;
      }
      ...
      var compute_threshold = params['compute_threshold'];
      var max_compute_threshold = Math.floor((compute_list.length+1)/2); // Because BGW
      if (compute_threshold == null) {
        compute_threshold = max_compute_threshold;
      }
      if (compute_threshold > max_compute_threshold) {
        throw new Error('Compute Threshold is too large for BGW: Max: ' + max_compute_threshold + ', Given: ' + compute_threshold);
      }
     ...
     *                               - an optional number compute_threshold parameter, which specifies threshold used
     *                               during the protocol execution. By default, this is the length of the compute_list.
     * @return {[SecretShare]} An array with this party's share of each generated bit
     */
    jiff.protocols.generate_bit_sequence = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      ...
      for (var i = 0; i < params.count; i++) {
        bits.push(jiff.protocols.generate_random_bit(threshold, receivers_list, compute_list, Zp, op_id + i));
    ...
    jiff.protocols.bits.share_bits = function (secret, bit_length, threshold, receivers_list, senders_list, Zp, share_id) {
      ...
      if (share_id == null) {
        share_id = jiff.counters.gen_share_id(receivers_list, senders_list) + ':bits';
      }
     ...
     * @instance
     * @param {SecretShare[]} bits - an array of the secret shares of bits, starting from least to most significant bits.
     * @param {number[]} parties - parties to open (same as jiff.open)
    ...
    jiff.protocols.bits.open_bits = function (bits, parties, op_id) {
      ...
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('bits.open', bits[0].holders);
      }
     ...
     * @memberof jiff-instance.protocols.bits
     * @instance
     * @param {number} [lower_bound=0] - the lower bound (inclusive).
     * @param {number} [upper_bound=jiff_instance.Zp] - the upper bound (exclusive).
     * @param {number} [threshold=receivers_list.length] - the min number of parties needed to reconstruct the secret, defaults to all the receivers.
     * @param {Array} [compute_list=all_parties] - array of party ids that will perform the sampling and receive its results.
     * @param {number} [Zp=jiff_instance.Zp] - the field of the returned share.
     ...
     */
    // TODO: pre-processing count is unknown: have some fallback?
    jiff.protocols.bits.rejection_sampling = function (lower_bound, upper_bound, threshold, compute_list, Zp, op_id) {
      // Defaults
      ...
      // If not a compute party, return null
      if (compute_list.indexOf(jiff.id) === -1) {
        return null;
      }
      ...
      if (op_id == null) {
        op_id = jiff.counters.gen_op_id('sample', compute_list);
      }
      ...
      var many_shares = many_secret_shares(jiff, finalLength, compute_list, threshold, Zp, op_id);
      var deferreds = many_shares.deferreds;
      var result = many_shares.shares;
    ...
    jiff.protocols.bits.cadd = function (bits, constant, op_id) {
      ...
      // initialize results
      var result = many_secret_shares(jiff, Math.max(constant_bits.length, bits.length), bits[0].holders, bits[0].threshold, bits[0].Zp, op_id);
    ...
    jiff.protocols.bits.sadd = function (bits1, bits2, op_id) {
      ...
      // initialize results
      var result = many_secret_shares(jiff, bits1.length, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id);
      var deferreds = result.deferreds;
      result = result.shares;
    ...
    jiff.protocols.bits.ssub = function (bits1, bits2, op_id) {
      ...
      // initialize results
      var result = many_secret_shares(jiff, Math.max(bits1.length, bits2.length), bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id);
      var deferreds = result.deferreds;
      result = result.shares;
    ...
    jiff.protocols.bits.cmult = function (bits, constant, op_id) {
      ...
      var result = many_secret_shares(jiff, bits.length + constant_bits.length, bits[0].holders, bits[0].threshold, bits[0].Zp, op_id);
      var deferreds = result.deferreds;
      result = result.shares;
    ...
    jiff.protocols.bits.smult = function (bits1, bits2, op_id) {
      ...
      var result = many_secret_shares(jiff, bits1.length + bits2.length + offset, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id);
      var deferreds = result.deferreds;
      result = result.shares;
    ...
    jiff.protocols.bits.sdiv = function (bits1, bits2, op_id) {
      ...
      // Initialize the result
      var quotient = many_secret_shares(jiff, bits1.length, bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id + ':quotient');
      var quotientDeferreds = quotient.deferreds;
      quotient = quotient.shares;

      var remainder = many_secret_shares(jiff, Math.min(bits1.length, bits2.length), bits1[0].holders, Math.max(bits1[0].threshold, bits2[0].threshold), bits1[0].Zp, op_id + ':remainder');
    ...
    jiff.protocols.bits.cdivl = function (bits, constant, op_id) {
      ...
      // Initialize the result
      var quotient = many_secret_shares(jiff, bits.length, bits[0].holders, bits[0].threshold, bits[0].Zp, op_id + ':quotient');
      var quotientDeferreds = quotient.deferreds;
      quotient = quotient.shares;
      ...
      var remainder = many_secret_shares(jiff, Math.min(constantLessBits, bits.length), bits[0].holders, bits[0].threshold, bits[0].Zp, op_id + ':remainder');
      var remainderDeferreds = remainder.deferreds;
      remainder = remainder.shares;
    ...
    jiff.protocols.bits.cdivr = function (constant, bits, op_id) {
      ...
      // Initialize the result
      var quotient = many_secret_shares(jiff, constant_bits.length, bits[0].holders, bits[0].threshold, bits[0].Zp, op_id + ':quotient');
      var quotientDeferreds = quotient.deferreds;
      quotient = quotient.shares;

      var remainder = many_secret_shares(jiff, Math.min(constant_bits.length, bits.length), bits[0].holders, bits[0].threshold, bits[0].Zp, op_id + ':remainder');
      var remainderDeferreds = remainder.deferreds;
      remainder = remainder.shares;
    ...
    jiff.preprocessing = function (dependent_op, count, batch, protocols, threshold, receivers_list, compute_list, Zp, id_list, params) {
      ...
      jiff.__preprocessingDeferred.then(function () {
    ...
    jiff.initialize_counters = function () {
      jiff.counters.share_op_count = {};
      jiff.counters.open_op_count = {};
      jiff.counters.triplet_op_count = {};
      jiff.counters.number_op_count = {};
