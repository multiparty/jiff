  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.smult = function (o, op_id) {
      ...
      if (self.jiff.preprocessing_table.length === 0) {
        ...
        triplet = self.jiff.preprocessing_table['preprocess' + op_id];
      }
    ...
    self.lt_halfprime = function (op_id) {
      ...
      var bits = self.jiff.server_generate_and_share({
        bit: true,
        count: bitLength
      }, share.holders, share.threshold, share.Zp, op_id + ':number:' + i);
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.preprocessing = function (label, op, protocol, count, id_list) {
      ...
      for (i = 0; i < count; i++) {
        ...
        jiff.preprocessing_table[id] = protocol(null, null, null, null, null, 'preprocess' + i);
        return true;
      }
