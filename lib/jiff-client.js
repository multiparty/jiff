  ...
  */
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.smult = function (o, op_id) {
      ...
      var triplet = self.jiff.get_preprocessing('triplet', op_id + ':triplet');
      if (triplet == null) {
     ...
     *                         is generated by increasing a local counter, default ids suffice when all parties execute the
     *                         instructions in the same order.
     * @param {boolean} [reshare=true] - flags whether to reshare after multiplication, to reduce the threshold back to the input threshold.
     * @return {SecretShare} this party's share of the result.
     * @memberof SecretShare
     * @instance
     */
    self.smult_bgw = function (o, op_id, reshare) {
      if (!(o.jiff === self.jiff)) {
      ...
      var final_deferred = $.Deferred();
      var final_promise = final_deferred.promise();
      var result = self.jiff.secret_share(self.jiff, false, final_promise, undefined, self.holders, self.threshold + o.threshold - 1, self.Zp, 'share:' + op_id);
    ...
    self.sdiv = function (o, l, op_id) {
      ...
      (function one_bit(i) {
        ...
        var tmpFits = o.iclteq(ZpOVERpower, op_id + ':clteq' + i);
        var tmpCmp = tmp.islteq(a, op_id + ':slteq' + i);
    ...
    self.lt_halfprime = function (op_id) {
      ...
      var bits = jiff.get_preprocessing('bitsequence', op_id + ':bitsequence');
      // var bits = self.jiff.server_generate_and_share({bit: true, count: bitLength}, share.holders, share.threshold, share.Zp, op_id + ':number:bits');
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.reshare = function (share, threshold, receivers_list, senders_list, Zp, op_id) {
      ...
      // Check if nothing needs to be done, before doing things..
      if (isSender && jiff.helpers.Zp_equals(share, {Zp: Zp}) && jiff.helpers.array_equals(receivers_list, share.holders) && threshold === share.threshold) {
        return share;
      }
    ...
    if (options.crypto_provider != null) {
    ...
    jiff.preprocessing_function_map = {
      base: {
        ...
        clt_bits: [
          { op: 'sor_bit', op_id: ':sOR:', count: clt_bits_count }
        ],
        cdiv: [
     ...
     */
    jiff.get_preprocessing = function (value_type, op_id) {
      var values = jiff.preprocessing_table[op_id];
      if (values == null) {
        if (!jiff.crypto_provider) {
          throw new Error('No preprocessed value(s) that correspond to the op_id "' + op_id + '"');
        } else {
          var provided_protocols = {
            'triplet': jiff.triplet,
            'bitsequence': jiff.server_generate_and_share,
            };
          return provided_protocols[value_type]()
     ...
     * @instance
     */
    jiff.__preprocessingDeferred = $.Deferred();
