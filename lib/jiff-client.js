      ...
      for (var i = 0; i < count; i++) {
        ...
        if (preprocessing_dependencies == null) {
          ...
          for (var k = 0; k < preprocessing_dependencies.length; k++) {
            ...
            var next_id = id + dependency['op_id']; // compose ids similar to how the actual operation is implemented
            jiff.preprocessing(next_op, 1, protocols, threshold, receivers_list, compute_list, Zp, [ next_id ], extra_params);
          }
        }
      }
      return Promise.all(promises);
