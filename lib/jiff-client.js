...
(function(exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.sxor_bit = function(o, op_id) {
      ...
      return self.sadd(o).ssub(self.smult(o, op_id).cmult(2));
    };

    /**
     * OR of two secret shares OF BITS.
     * @method sor_bit
     * @param {share-object} o - the share to OR with.
     * @param {string} op_id - the operation id which is used to identify this operation.
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
     *                         ensure that corresponding instructions accross different parties are matched correctly [optional].
     * @return {share-object} this party's share of the result, the final result is 1 if this < o, and 0 otherwise.
    ...
    self.cgteq = function(cst, op_id) {
      ...
      var share_cst = self.jiff.coerce_to_share(cst, self.holders, self.Zp, op_id + ":cst_share");
      return self.clt(share_cst, op_id).not();
    }
    ...
    self.cgt = function(cst, op_id) {
      ...
      var share_cst = self.jiff.coerce_to_share(cst, self.holders, self.Zp, op_id + ":cst_share");
      return share_cst.clt(self, op_id);
    };
    ...
    self.clteq = function(cst, op_id) {
      ...
      var share_cst = self.jiff.coerce_to_share(cst, self.holders, self.Zp, op_id + ":cst_share");
      return share_cst.clt(self, op_id).not();
    };
    ...
    self.clt = function(cst, op_id) {
      ...
      var share_cst = self.jiff.coerce_to_share(cst, self.holders, self.Zp, op_id + ":cst_share");
      return self.slt(share_cst, op_id);
    };
    ...
    self.seq = function(o, op_id) {
      ...
      var sgteq1_id = op_id+":<=";
      var sgteq2_id = op_id+":>=";
      var smult_id = op_id+":smult";

      var one_direction = self.sgteq(o, sgteq1_id);
      var other_direction = o.sgteq(self, sgteq2_id);
    ...
    self.ceq = function(cst, op_id) {
      ...
      var share_cst = self.jiff.coerce_to_share(cst, self.holders, self.Zp, op_id + ":cst_share");
      return self.seq(share_cst, op_id);
    }
    ...
    self.cneq = function(cst, op_id) {
      if (!(self.isConstant(cst))) throw "parameter should be a number (!=)";
      return self.ceq(share_cst, op_id).not();
    }
    ...
    self.sdiv = function(o, l, op_id) {
      ...
      if(l == null) l = Math.ceil(self.jiff.helpers.bLog(self.Zp));

      var q_shr = self.jiff.server_generate_and_share({"number": 0}, self.holders, self.threshold, self.Zp, op_id+":number");
      for (var i = 0; i < l; i++) {
        var power = Math.pow(2, (l-1)-i);

        var mask = q_shr.cadd(power);
        var tmp_m = mask.smult(o, op_id+":smult"+i).slteq(self, op_id+":slteq"+i);
    ...
    self.lt_halfprime = function(op_id) {
      ...
      var share = self.cmult(2);

      // To check if share is even, we will use pre-shared bits as some form of a bit mask
      var bitLength = Math.floor(self.jiff.helpers.bLog(share.Zp, 2)); // TODO: this leaks one bit, fix it for mod 2^n
      ...
      for(var i = 1; i < bitLength; i++) {
        pow = pow * 2;
        r = bits[i].cmult(pow).sadd(r);
      }

      function clt_bits(constant, secretBits) {
        // Decompose result into bits
        constant = constant.toString(2);

        var constantBits = [];
        for(var i = 0; i < constant.length; i++)
          constantBits[i] = parseInt(constant.charAt(constant.length - 1 - i));
        while(constantBits.length < secretBits.length) constantBits.push(0);

        // XOR
        var c = [];
        for(var i = 0; i < constantBits.length; i++)
          c[i] = secretBits[i].cxor_bit(constantBits[i]);
        ...
        var isNotEqual = e[0];
        var isGreaterThan = e[0].cmult(constantBits[0]);
        for(var i = 1; i < e.length; i++) {
          isGreaterThan = isGreaterThan.sadd(e[i].cmult(constantBits[i]));
          isNotEqual = isNotEqual.sadd(e[i]);
        }

        return isNotEqual.not().sadd(isGreaterThan).not();
      }

      share.jiff.open(r.sadd(share), share.holders).then(function(result) {
        var wrapped = clt_bits(result, bits);
