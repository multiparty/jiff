...
(function (exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    var guard_socket = function (socket) {
      ...
      socket.safe_disconnect = function () {
        if (socket.mailbox.head == null) {
          socket.disconnect();
        } else {
          socket.empty_deferred = $.Deferred();
          socket.empty_deferred.promise().then(function () {
     ...
     * @memberof jiff-instance
     * @instance
     * @param free {boolean} - if set to true, it means this party's disconnection is final, and all resources
     ...
     */
    jiff.disconnect = function (free) {
      if (free === true) {
        jiff.socket.safe_emit('free', '');
      }

      jiff.socket.safe_disconnect();
