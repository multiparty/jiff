...
(function (exports, node) {
    ...
    if (lengths == null) {
      ...
      for (i = 0; i < senders_list.length; i++) {
        if (lengths[senders_list[i]] == null || lengths[senders_list[i]].rows == null) {
          throw 'share_2D_array: missing rows length'
        }
      }
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
     ...
     * @param {function(string)} [failure=SecretShare.error] - the function to handle errors and error messages.
     ...
     * @method open_to
     * @param {Array} [parties=all_parties] - the ids of parties to reveal secret to.
     * @param {function(number)} success - the function to handle successful open.
     * @param {function(string)} [failure=SecretShare.error] - the function to handle errors and error messages.
     * @memberof SecretShare
     */
    ...
    self.smult = function (o, op_id) {
      ...
      var triplet;
      //TODO check triplet table length not length of whole preprocessing table
     ...
     * @param {number} l - the maximum bit length of the answer.
     * @param {string} [op_id=auto_gen()] - the operation id which is used to identify this operation.
     *                         This id must be unique, and must be passed by all parties to the same instruction, to
     *                         ensure that corresponding instructions across different parties are matched correctly.
     * @return {SecretShare} this party's share of the result.
    ...
    self.cdiv = function (cst, op_id) {
      ...
      if (self.jiff.preprocessing_table.rand_nums['preprocess' + op_id] == null) {
        ...
        nMODc = self.jiff.server_generate_and_share({max: ZpOVERc}, self.holders, self.threshold, self.Zp, op_id + ':nOVERc')[0];
      } else {
    ...
    self.smod= function (o, l, op_id) {
      ...
      var r = self.sdiv(o, l, op_id + ':sdiv');
      return self.ssub(r.smult(o, op_id + ':smult'));
    };
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.resolve_messages_waiting_for_keys = function () {
      for (var party_id in jiff.keymmap) {
        if (!jiff.keymmap.hasOwnProperty(party_id)) {
          continue;
        }
    ...
    jiff.emit = function (tag, receivers, message, encrypt) {
      if (typeof(message) !== 'string') {
        throw 'Emit: message must be a string';
      }
    ...
    jiff.protocols.generate_beaver_bgw = function (compute_threshold, compute_list, receiving_threshold, receivers_list, Zp, op_id) {
      ...
      if (compute_threshold > (compute_list.length + 1) / 2) {
        throw 'generate_beaver_bgw: compute threshold too high for bgw';
      }
     ...
     * @param {string} //TODO documentation
     * @param {string}
     * @param {Function}
     * @param {Number}
     * @param {Array}
