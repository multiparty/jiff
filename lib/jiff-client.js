 ...
 *                           all parameters are optional, However, private and public key must either be both provided or neither of them provided.
 <pre>
 {
   ...
   "crypto_provider": a boolean that flags whether to get beaver triplets and other preprocessing entities from the server (defaults to false),
                      alternatively, could be an address (e.g. ip:port) for a third party crypto_provider.
...
function JIFFClient(hostname, computation_id, options) {
   ...
   * @type {?boolean|string}
   */
  this.crypto_provider = false;
  this.external_crypto_provider = false;
  if (options.crypto_provider === true) {
    this.crypto_provider = true;
  } else if (options.crypto_provider !== false && options.crypto_provider != null) {
    this.crypto_provider = true;
    this.external_crypto_provider = true;
  }
  ...
  this.socketConnect = function (JIFFClientInstance) {
    if (options.__internal_socket == null) {
       ...
       * @type {!GuardedSocket}
       */
      JIFFClientInstance.socket = guardedSocket(JIFFClientInstance, JIFFClientInstance.hostname);
    } else {
      JIFFClientInstance.socket = internalSocket(JIFFClientInstance, options.__internal_socket);
    }

    // Crypto provider may use a different socket.
    if (JIFFClientInstance.crypto_provider) {
      JIFFClientInstance.crypto_provider_socket = JIFFClientInstance.socket;
    }
    if (JIFFClientInstance.external_crypto_provider) {
      JIFFClientInstance.crypto_provider_socket = guardedSocket(JIFFClientInstance, options.crypto_provider);
    }
    ...
    if (JIFFClientInstance.external_crypto_provider) {
      JIFFClientInstance.crypto_provider_socket.connect();
    }
  }
