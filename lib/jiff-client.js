...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp) {
    ...
    self.bit_decomposition = function (op_id) {
      ...
      if (bits == null) {
        bits = self.jiff.protocols.bits.__rejection_sampling(0, self.Zp, self.threshold, self.holders, self.Zp, op_id + ':sampling');
      }
      var r = self.jiff.protocols.bits.bit_composition(bits);
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.bits.__rejection_sampling = function (lower_bound, upper_bound, threshold, compute_list, Zp, op_id, params, protocols) {
      ...
      upper_bound = upper_bound != null ? upper_bound : Zp;
      ...
      (function resample(reject_count) {
        var bits = protocols.bits(threshold, compute_list, compute_list, Zp, op_id + ':sampling:' + reject_count, {count: bitLength}).share;
        ...
        if (lower_bound.toString() !== '0') {
          var promise2 = jiff.__preprocessing('bits.cadd', 1, protocols, threshold, compute_list, compute_list, Zp, [ op_id + ':bits.cadd:' + reject_count ], params);
    ...
    var constant_bits_count = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
    ...
    var dynamic_bits_cmult = function (dependent_op, count, protocols, threshold, receivers_list, compute_list, Zp, id_list, params) {
      ...
      for (var i = 0; i < constantBits; i++) {
        var accLength = i === 0 ? 1 : (bitLength + i + 1);
