...
(function (exports, node) {
   ...
   */
  function encrypt_and_sign(jiff, message, encryption_public_key, signing_private_key, operation_type) {
   ...
   */
  function decrypt_and_sign(jiff, cipher_text, decryption_secret_key, signing_public_key, operation_type) {
  ...
  function jiff_share(jiff, secret, threshold, receivers_list, senders_list, Zp, share_id) {
    ...
    if (senders_list.indexOf(jiff.id) > -1) {
      ...
      for (i = 0; i < receivers_list.length; i++) {
        ...
        msg['share'] = jiff.hooks.encryptSign(jiff, msg['share'], jiff.keymap[msg['party_id']], jiff.secret_key, 'share');
  ...
  function receive_share(jiff, json_msg) {
    // Decrypt share
    json_msg['share'] = jiff.hooks.decryptSign(jiff, json_msg['share'], jiff.secret_key, jiff.keymap[json_msg['party_id']], 'share');
  ...
  function jiff_broadcast(jiff, share, parties, op_ids) {
    for (var index = 0; index < parties.length; index++) {
      ...
      msg['share'] = jiff.hooks.encryptSign(jiff, msg['share'], jiff.keymap[msg['party_id']], jiff.secret_key, 'open');
  ...
  function receive_open(jiff, json_msg) {
    ...
    if (json_msg['party_id'] !== jiff.id) {
      json_msg['share'] = jiff.hooks.decryptSign(jiff, json_msg['share'], jiff.secret_key, jiff.keymap[json_msg['party_id']], 'open');
  ...
  function jiff_triplet(jiff, receivers_list, threshold, Zp, triplet_id) {
    ...
    if (jiff.id === 's1') {
      ...
      jiff.triplets_socket.safe_emit('triplet', jiff.hooks.encryptSign(jiff, msg, jiff.keymap['s1'], jiff.secret_key, 'triplet'));
    }
  ...
  function jiff_server_share_number(jiff, options, receivers_list, threshold, Zp, number_id) {
    ...
    if (jiff.id === 's1') {
      ...
      jiff.numbers_socket.safe_emit('number', jiff.hooks.encryptSign(jiff, msg, jiff.keymap['s1'], jiff.secret_key, 'number'));
    }
  ...
  function receive_custom(jiff, json_msg) {
    if (json_msg['encrypted'] === true) {
      json_msg['message'] = jiff.hooks.decryptSign(jiff, json_msg['message'], jiff.secret_key, jiff.keymap[json_msg['party_id']], 'custom');
    }
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.emit = function (tag, receivers, message, encrypt) {
      ...
      for (var p = 0; p < receivers.length; p++) {
        ...
        if (message_to_send['encrypted'] !== false) {
          message_to_send['message'] = jiff.hooks.encryptSign(jiff, message_to_send['message'], jiff.keymap[message_to_send['party_id']], jiff.secret_key, 'custom');
    ...
    jiff.free = function (socket) {
      ...
      var msg = {};
      if (socket === jiff.socket) {
        msg = jiff.execute_array_hooks('beforeOperation', [jiff, 'free', msg], 2);
      }
    ...
    jiff.socket.on('init', function (msg) {
      ...
      msg = { public_key: jiff.hooks.dumpKey(jiff, jiff.public_key) };
      msg = jiff.execute_array_hooks('beforeOperation', [jiff, 'public_key', msg], 2);
    ...
    jiff.socket.on('public_key', function (msg) {
      msg = JSON.parse(msg);
      msg = jiff.execute_array_hooks('afterOperation', [jiff, 'public_key', msg], 2);
      ...
      for (i in msg) {
        if (msg.hasOwnProperty(i) && jiff.keymap[i] == null) {
          jiff.keymap[i] = jiff.hooks.parseKey(jiff, msg[i]);
        }
      }
    ...
    jiff.triplets_socket.on('triplet', function (msg, callback) {
      ...
      if (jiff.id !== 's1' || (options.triplets_server != null && options.triplets_server !== hostname)) {
        ...
        msg = jiff.hooks.decryptSign(jiff, msg, jiff.secret_key, jiff.keymap['s1'], 'triplet');
        msg = JSON.parse(msg);
      }
    ...
    jiff.numbers_socket.on('number', function (msg, callback) {
      ...
      if (jiff.id !== 's1' || (options.numbers_server != null && options.numbers_server !== hostname)) {
        // decrypt and verify message signature
        msg = jiff.hooks.decryptSign(jiff, msg, jiff.secret_key, jiff.keymap['s1'], 'number');
        msg = JSON.parse(msg);
      }
