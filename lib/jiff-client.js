...
(function (exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * @method beaver_bgw
     * @param {jiff_instance} jiff - the jiff instance.
     * @param {number} x - the party's x value, likely sampled uniformly, will be built into a collective, secret a
     * @param {number} y - the party's y value, likely sampled uniformly, will be built into a collective, secret b
     ...
     *                              is generated by increasing a local counter, default ids suffice when all parties execute the
     *                              instructions in the same order. [optional]
     * @return {share-object[]} array of this party's share of the resulting triplet, a,b,c such that a*b=c.
     * @memberof jiff.protocols
     */
    jiff.protocols.beaver_bgw = function (jiff_instance, x, y, receivers_list, threshold, Zp, triplet_id) {
      /**
       * 1. share x, y
      ...
      if (triplet_id == null) {
        triplet_id = jiff_instance.counters.gen_triplet_id(receivers_list);
      }

      var final_deferred = $.Deferred();
      var final_promise = final_deferred.promise();
      var result = jiff_instance.secret_share(jiff, false, final_promise, undefined, receivers_list, threshold, Zp, 'share:'+triplet_id);

      var x_shares = jiff_instance.internal_share(x, threshold, receivers_list, receivers_list, Zp, null);
      var y_shares = jiff_instance.internal_share(y, threshold, receivers_list, receivers_list, Zp, null);

      var x_sum = x_shares[1];
      var y_sum = y_shares[1];

      var promises = [];
      for (var i = 0; i < x_shares.length; i++) {
        promises.push(x_shares[i].promise);
        promises.push(y_shares[i].promise);
      }
      ...
      Promise.all(promises).then( function (results) {
        ...
        // mult a*b=c
        var c;
        Promise.all([x_sum, y_sum]).then( function (results) {
          try{
            c = x_sum.smult_bgw(y_sum);
          }catch(err) {
            console.log(err);
          }
          Promise.all([c]).then( function(results) {
            var triplet = [x_sum, y_sum, c];
            //console.log(triplet);
            final_deferred.resolve(triplet);
          },
            function(err) {
              console.log('promises failed to resolve', err);
            });
        },
          function (err) {
            console.log(err);
