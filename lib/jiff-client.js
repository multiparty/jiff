...
(function(exports, node) {
  ...
  /** Check two sorted arrays are equal by value */
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp) {
    ...
    this.sadd = function(o) {
      ...
      if (!array_equals(self.holders, o.holders)) throw "shares must be held by the same parties (+)";
    ...
    this.sqrt = function() {
      ...
      var temp = 1;
      var a;
      a = self.clteq(temp*temp);
      temp = a.cmult(temp) + ((a.cmult(-1)).cadd(1)).cmult(temp+1);

      for (var i = 0; i < 4; i++)
      {
        a = self.slteq(temp.mult(temp));
        temp = a.mult(temp) + ((a.cmult(-1)).cadd(1)).mult(temp.cadd(1));
      }
      return temp;
    }
