...
(function (exports, node) {
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp) {
    ...
    self.lt_halfprime = function (op_id) {
      ...
      // Retrieve pre-processed random bit sequence
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * @param {object} [protocols=defaults] - the protocols to use for preprocessing.
     * @param {boolean} [preprocess=true] - if false, the protocol is run assuming that either preprocessing for it is already done, or will fallback to the crypto provider.
     *                                   if true, the protocol executes pre-processing on demand whenever a rejection happens, and another iteration is needed.
     ...
     */
    jiff.protocols.bits.rejection_sampling = function (threshold, receivers_list, compute_list, Zp, op_id, params, protocols, preprocess) {
      // Defaults
      if (compute_list == null) {
        compute_list = [];
        for (var p = 1; p <= jiff.party_count; p++) {
      ...
      preprocess = preprocess != null ? preprocess : true;
      ...
      var promises = [];
      if (compute_list.indexOf(jiff.id) > -1) {
        result = jiff.protocols.bits.__rejection_sampling(lower_bound, upper_bound, params.threshold, compute_list, Zp, op_id, params, protocols, preprocess);
     ...
     * @private
     */
    jiff.protocols.bits.__rejection_sampling = function (lower_bound, upper_bound, threshold, compute_list, Zp, op_id, params, protocols, preprocess) {
      ...
      // honest majority BGW
      threshold = threshold != null ? threshold : Math.floor((compute_list.length + 1) / 2);
      Zp = Zp != null ? Zp : jiff.Zp;
      lower_bound = lower_bound != null ? lower_bound : 0;
      upper_bound = upper_bound != null ? upper_bound : Zp;
      preprocess = preprocess != null ? preprocess : false;
      ...
      (function resample(reject_count) {
        ...
        var bits = protocols.bits(threshold, compute_list, compute_list, Zp, op_id + ':sampling:' + reject_count, paramsCopy).share;
        ...
        if (preprocess === true) {
          ...
          var promise = jiff.__preprocessing('bits.clt', 1, protocols, threshold, compute_list, compute_list, Zp, [op_id + ':bits.clt:' + reject_count], paramsCopy);
          if (lower_bound.toString() !== '0') {
            var promise2 = jiff.__preprocessing('bits.cadd', 1, protocols, threshold, compute_list, compute_list, Zp, [op_id + ':bits.cadd:' + reject_count], paramsCopy);
            promise = Promise.all([promise, promise2]);
          }
          promise.then(online_resample);
        } else {
          online_resample();
        }
      })(0);
