...
(function (exports, node) {
  ...
  function resolve_many_secrets(deferreds, shares) {
    for (var i = 0; i < deferreds.length; i++) {
      if (shares[i].ready) {
        deferreds[i].resolve(shares[i].value);
        continue;
      }

      shares[i].promise.then(function (i) {
        deferreds[i].resolve(shares[i].value);
      }.bind(null, i));
    }
  }
   ...
   */
  function bit_combinator(deferred, length, func, promisify, valufy, initial) {
    if (promisify == null) {
    ...
    (function __bit_combinator(start, val) {
      if (start === length) {
        // done
        return deferred.resolve(valufy(val));
      }
      ...
      promisify(val).then(__bit_combinator.bind(null, start+1, val));
    })(0, initial);
  }
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.bits.cneq = function (bits, constant, op_id) {
      ...
      bit_combinator(deferred, bits.length, function (i, prev) {
        var next = bits[i].icxor_bit(constant_bits[i]);
        if (i > 0) {
          next = prev.isor_bit(next, op_id + ':sor_bit:' + (i - 1));
        }
        return next;
      });
    ...
    jiff.protocols.bits.rejection_sampling = function (lower_bound, upper_bound, threshold, compute_list, Zp, op_id) {
      ...
      (function resample(reject_count) {
        ...
        if (jiff.helpers.bLog(range, 2).toString().indexOf('.') === -1) {
          ...
          resolve_many_secrets(deferreds, bits_add);
          return;
        }
