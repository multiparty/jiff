  ...
  */
  ...
  function secret_share(jiff, ready, promise, value, holders, threshold, Zp, id) {
    ...
    self.smult_bgw = function (o, op_id) {
      ...
      return result.reshare(max(self.threshold, null, o.threshold), self.holders, op_id + ':threshold');
    };
    ...
    self.sdiv = function (o, l, op_id) {
      ...
      var q;
      if (self.jiff.preprocessing_table.rand_nums['preprocess' + op_id] == null) {

        q = self.jiff.server_generate_and_share({number: 0}, self.holders, max(self.threshold, o.threshold), self.Zp, op_id + ':number')[0];
      } else {
        q = self.jiff.preprocessing_table.rand_nums['preprocess' + op_id];
      }
      var a = self; // dividend
    ...
    self.cdiv = function (cst, op_id) {
      ...
      if (self.jiff.preprocessing_table.rand_nums['preprocess' + op_id] == null) {
        ...
        nOVERc = self.jiff.server_generate_and_share({max: ZpOVERc}, self.holders, self.threshold, self.Zp, op_id + ':nOVERc')[0];
      } else {
        nOVERc = self.jiff.preprocessing_table.rand_nums['preprocess' + op_id];
      }
      if (self.jiff.preprocessing_table.rand_nums['preprocess' + op_id] == null) {

        nMODc = self.jiff.server_generate_and_share({max: cst}, self.holders, self.threshold, self.Zp, op_id + ':nMODc')[0];
      } else {
        nMODc = self.jiff.preprocessing_table.rand_nums['preprocess' + op_id];
      }
    ...
    self.lt_halfprime = function (op_id) {
      ...
      var bits;
      if (self.jiff.preprocessing_table.bit_sequences['preprocess' + op_id] == null) {
        bits = self.jiff.server_generate_and_share({
          bit: true,
          count: bitLength
        },
        share.holders, share.threshold, share.Zp, op_id + ':number');
        bits[bitLength] = self.jiff.server_generate_and_share({number: 0}, share.holders, share.threshold, share.Zp, op_id + ':number:' + bitLength)[0]; // remove this line when fixing TODO because we need most significant bit to be 0

      } else {

        bits = self.jiff.preprocessing_table.bit_sequences['preprocess' + op_id];
      }
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * @param {secret_share} share - the share you would like to reshare (null if you are a receiver but not a sender).
     * @param {number} [threshold=null] - the new threshold, defaults to the length of receivers_list param
    ...
    jiff.protocols.reshare = function (share, threshold, receivers_list, senders_list, Zp, op_id) {
      ...
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('reshare:', senders_list);
      }
      ...
      if (isReceiver) {
        ...
        result = self.jiff.secret_share(self.jiff, false, final_deferred.promise(), undefined, self.holders, threshold, self.Zp, 'share:' + op_id);
      }

      // This function is called when the share is ready: the value of the share has been received.
      var ready_share = function () {
        var intermediate_shares = self.jiff.internal_share(isSender ? share.value : null, threshold, receivers_list, senders_list, Zp, op_id);
    ...
    jiff.protocols.generate_beaver_bgw = function (threshold, receivers_list, compute_list, Zp, op_id, params) {
      ...
      return { op_id: op_id + ':triplet', share: [a, b, c] };
    };
    ...
    jiff.get_preprocessing = function (op_id) {
      var share = jiff.preprocessing_table[op_id];
      if (share != null) {
        return share;
      }
     ...
     * @param {Function} protocol - function within jiff.protocols thatgenerates the specified values (e.g. jiff.protocols.generate_beaver_bgw).
     ...
     */
    jiff.preprocessing = function (op, protocol, count, id_list) {
      var receivers_list = [];
      for (var i = 1; i <= jiff.party_count; i++) {
        receivers_list.push(i);
      }
      ...
      for (i = 0; i < count; i++) {
        ...
        var result = protocol(null, receivers_list, null, null, id, null);
        jiff.store_preprocessing(result.op_id, result.share);
        //should I reset label counters here? or in a separate function?
        // TODO labels have to be reset after all preprocessing operations, as this function is called many times
        // really this function should take an array of operations and an array of values corresponding to the number of times those protocols need to be executed, then labels can be reset here
      }
      // return details of the computation / what we generated in case you want to keep track of ids or whatever? you could either save the return value if you cared or just execute and ignore the return
    };
