...
(function(exports, node) {
   ...
   * @param {null/number/object} length - the option for the length
   ...
   * 
   * @param {number} threshold - for jiff.share(): the minimimum number of parties needed to reconstruct the secret, defaults to all the recievers. [optional]
   * @param {array} receivers_list - for jiff.share(): array of party ids to share with, by default, this includes all parties. [optional]
   * @param {array} senders_list - for jiff.share(): array of party ids to receive from, by default, this includes all parties. [optional]
   * @param {number} Zp - for jiff.share(): the modulos (if null then the default Zp for the instance is used). [optional]
   * @param {string/number} share_id - for jiff.share(): the tag used to tag the messages sent by this share operation, this tag is used
   *                                   so that parties distinguish messages belonging to this share operation from other
   *                                   share operations between the same parties (when the order of execution is not
   *                                   deterministic). An automatic id is generated by increasing a local counter, default
   *                                   ids suffice when all parties execute all sharing operations with the same senders
   *                                   and receivers in the same order. [optional]
   * @returns {promise} the promise of the array of shares.
   */
  var jiff_share_array_parameters;
  function jiff_share_array(jiff, array, length, callback, padding, /*parameters for the 
    jiff.share function:*/ threshold, receivers_list, senders_list, Zp, share_id,
    share_array_populated_lengths) {
    var share_array_deferred = $.Deferred();
    var share_array_promise = share_array_deferred.promise();
    share_array_promise.then = callback;

    // the case where length is null
    if(!length) {
      jiff_share_array_parameters = {
        array:array,
        share_array_promise:share_array_promise,
        threshold:threshold,
      ...
      jiff.emit("array-length", null, array.length);
    }
    else if(typeof length === "number") {
      var json_array_lengths = {};
      for(var i = 1; i <= jiff.party_count; i++)
        json_array_lengths[i] = length;
      share_array_populated_lengths(jiff, array, json_array_lengths, share_array_promise,  threshold, receivers_list, senders_list, Zp, share_id);
    } else if(typeof length === "object")
      share_array_populated_lengths(jiff, array, length, share_array_promise,  threshold, receivers_list, senders_list, Zp, share_id);

    return share_array_promise;
  }
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     * @returns {promise} the promise of the array of shares.
     */
    jiff.share_array = function(array, length, callback, padding, threshold, receivers_list, senders_list, Zp, share_id) { return jiff_share_array(jiff, array, length, callback, padding, threshold, receivers_list, senders_list, Zp, share_id, share_array_populated_lengths); }

    var parties_array_lengths;
    jiff.listen("array-length", function(sender, length) {
      if(!parties_array_lengths)
        parties_array_lengths = {};
      parties_array_lengths[sender] = length;

      if(Object.keys(parties_array_lengths).length == jiff.party_count) {
        share_array_populated_lengths(
          jiff,
          jiff_share_array_parameters.array,
          parties_array_lengths,
          jiff_share_array_parameters.share_array_promise,
          jiff_share_array_parameters.threshold,
          jiff_share_array_parameters.receivers_list,
          jiff_share_array_parameters.senders_list,
          jiff_share_array_parameters.Zp,
          jiff_share_array_parameters.share_id
        );
        parties_array_lengths = null;
      }
    });

    /**
     * The next step for share_array after the lengths have been populated from anywhere
     */
    function share_array_populated_lengths(jiff, array, json_array_lengths, share_array_promise,  threshold, receivers_list, senders_list, Zp, share_id) {
      // compute the max
      var max = 0;
      for(var party in json_array_lengths) {
        max = max > json_array_lengths[party] ? max : json_array_lengths[party];
      }

      // share the array of this party
      var shares = [];
      for(var i = 0; i < array.length; i++) {
        shares.push(jiff.share(array[i]));
      }
      // array of receivers
      var parties = [];
      for(var i = 1; i <= jiff.party_count; i++)
        parties.push(i);
      // .share(null) if the length of the provided array is less than the max
      for(var i = 0; i < max - array.length; i++) {
        shares.push(jiff.share(null, null, parties));
      }
      
      // put the shares in the standard format
      var arrays_of_shares = {};
      for(var i = 1; i <= jiff.party_count; i++)
        arrays_of_shares[i] = [];
      for(var i = 0; i < max; i++) {
        for(var party in shares[i]) {
          arrays_of_shares[party].push(shares[i][party]);
        }
      }

      // for now we can return both formats for backward compatibility
      // share_array_promise.then(arrays_of_shares, shares);
      share_array_promise.then(shares, arrays_of_shares);
    }
