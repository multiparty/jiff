...
(function(exports, node) {
   ...
   *          will be empty.
   */
  function jiff_share(jiff, secret, receivers_list, senders_list, Zp, op_id) {
    // defaults
    if(Zp == null) Zp = gZp;
    ...
    if(senders_list.indexOf(jiff.id) > -1) {
      // compute shares
      var shares = jiff_compute_shares(secret, jiff.party_count, receivers_list, Zp);
    ...
    if(receivers_list.indexOf(jiff.id) > -1) {
      ...
      for(var i = 0; i < senders_list.length; i++) {
        ...
        if(p_id == jiff.id) {
          result[p_id] = new secret_share(jiff, true, null, shares[p_id], receivers_list, Zp);
        ...
        result[p_id] = new secret_share(jiff, false, promise, undefined, receivers_list, Zp);
      }
    }
   ...
   */
  function jiff_triplet(jiff, receivers_list, Zp) {
    if(Zp == null) Zp = gZp;
    if(receivers_list == null)
      for(var i = 1; i <= jiff.party_count; i++) receivers_list.push(i);
    ...
    var msg = JSON.stringify({triplet_id: triplet_id, receivers: receivers_list, Zp: Zp});
    ...
    var a_share = new secret_share(jiff, false, a_deferred.promise(), undefined, receivers_list, Zp);
    var b_share = new secret_share(jiff, false, b_deferred.promise(), undefined, receivers_list, Zp);
    var c_share = new secret_share(jiff, false, c_deferred.promise(), undefined, receivers_list, Zp);
   ...
   * @param {number} Zp - the modulos (if null then global Zp is used by default) [optional].
   */
  function jiff_server_share_number(jiff, options, receivers_list, Zp) {
    if(Zp == null) Zp = gZp;
    if(receivers_list == null)
      for(var i = 1; i <= jiff.party_count; i++) receivers_list.push(i);
    ...
    var msg = { number_id: number_id, receivers: receivers_list, Zp: Zp };
    ...
    var share = new secret_share(jiff, false, deferred.promise(), undefined, receivers_list, Zp);
    return share;
  }
  ...
  function jiff_coerce_to_share(jiff, number, holders, Zp) {
    ...
    return new secret_share(jiff, true, null, number, holders, Zp);
  }
   ...
   *
   */
  function secret_share(jiff, ready, promise, value, holders, Zp) {
    var self = this;
    ...
    this.refresh = function() {
      return self.add(self.jiff.server_generate_and_share({"number": 0}, self.holders, self.Zp));
    };
    ...
    this.add_cst = function(cst){
      ...
      if(self.ready) // if share is ready
        return new secret_share(self.jiff, true, null, mod((self.value + cst), self.Zp), self.holders, self.Zp);

      var promise = self.promise.then(function() { return mod((self.value + cst), self.Zp); }, self.error);
      return new secret_share(self.jiff, false, promise, undefined, self.holders, self.Zp);
    };
    ...
    this.add = function(o) {
      ...
      if(self.ready && o.ready) // both shares are ready
        return new secret_share(self.jiff, true, null, ready_add(), self.holders, self.Zp);
    ...
    this.sub = function(o) {
      ...
      if(self.ready && o.ready) // both shares are ready
        return new secret_share(self.jiff, true, null, ready_sub(), self.holders, self.Zp);
    ...
    this.mult = function(o) {
      ...
      var result = new secret_share(self.jiff, false, final_promise, undefined, self.holders, self.Zp);

      // Get shares of triplets.
      var triplet = jiff.triplet(self.holders, self.Zp);
    ...
    this.greater_or_equal = function(o, l) {
      ...
      function preprocess() {
        ...
        var r_bits = [];
        for(var i = 0; i < l + k; i++)
          r_bits[i] = self.jiff.server_generate_and_share({ "bit": true }, self.holders, self.Zp);
        ...
        var s_bit = self.jiff.server_generate_and_share({ "bit": true }, self.holders, self.Zp);
        var s_sign = s_bit.mult_cst(-2).add_cst(1);

        var mask = self.jiff.server_generate_and_share({ "nonzero": true }, self.holders, self.Zp);
  ...
  function make_jiff(hostname, computation_id, options) {
     ...
     */
    jiff.share = function(secret, receivers_list, senders_list, Zp) { return jiff_share(jiff, secret, receivers_list, senders_list, Zp); };
     ...
     * @returns {promise} a (JQuery) promise to the open value of the secret.
     */
    jiff.receive_open = function(parties) { return jiff_open(jiff, new secret_share(jiff, true, null, null, parties, gZp), [ jiff.id ]); };
     ...
     */
    jiff.triplet = function(receivers_list, Zp) { return jiff_triplet(jiff, receivers_list, Zp); };
     ...
     */
    jiff.server_generate_and_share = function(options, receivers_list, Zp) { return jiff_server_share_number(jiff, options, receivers_list, Zp) };
