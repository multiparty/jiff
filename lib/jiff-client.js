...
(function (exports, node) {
    ...
    self.bit_decomposition = function (op_id) {
      ...
      bitsPromise.then(function (bits) {
        ...
        self.jiff.internal_open(r.isadd(self), self.holders, op_id + ':open').then(function (result) {
          var resultBits = self.jiff.helpers.number_to_bits(result, bitLength);
          var resultZpBits = self.jiff.helpers.number_to_bits(share_helpers['+'](result, self.Zp), bitLength);

          // compute bits assuming r+self < Zp
          var noWrap = resultBits.jiff.protocols.bits.csubr(resultBits, bits, op_id + ':bits.csubr:1');

          // compute bits assuming r+self >= Zp
          var withWrap = resultBits.jiff.protocols.bits.csubr(resultZpBits, bits, op_id + ':bits.csubr:2');

          // choose noWrap if first subtraction does not overflow (sign bit is zero), otherwise choose withWrap.
          result = [];
          for (var i = 0; i < bitLength; i++) {
            result[i] = noWrap[bitLength+1].iif_else(withWrap[i], noWrap[i], op_id + ':if_else:'+i);
          }
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.bits.rejection_sampling = function (lower_bound, upper_bound, threshold, compute_list, Zp, op_id) {
      ...
      if (op_id == null) {
        op_id = jiff.gen_op_id('sample', compute_list);
      }
    ...
    jiff.protocols.bits.cadd = function (bits, constant, op_id) {
      ...
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.cadd', bits[0].holders);
      }
      ...
      for (var i = 1; i < bits.length; i++) {
        ...
        if (constant_bits[i].toString() === '1') {
          carry = and.isor_bit(xorAndCarry, op_id + ':sor_bit:' + (i - 1));
        }
      }
    ...
    jiff.protocols.bits.csubl = function (bits, constant, op_id) {
      ...
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.csubl', bits[0].holders);
      }
    ...
    jiff.protocols.bits.csubr = function (constant, bits, op_id) {
      ...
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.csubr', bits[0].holders);
      }
      ...
      // initialize results
      var diff = [ bits[0].icxor_bit(constant[0]) ];
      var borrow = bits[0].issub(bits[0].icmult(constant[0]));

      // loop over the remaining bits
      for (var i = 1; i < bits.length; i++) {
        var xor = bits[i].icxor_bit(constant[i]);
        var andNot = bits[i].issub(bits[i].icmult(constant[i]));
    ...
    jiff.protocols.bits.sadd = function (bits1, bits2, op_id) {
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.sadd', bits1[0].holders);
      }
      ...
      for (; i < bits1.length; i++) {
        ...
        sum[i] = bits1.isxor_bit(carry, op_id + 'sxor_bit1:' + i);
        carry = bits1.ismult(carry, op_id + ':smult1:' + i);
      }
    ...
    jiff.protocols.bits.ssub = function (bits1, bits2, op_id) {
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.ssub', bits1[0].holders);
      }
    ...
    jiff.protocols.bits.cmult = function (bits, constant, op_id) {
      ...
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.cmult', bits[0].holders);
      }
    ...
    jiff.protocols.bits.smult = function (bits1, bits2, op_id) {
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.smult', bits1[0].holders);
      }
    ...
    jiff.protocols.bits.sdiv = function (bits1, bits2, op_id) {
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.sdiv', bits1[0].holders);
      }
      ...
      for (var i = bits1.length - 1; i >= 0; i--) {
        ...
        // add bit i to the head of remainder (lease significant bit)
    ...
    jiff.protocols.bits.cdivl = function (bits, constant, op_id) {
      ...
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.cdivl', bits[0].holders);
      }
    ...
    jiff.protocols.bits.cdivr = function (constant, bits, op_id) {
      ...
      if (op_id == null) {
        op_id = self.jiff.counters.gen_op_id('bits.cdivr', bits[0].holders);
      }
