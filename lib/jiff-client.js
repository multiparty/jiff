...
(function (exports, node) {
  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    jiff.protocols.bits.sadd = function (bits1, bits2, op_id) {
      ...
      // initialize results
      var result = many_secret_shares(jiff, bits1.length, bits1[0].holders, bits1[0].threshold, bits1[0].Zp, op_id);
      var deferreds = result.deferreds;
      result = result.shares;
    ...
    jiff.protocols.bits.ssub = function (bits1, bits2, op_id) {
      ...
      // initialize results
      var result = many_secret_shares(jiff, Math.max(bits1.length, bits2.length), bits1[0].holders, bits1[0].threshold, bits1[0].Zp, op_id);
      var deferreds = result.deferreds;
      result = result.shares;
     ...
     *                                              default value should suffice when the code of all parties executes all instructions
     *                                              in the same exact order, otherwise, a unique base name is needed here.
     * @returns {SecretShare[]} bitwise sharing of the result, the length of the result will be bits.length + ceil(log2(constant)) inclusive,
