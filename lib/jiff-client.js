  ...
  function make_jiff(hostname, computation_id, options) {
    ...
    var guard_socket = function (socket) {
      ...
      socket.safe_disconnect = function (free, callback) {
        if (socket.mailbox.head == null && jiff.counters.pending_opens === 0) {
          if (free) {
            socket.safe_emit('free', JSON.stringify({}));
          }
    ...
    jiff.emit = function (tag, receivers, message, encrypt) {
      ...
      for (var p = 0; p < receivers.length; p++) {
        ...
        if (encrypt !== false) {
          ...
          jiff.socket.safe_emit('custom', JSON.stringify({tag: tag, party_id: receivers[p], message: message_to_send, encrypted: true}));
        } else {
          jiff.socket.safe_emit('custom', JSON.stringify({tag: tag, party_id: receivers[p], message: message_to_send}));
        }
      }
    };
    ...
    jiff.disconnect = function (safe, free, callback) {
      if (safe) {
        ...
        if (free) {
          jiff.socket.emit('free', JSON.stringify({}));
        }
    ...
    jiff.socket.on('init', function (msg) {
      ...
      jiff.socket.emit('public_key', JSON.stringify({ public_key: jiff.hooks.dumpKey(jiff.public_key) }));
