  ...
  make_jiff: function(http, options) {
    ...
    jiff.request_triplet_share = function(msg, computation_id, from_id) {
      ...
      if(all_triplets[triplet_id] == null) { // Generate Triplet.
        ...
        var triplet_shares = {};
        for(var i = 0; i < all_parties_list.length; i++) {
          var pid = all_parties_list[i];
          var a = a_shares[pid];
          var b = b_shares[pid];
          var c = c_shares[pid];
    ...
    jiff.request_number_share = function(msg, computation_id, from_id) {
      ...
      if(all_numbers[number_id] == null) { // Generate shares for number.
        ...
        var all_parties_list = [ "s1" ];
        for(var i = 1; i <= totalparty_map[computation_id]; i++) all_parties_list.push(i);
        all_numbers[number_id] = client.jiff_compute_shares(number, totalparty_map[computation_id], all_parties_list, Zp);
      }
