...
exports.make_jiff = function (http, options) {
  ...
  jiff.initializeParty = function (computation_id, party_id, party_count, socket) {
    ...
    if (party_count == null) { // no party count given or saved.
      return { success: false, error: 'party count is not specified nor pre-saved' };
    } else if (party_count < 1) { // Too small
      return { success: false, error: 'party count is less than 1' };
    } else if (jiff.totalparty_map[computation_id] != null && party_count !== jiff.totalparty_map[computation_id]) {
      // contradicting values
      return { success: false, error: 'contradicting party count' };
    }
    ...
    if (party_id != null) { // party_id is given, check validity
      ...
      if (party_id == null) { // first() could not find a spare id!
        return { success: false, error: 'Maximum parties capacity reached' };
      }
    }
  ...
  jiff.share = function (computation_id, from_id, msg) {
    ...
    try {
      msg = jiff.execute_array_hooks('onReceive', ['share', computation_id, from_id, msg], 3);
    } catch (err) {
  ...
  jiff.open = function (computation_id, from_id, msg) {
    ...
    try {
      msg = jiff.execute_array_hooks('onReceive', ['open', computation_id, from_id, msg], 3);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
  ...
  jiff.triplet = function (computation_id, from_id, msg) {
    ...
    try {
      msg = jiff.execute_array_hooks('onReceive', ['triplet', computation_id, from_id, msg], 3);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
  ...
  jiff.number = function (computation_id, from_id, msg) {
    ...
    try {
      msg = jiff.execute_array_hooks('onReceive', ['number', computation_id, from_id, msg], 3);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
  ...
  jiff.custom = function (computation_id, from_id, msg) {
    ...
    try {
      msg = jiff.execute_array_hooks('onReceive', ['custom', computation_id, from_id, msg], 3);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
  ...
  jiff.io.on('connection', function (socket) {
    ...
    socket.on('public_key', function (msg) {
      ...
      var output = jiff.public_key(computation_id, party_id, msg);
      if (!output.success) {
...
function initialize_hooks(jiff, options) {
  ...
  if (jiff.hooks.onReceive == null) {
    jiff.hooks.onReceive = [];
  }
}
