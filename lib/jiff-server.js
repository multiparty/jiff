...
module.exports = {
  ...
  make_jiff: function (http, options,app) {
    ...
    app.post('/share', function (req,res) {
      ...
      console.log(req.body.computation_id);
      if (jiff.logs) {
        console.log('share from ' + req.body.computation_id + '-' + req.body.from_id + ':' + req.body.msg);
      }

      const json_msg = JSON.parse(req.body.msg);
      const to_id = json_msg['party_id'];

      json_msg['party_id'] = req.body.from_id;

      if (to_id === 's1') {
        computation_instances_deferred[req.body.computation_id].then(function () {
        ...
        //jiff.safe_emit('share', JSON.stringify(json_msg), req.body.computation_id , to_id);
        console.log(jiff.safe_emit('share',json_msg),req.body.computation_id,to_id);
        res.json(json_msg);
      }
