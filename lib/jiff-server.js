...
exports.make_jiff = function (http, options) {
  ...
  if (options.hooks.beforeInitialize == null) {
    jiff.hooks.beforeInitialize.push(function (jiff, computation_id, msg, params) {
  ...
  if (options.hooks.onInitializeUsedId == null) {
    jiff.hooks.onInitializeUsedId.push(function (jiff, computation_id, party_id, party_count, msg) {
  ...
  jiff.initializeParty = function (computation_id, party_id, party_count, msg) {
    ...
    // validate parameters
    try {
      var hook_result = jiff.execute_array_hooks('beforeInitialize', [jiff, computation_id, msg, { party_id: party_id, party_count: party_count }], 3);
    ...
    if (party_id != null) { // party_id is given, check validity
      if (!jiff.spare_party_ids[computation_id].is_free(party_id)) {
        ...
        try {
          party_id = jiff.execute_array_hooks('onInitializeUsedId', [jiff, computation_id, party_id, party_count, msg], 2);
        } catch (err) {
          return { success: false, error: typeof(err) === 'string' ? err : err.message };
        }
      }
    ...
    var message = jiff.execute_array_hooks('afterInitialize', [jiff, computation_id, party_id, party_count, msg, { party_id: party_id, party_count: party_count }], 5);
  ...
  jiff.public_key = function (computation_id, party_id, msg) {
    jiff.hooks.log(jiff, 'public_key from ' + computation_id + '-' + party_id + ' : ' + JSON.stringify(msg));

    // validate parameters
    try {
      jiff.execute_array_hooks('beforePublicKey', [jiff, computation_id, party_id, msg], -1);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    keymap_to_send = jiff.execute_array_hooks('afterPublicKey', [jiff, computation_id, party_id, msg, keymap_to_send], 4);
  ...
  jiff.triplet = function (computation_id, from_id, msg) {
    ...
    try {
      msg = jiff.hooks.decryptSign(jiff, msg, jiff.secret_key_map[computation_id], jiff.key_map[computation_id][from_id], 'triplet');
      msg = JSON.parse(msg);
    ...
    triplet_msg = jiff.hooks.encryptSign(jiff, triplet_msg, pkey, jiff.secret_key_map[computation_id], 'triplet');
  ...
  jiff.number = function (computation_id, from_id, msg) {
    // decrypt and verify signature.
    try {
      msg = jiff.hooks.decryptSign(jiff, msg, jiff.secret_key_map[computation_id], jiff.key_map[computation_id][from_id], 'number');
      msg = JSON.parse(msg);
    } catch (error) { // invalid signature
    ...
    var pkey = jiff.key_map[computation_id][from_id];
    number_msg = jiff.hooks.encryptSign(jiff, number_msg, pkey, jiff.secret_key_map[computation_id], 'number');
...
function initialize_hooks(jiff, options) {
  ...
  if (jiff.hooks.encryptSign == null) {
    if (options.sodium !== false) {
      ...
      jiff.hooks.encryptSign = function encrypt_and_sign(jiff, message, encryption_public_key, signing_private_key, operation_type) {
        return message;
      }
    }
  }
