...
module.exports = {
  ...
  make_jiff: function (http, options,app) {
    ...
    //modularize initalization for RESTful and socket connection

    //function initalize(msg,type) {

    //}

    function initalize(msg,req,res) {
      console.log('entering initalize');
        ...
        if (party_count == null) { // no party count given or saved.
          res.send('error', 'party count is not specified nor pre-saved');
          return;
        } else if (party_count < 1) { // Too small
          res.send('error', 'party count is less than 1');
          return;
        } else if (totalparty_map[computation_id] != null && party_count !== totalparty_map[computation_id]) {
          // contradicting values
          res.send('error', 'contradicting party count');
          return;
        }
        ...
        if (party_id != null) { // party_id is given, check validity
          ...
          if (isNaN(party_id) || party_id <= 0 || party_id > party_count) {
            ...
            if (previous_socket != null && previous_socket.connected) {
              res.send('error', party_id + ' is already taken');
              return;
            }
            */
          ...
          if (party_id == null) { // first() could not find a spare id!
            res.send('error', 'Maximum parties capacity reached');
            return;
          }
        }
        ...
        */
        ...
        res.send('init', JSON.stringify({party_id: party_id, party_count: party_count}));
    ...
    app.post('/open', function (req,res) {
      ...
      if (jiff.logs) {
        console.log('open from' + packet.computation_id + '-' + packet.from_id + ':' + packet.msg);
      }

      const json_msg = packet.msg;
      var to_id = json_msg['party_id'];
      packet.msg['party_id'] = packet.from_id;

      if (to_id === 's1') {
        computation_instances_deferred[packet.computation_id].then(function () {
          console.log('to_id = s1 ');
        });
      } else {
        jiff.safe_emit('open', json_msg, packet.computation_id, to_id);
      }
    });
    ...
    app.post('/share', function (req,res) {
      ...
      const packet = req.body.packet;  
      if (jiff.logs) {
        console.log('share from ' + packet.computation_id + '-' + packet.from_id + ':' + packet.msg);
      }

      const json_msg = packet.msg;
      const to_id = json_msg['party_id'];

      json_msg['party_id'] = req.body.from_id;

      if (to_id === 's1') {
        computation_instances_deferred[packet.computation_id].then(function () {
          console.log('to_id = s1 ')
          //computation_instances_map[req.body.computation_id].socket.receive('share', JSON.stringify(json_msg));
        });

      } else {

        jiff.safe_emit('share', json_msg, packet.computation_id , to_id);
        console.log(jiff)
        //res.json(json_msg);
      }
