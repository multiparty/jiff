...
module.exports = {
  ...
  make_jiff: function (http, options) {
    ...
    jiff.compute = function (computation_id, options) {
      if (!computation_instances_deferred[computation_id]) {
        computation_instances_deferred[computation_id] = $.Deferred();
      }
      if (!mailbox[computation_id]) {
    ...
    jiff.safe_emit = function (label, msg, computation_id, to_id) {
      ...
      if (socket && socket.connected) {
        ...
        socket.emit(label, msg, function (status) {
          if (status) {
            computation_mailbox[to_id].delete(mailbox_pointer);
          }
        });
      }
    };
    ...
    jiff.resend_mailbox = function (computation_id, party_id) {
      var computation_mailbox = mailbox[computation_id];
      if (!computation_mailbox[party_id]) {
    ...
    io.on('connection', function (socket) {
      ...
      socket.on('computation_id', function (msg) {
        ...
        if (!party_count) { // no party count given or saved.
          ...
          return;
        } else if (totalparty_map[computation_id] && party_count !== totalparty_map[computation_id]) {
        ...
        if (party_id) { // party_id is given, check validity
          ...
          if (isNaN(party_id) || party_id <= 0 || party_id > party_count) {
            ...
            return;
          } else if (spare_party_ids[computation_id] && spare_party_ids[computation_id].remove(party_id) === false) { // remove party_id if spare
          ...
          if (spare_party_ids[computation_id]) {
            party_id = spare_party_ids[computation_id].first();
          } else {
            party_id = 1;
          }

          if (!party_id) { // first() could not find a spare id!
        ...
        if (!client_map[computation_id]) {
          client_map[computation_id] = [];
        }
        if (!socket_map[computation_id]) {
          socket_map[computation_id] = {};
        }
        if (!totalparty_map[computation_id]) {
          totalparty_map[computation_id] = party_count;
        }
        if (!mailbox[computation_id]) {
          mailbox[computation_id] = {};
        }
        if (!triplets_map[computation_id]) {
          triplets_map[computation_id] = {};
        }
        if (!numbers_map[computation_id]) {
      ...
      socket.on('public_key', function (msg) {
        jiff.sodium_promise.then(function () {
          ...
          if (!tmp) { // generate public and secret key for server if they dont exist
          ...
          for (i = 0; i < send_to_parties.length; i++) {
    ...
    jiff.request_triplet_share = function (msg, computation_id, from_id) {
      ...
      if (!all_triplets[triplet_id]) { // Generate Triplet.
    ...
    jiff.request_number_share = function (msg, computation_id, from_id) {
      ...
      for (var i = 0; i < count; i++) {
        ...
        if (!all_numbers[number_id]) { // Generate shares for number.
...
function create_computation_instance(jiff, computation_id, options) {
  ...
  var internal_socket = {
    ...
    emit: function (label, msg) { // from inside the computation instance to the outside world
      var result, json_msg, from_id, to_id;
  ...
  if (jiff.secret_key_map[computation_id]) {
  ...
  // party id and count
  if (jiff.totalparty_map[computation_id]) {
  ...
  if (jiff.key_map[computation_id]) {
    var keymap_to_send = {};
