...
module.exports = {
  ...
  make_jiff: function(http, options) {
    ...
    var establish_server_side_connections = false;
    var server_side_jiff_instances = {};
    ...
    io.on('connection', function(socket) {
      ...
      socket.on('computation_id', function(msg) {
        ...
        if (msg.establish_server_side_connections) {
          establish_server_side_connections = msg.establish_server_side_connections;
          ...
          if (server_side_clients > 0){
            ...
            for (var i = 0; i < server_side_clients; i++){
              var jiff_instance = client.make_jiff("http://localhost:3000", computation_id, {"computation_id":computation_id,"party_count":msg.party_count});
      ...
      socket.on('public_key', function(msg) {
        jiff.sodium_promise.then(function() {
          ...
          if(computation_instances_map[computation_id] != null && computation_instances_map[computation_id].secret_key == null) {
      ...
      socket.on('custom', function(msg) {
        ...
        for(var i = 0; i < receivers.length; i++) {
          ...
          if(to_id == 's1') send_to_self = true;
            ...
            io.to(socket_map[computation_id][to_id]).emit('custom', msg);
        }
      ...
      if(label == 'custom') {
        ...
        for(var i = 0; i < receivers.length; i++) {
          ...
          if(to_id == 's1') send_to_self = true;
            ...
            var socket_to_use = jiff.socket_map[computation_id][to_id];
            jiff.io.to(socket_to_use).emit('custom', msg);
          }
        }
