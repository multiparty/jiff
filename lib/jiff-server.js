...
module.exports = {
  ...
  make_jiff: function(http, options) {
    ...
    // Parse encryption option. Default is 'AsymmetricKeys'
    jiff.encryptionType = !options.encryptionType ? 'AsymmetricKeys' : options.encryptionType;
    ...
    // { computation_id -> { party_id -> symmetric_key}}
    var symmetric_key_map = {};
    jiff.symmetric_key_map = symmetric_key_map;
    ...
    io.on('connection', function(socket) {
      ...
      socket.on('symmetric_key', function(_) {
        if(jiff.encryptionType == "SymmetricKey") {
          jiff.sodium_promise.then(function() {
            var party_id = party_map[socket.id];
            var computation_id = computation_map[socket.id];
            var generatedSymmetricKey = jiff.sodium.crypto_aead_xchacha20poly1305_ietf_keygen();

            if(symmetric_key_map[computation_id] == null)
              symmetric_key_map[computation_id] = {};
            symmetric_key_map[computation_id][party_id] = generatedSymmetricKey;

            var encryptedSymmetricKey = client.utils.encrypt_and_sign(
              '[' + generatedSymmetricKey.toString() + ']',
              key_map[computation_id][party_id],
              secret_key_map[computation_id],
              'symmetric_key'
            );
            io.to(socket.id).emit('symmetric_key', encryptedSymmetricKey);
          });
        } else {
          //do nothing. public keys should be used.
        }
      });
