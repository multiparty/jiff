  ...
  make_jiff: function(http, options) {  
    ...
    jiff.request_triplet_share = function(msg, computation_id, from_id) {
      ...
      if(all_triplets[triplet_id] == null) { // Generate Triplet.
        ...
        var a_shares = client.sharing_schemes.shamir_share({ party_count: totalparty_map[computation_id] }, a, receivers, threshold, Zp);
        var b_shares = client.sharing_schemes.shamir_share({ party_count: totalparty_map[computation_id] }, b, receivers, threshold, Zp);
        var c_shares = client.sharing_schemes.shamir_share({ party_count: totalparty_map[computation_id] }, c, receivers, threshold, Zp);
    ...
    jiff.request_number_share = function(msg, computation_id, from_id) {
      ...
      if(all_numbers[number_id] == null) { // Generate shares for number.
        ...
        // Compute shares
        all_numbers[number_id] = client.sharing_schemes.shamir_share({ party_count: totalparty_map[computation_id] }, number, receivers, threshold, Zp);
      }
