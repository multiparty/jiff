...
module.exports = {
  ...
  make_jiff: function(http, options) {  
    ...
    jiff.request_triplet_share = function(msg, computation_id, from_id) {
      ...
      var Zp = parseInt(msg.Zp.toString(10), 10);;
      ...
      if(all_triplets[triplet_id] == null) { // Generate Triplet.
        ...
        var a_shares = client.sharing_schemes.shamir_share({ party_count: totalparty_map[computation_id], helpers: { random: function(m) { return Math.floor(Math.random() * m); } } }, a, receivers, threshold, Zp);
        var b_shares = client.sharing_schemes.shamir_share({ party_count: totalparty_map[computation_id], helpers: { random: function(m) { return Math.floor(Math.random() * m); } } }, b, receivers, threshold, Zp);
        var c_shares = client.sharing_schemes.shamir_share({ party_count: totalparty_map[computation_id], helpers: { random: function(m) { return Math.floor(Math.random() * m); } } }, c, receivers, threshold, Zp);
    ...
    jiff.request_number_share = function(msg, computation_id, from_id) {
      ...
      var max = msg.max;
      if(max == null) max = Zp;
      max = parseInt(max.toString(10), 10);
      ...
      if(all_numbers[number_id] == null) { // Generate shares for number.
        ...
        // Compute shares
        all_numbers[number_id] = client.sharing_schemes.shamir_share({ party_count: totalparty_map[computation_id], helpers: { random: function(m) { return Math.floor(Math.random() * m); } } }, number, receivers, threshold, Zp);
      }
