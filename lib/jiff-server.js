...
exports.make_jiff = function (http, options) {
  ...
  if (jiff.hooks.trackFreeIds == null) {
    jiff.hooks.trackFreeIds = function (party_count) {
      return intervals(1, party_count);
    };
  }

  if (options.hooks.beforeInitialize == null) {
    jiff.hooks.beforeInitialize.push(function (computation_id, msg, params) {
  ...
  jiff.lastSocket = null;
  if (options.hooks.onInitializeUsedId == null) {
    jiff.hooks.onInitializeUsedId.push(function (computation_id, party_id, party_count, msg) {
      ...
      if (previous_socket != null && previous_socket.connected && previous_socket !== jiff.lastSocket) {
        return { success: false, error: party_id + ' is already taken' };
      }
    });
  }
  ...
  // Functionality
  jiff.initializeParty = function (computation_id, party_id, party_count, msg) {
    jiff.hooks.log('initialize with ', computation_id, '-', party_id, ' #', party_count, ' : ', JSON.stringify(msg));
    ...
    // validate parameters
    try {
      var hook_result = jiff.execute_array_hooks('beforeInitialize', [computation_id, msg, { party_id: party_id, party_count: party_count }], 2);
    ...
    if (jiff.spare_party_ids[computation_id] == null) {
      jiff.spare_party_ids[computation_id] = jiff.hooks.trackFreeIds(party_count);
    }
    ...
    if (party_id != null) { // party_id is given, check validity
      if (!jiff.spare_party_ids[computation_id].is_free(party_id)) {
        ...
        try {
          party_id = jiff.execute_array_hooks('onInitializeUsedId', [computation_id, party_id, party_count, msg], 1);
        } catch (err) {
          return { success: false, error: typeof(err) === 'string' ? err : err.message };
        }
      }
    ...
    var message = jiff.execute_array_hooks('afterInitialize', [computation_id, party_id, party_count, msg, { party_id: party_id, party_count: party_count }], 4);
  ...
  jiff.public_key = function (computation_id, party_id, msg) {
    jiff.hooks.log('public_key from ' + computation_id + '-' + party_id + ' : ' + JSON.stringify(msg));

    // validate parameters
    try {
      jiff.execute_array_hooks('beforePublicKey', [computation_id, party_id, msg], -1);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    if (tmp == null) { // generate public and secret key for server if they dont exist
      var genkey = jiff.hooks.generateKeyPair();
      jiff.secret_key_map[computation_id] = genkey.secret_key;
      tmp = {s1: genkey.public_key};
    }

    tmp[party_id] = jiff.hooks.parseKey(msg.public_key);
    ...
    keymap_to_send = { public_keys: keymap_to_send };
    keymap_to_send = jiff.execute_array_hooks('afterPublicKey', [computation_id, party_id, msg, keymap_to_send], 3);

    var keymap_str = JSON.stringify(keymap_to_send);
    var send_to_parties = jiff.client_map[computation_id];
    for (var i = 0; i < send_to_parties.length; i++) {
      jiff.emit('public_key', keymap_str, computation_id, send_to_parties[i]);
    }

    // Send the public keys to any server side instance that is supposed to participate in the computation.
    jiff.computation_instances_deferred[computation_id].then(function () {
      jiff.computation_instances_map[computation_id].socket.receive('public_key', keymap_str);
    });
    ...
    return { success: true, message: keymap_to_send };
  };

  jiff.share = function (computation_id, from_id, msg) {
    jiff.hooks.log('share from ' + computation_id + '-' + from_id + ' : ' + JSON.stringify(msg));

    try {
      msg = jiff.execute_array_hooks('beforeOperation', ['share', computation_id, from_id, msg], 3);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    msg = jiff.execute_array_hooks('afterOperation', ['share', computation_id, from_id, msg], 3);
  ...
  jiff.open = function (computation_id, from_id, msg) {
    jiff.hooks.log('open from ' + computation_id + '-' + from_id + ' : ' + JSON.stringify(msg));

    try {
      msg = jiff.execute_array_hooks('beforeOperation', ['open', computation_id, from_id, msg], 3);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    msg = jiff.execute_array_hooks('afterOperation', ['open', computation_id, from_id, msg], 3);
  ...
  jiff.triplet = function (computation_id, from_id, msg) {
    ...
    try {
      msg = jiff.hooks.decryptSign(msg, jiff.secret_key_map[computation_id], jiff.key_map[computation_id][from_id], 'triplet');
      msg = JSON.parse(msg);
    } catch (error) { // invalid signature
      jiff.hooks.log('Error in triplet from ' + computation_id + '-' + from_id + ': ' + error);
      return { success: false, error: 'invalid signature'};
    }

    try {
      msg = jiff.execute_array_hooks('beforeOperation', ['triplet', computation_id, from_id, msg], 3);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    triplet_msg = jiff.execute_array_hooks('afterOperation', ['triplet', computation_id, from_id, triplet_msg], 3);
    triplet_msg = JSON.stringify(triplet_msg);

    // encrypt an sign message then send it.
    var pkey = jiff.key_map[computation_id][from_id];
    triplet_msg = jiff.hooks.encryptSign(triplet_msg, pkey, jiff.secret_key_map[computation_id], 'triplet');
  ...
  jiff.number = function (computation_id, from_id, msg) {
    // decrypt and verify signature.
    try {
      msg = jiff.hooks.decryptSign(msg, jiff.secret_key_map[computation_id], jiff.key_map[computation_id][from_id], 'number');
      msg = JSON.parse(msg);
    } catch (error) { // invalid signature
      jiff.hooks.log('Error in number from ' + computation_id + '-' + from_id + ': ' + error);
      return { success: false, error: 'invalid signature' };
    }

    try {
      msg = jiff.execute_array_hooks('beforeOperation', ['number', computation_id, from_id, msg], 3);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    number_msg = jiff.execute_array_hooks('afterOperation', ['number', computation_id, from_id, number_msg], 3);
    number_msg = JSON.stringify(number_msg);

    // encrypt and sign message then send it.
    var pkey = jiff.key_map[computation_id][from_id];
    number_msg = jiff.hooks.encryptSign(number_msg, pkey, jiff.secret_key_map[computation_id], 'number');

    return { success: true, message: number_msg };
  };

  jiff.custom = function (computation_id, from_id, msg) {
    jiff.hooks.log('custom from ' + computation_id + '-' + from_id + ' : ' + msg);

    try {
      msg = jiff.execute_array_hooks('beforeOperation', ['custom', computation_id, from_id, msg], 3);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    msg = jiff.execute_array_hooks('afterOperation', ['custom', computation_id, from_id, msg], 3);
  ...
  jiff.free = function (computation_id, party_id, msg) {
    jiff.hooks.log('free ' + computation_id + '-' + party_id);

    try {
      jiff.execute_array_hooks('beforeFree', [computation_id, party_id, msg], -1);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    if (jiff.freeparty_map[computation_id] === jiff.totalparty_map[computation_id]) {
      jiff.hooks.log('free computation ' + computation_id);
      ...
      jiff.execute_array_hooks('afterFree', [computation_id, party_id, msg], -1);
    }
  ...
  jiff.io.on('connection', function (socket) {
    ...
    socket.on('computation_id', function (msg) {
      ...
      jiff.lastSocket = socket; // Hack-ish trick to pass this as a parameter to the default hook.
      ...
      if (output.success) {
        ...
        jiff.io.to(socket.id).emit('init', output.message);
      } else {
    ...
    socket.on('public_key', function (msg) {
      ...
      var output = jiff.public_key(computation_id, from_id, msg);
      if (!output.success) {
    ...
    socket.on('disconnect', function () {
      ...
      jiff.hooks.log('user disconnected', computation_id, from_id);

      jiff.execute_array_hooks('onDisconnect', [ computation_id, from_id ], -1);
    });
...
function initialize_hooks(jiff, options) {
  ...
  if (jiff.hooks.log == null) {
    jiff.hooks.log = function (l) {
      if (jiff.logs === true) {
        console.log(l);
      }
    };
  }
  if (jiff.hooks.beforeInitialize == null) {
    jiff.hooks.beforeInitialize = [];
  }
  if (jiff.hooks.onInitializeUsedId == null) {
    jiff.hooks.onInitializeUsedId = [];
  }
  if (jiff.hooks.afterInitialize == null) {
    jiff.hooks.afterInitialize = [];
  }
  if (jiff.hooks.beforePublicKey == null) {
    jiff.hooks.beforePublicKey = [];
  }
  if (jiff.hooks.afterPublicKey == null) {
    jiff.hooks.afterPublicKey = [];
  }
  ...
  if (jiff.hooks.encryptSign == null) {
    if (options.sodium !== false) {
      ...
      jiff.hooks.encryptSign = function encrypt_and_sign(message, encryption_public_key, signing_private_key, operation_type) {
        return message;
      }
    }
  }
  ...
  if (jiff.hooks.generateKeyPair == null) {
    if (options.sodium !== false) {
      jiff.hooks.generateKeyPair = function () {
  ...
  if (jiff.hooks.parseKey == null) {
    if (options.sodium !== false) {
      jiff.hooks.parseKey = function (keyString) {
  ...
  if (jiff.hooks.dumpKey == null) {
    if (options.sodium !== false) {
      jiff.hooks.dumpKey = function (key) {
        return '[' + key.toString() + ']';
      };
    } else {
      jiff.hooks.dumpKey = function (key) {
  ...
  // computing hooks
  function generateTriplet(computation_id, Zp) {
  ...
  function generateNumber(computation_id, params) {
    var bit = params.bit;
...
function manage_mailboxes(jiff) {
  ...
  if (jiff.hooks.put_in_mailbox == null) {
    jiff.hooks.put_in_mailbox = function (label, msg, computation_id, to_id) {
  ...
  if (jiff.hooks.get_mailbox == null) {
    jiff.hooks.get_mailbox = function (computation_id, party_id) {
      var computation_mailbox = jiff.mailbox[computation_id];
  ...
  if (jiff.hooks.remove_from_mailbox == null) {
    jiff.hooks.remove_from_mailbox = function (computation_id, party_id, mailbox_pointer) {
      jiff.mailbox[computation_id][party_id].remove(mailbox_pointer);
    };
  }

  if (jiff.hooks.slice_mailbox == null) {
    jiff.hooks.slice_mailbox = function (computation_id, party_id, mailbox_pointer) {
  ...
  jiff.safe_emit = function (label, msg, computation_id, to_id) {
    ...
    var store_id = jiff.hooks.put_in_mailbox(label, msg, computation_id, to_id);
    jiff.emit(label, msg, computation_id, to_id, function () {
      jiff.hooks.remove_from_mailbox(computation_id, to_id, store_id);
    });
  };
  ...
  jiff.resend_mailbox = function (computation_id, party_id) {
    ...
    for (var i = 0; i < mailbox.length; i++) {
      ...
      var callback = function (store_id_scoped) {
        return function () {
          jiff.hooks.remove_from_mailbox(computation_id, party_id, store_id_scoped);
        };
      };
...
function create_computation_instance(jiff, computation_id, options) {
  ...
  var internal_socket = {
    ...
    emit: function (label, msg) { // from inside the computation instance to the outside world
      ...
      var from_id = 's1';
      try {
        msg = jiff.execute_array_hooks('beforeOperation', [label, computation_id, from_id, msg], 3);
      } catch (err) {
      ...
      if (label === 'share' || label === 'open') {
        ...
        msg = jiff.execute_array_hooks('afterOperation', [label, computation_id, from_id, msg], 3);
      ...
      if (label === 'number') {
        // Use server code to retrieve/compute share
        msg = jiff.request_number_share(msg, computation_id, computation_instance.id);
  ...
  if (jiff.key_map[computation_id] != null) {
    ...
    for (var i in jiff.key_map[computation_id]) {
      if (jiff.key_map[computation_id].hasOwnProperty(i)) {
        keymap_to_send[i] = jiff.hooks.dumpKey(jiff.key_map[computation_id][i]);
      }
    }
...
function default_preprocessing(jiff) {
  ...
  jiff.request_triplet_share = function (msg, computation_id, from_id) {
    ...
    jiff.hooks.log('triplet ' + triplet_id + ' from ' + computation_id + '-' + from_id + ':: ' + JSON.stringify(msg));
    ...
    if (all_triplets[triplet_id] == null) { // Generate Triplet.
      var triplet = jiff.hooks.generateTriplet(computation_id, Zp);
      var a = triplet.a;
      var b = triplet.b;
      var c = triplet.c;


      var jiff_client_imitation = jiff.create_jiff_imitation(jiff.totalparty_map[computation_id]);
      var a_shares = jiff.hooks.computeShares(jiff_client_imitation, a, receivers, threshold, Zp);
      var b_shares = jiff.hooks.computeShares(jiff_client_imitation, b, receivers, threshold, Zp);
      var c_shares = jiff.hooks.computeShares(jiff_client_imitation, c, receivers, threshold, Zp);
  ...
  jiff.request_number_share = function (msg, computation_id, from_id) {
    ...
    jiff.hooks.log('number ' + base_number_id + ' from ' + computation_id + '-' + from_id + ':: ' + JSON.stringify(msg));
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (all_numbers[number_id] == null) { // Generate shares for number.
        var number = jiff.hooks.generateNumber(computation_id, msg);

        var jiff_client_imitation = jiff.create_jiff_imitation(jiff.totalparty_map[computation_id]);
        all_numbers[number_id] = jiff.hooks.computeShares(jiff_client_imitation, number, receivers, threshold, Zp);
      }
