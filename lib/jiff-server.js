...
exports.make_jiff = function (http, options) {
  ...
  // Functionality
  jiff.initialize_party = function (computation_id, party_id, party_count, msg) {
  ...
  jiff.store_public_key = function (computation_id, party_id, msg) {
    ...
    for (var i = 0; i < send_to_parties.length; i++) {
      if (send_to_parties[i] !== party_id) {
        ...
        var cipher_text = JSON.stringify(jiff.hooks.encryptSign(jiff, broadcast_message, pkey, jiff.secret_key_map[computation_id]));
  ...
  jiff.io.on('connection', function (socket) {
    ...
    socket.on('triplet', function (msg, callback) {
      ...
      if (res.success) {
        ...
        var errorMsg = JSON.stringify({label: 'triplet', error: res.error});
        jiff.emit('error', errorMsg, computation_id, from_id);
      }
    });
    ...
    socket.on('number', function (msg, callback) {
      ...
      if (res.success) {
        ...
        var errorMsg = JSON.stringify({label: 'number', error: res.error});
        jiff.emit('error', errorMsg, computation_id, from_id);
      }
    });
...
function initialize_hooks(jiff, options) {
  ...
  function generateNumber(jiff, computation_id, params) {
    ...
    if (params.number != null) {
      ...
      number = jiff.helpers.random(max - min) + min;
    }
  ...
  var internal_socket = {
    ...
    emit: function (label, msg) { // from inside the computation instance to the outside world
      ...
      try {
        ...
        internal_socket.receive('error', errorMsg);
      }
  ...
  jiff.request_number_share = function (msg, computation_id, from_id) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (all_numbers[number_id] == null) { // Generate shares for number.
        ...
        if (bitLength != null) {
          // format as an object per party with arrays of bit shares for that party
          all_numbers[number_id] = {};
          for (var p = 0; p < receivers.length; p++) {
            all_numbers[number_id][receivers[p]] = [];
          }
          // share in binary, store as array
          var bits = jiff.helpers.number_to_bits(number, bitLength);
          for (var b = 0; b < bits.length; b++) {
            bits[b] = jiff.hooks.computeShares(jiff, bits[b], receivers, threshold, Zp);
            for (var k = 0; k < receivers.length; k++) {
              all_numbers[number_id][receivers[k]].push(bits[b][receivers[k]]);
            }
          }
        } else {
