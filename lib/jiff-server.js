...
exports.make_jiff = function (http, options) {
  ...
  jiff.share = function (computation_id, from_id, msg) {
    ...
    if (to_id === 's1') {
      ...
      jiff.put_in_mailbox('share', JSON.stringify(msg), computation_id, to_id);
    }
  };
  ...
  jiff.open = function (computation_id, from_id, msg) {
    ...
    if (to_id === 's1' ) {
      ...
      jiff.put_in_mailbox('open', JSON.stringify(msg), computation_id, to_id);
    }
  };
  ...
  jiff.custom = function (computation_id, from_id, msg) {
    ...
    if (receiver === 's1') {
      ...
      jiff.put_in_mailbox('custom', msg, computation_id, receiver);
    }
  };
  ...
  jiff.io.on('connection', function (socket) {
    ...
    socket.on('computation_id', function (msg) {
      ...
      jiff.io.to(socket.id).emit(output.label, JSON.stringify(output.msg));
    ...
    socket.on('triplet', function (msg, callback) {
      ...
      jiff.put_in_mailbox(res.label, res.message, computation_id, from_id);
    });
    ...
    socket.on('free', function (_, callback) {
      ...
      if (jiff.freeparty_map[computation_id] === jiff.totalparty_map[computation_id]) {
        ...
        // remove when pre-processing/preprocessing is done
...
function manage_mailboxes(jiff) {
  ...
  jiff.resend_mailbox = function (computation_id, party_id) {
    ...
    while (current_node != null) {
      ...
      jiff.put_in_mailbox(label, msg, computation_id, party_id);
  ...
  var internal_socket = {
    ...
    emit: function (label, msg) { // from inside the computation instance to the outside world
      ...
      if (label === 'share' || label === 'open') {
        ...
        jiff.put_in_mailbox(label, JSON.stringify(json_msg), computation_id, to_id);
      }
      ...
      if (label === 'custom') {
        ...
        jiff.put_in_mailbox('custom', msg, computation_id, to_id);
      }
    }
  };
