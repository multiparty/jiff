...
const client = require('../jiff-client.js');
...
function create_computation_instance(jiff, computation_id, options) {
  ...
  // Create instance
  const computation_instance = new client('<server_instance>', computation_id, options);
...
InternalSocket.prototype.emit = function (label, msg) {
  ...
  if (['share', 'open', 'custom', 'crypto_provider'].indexOf(label) > -1) {
    const output = this.jiff.handlers[label](this.computation_id, from_id, msg);
    if (!output.success) {
      const errorMsg = JSON.stringify({ label: label, error: output.error });
