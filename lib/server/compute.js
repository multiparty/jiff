...
InternalSocket.prototype.connect = function () {
  ...
  var output = this.jiff.initializeParty(this.computation_id, 's1', computation_instance.party_count, msg, true);
...
InternalSocket.prototype.emit = function (label, msg) {
  ...
  if (['share', 'open', 'custom', 'crypto_provider'].indexOf(label) > -1) {
    var output = this.jiff.handlers[label](computation_instance, from_id, msg);
    if (!output.success) {
