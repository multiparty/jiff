...
function remove_point(intervals, point) {
  ...
  function remove_from_interval(index) {
    const current_interval = intervals[index];
  ...
  const index = find_point(intervals, point);
  if (index === -1) {
    // point not found
    return false;
  }
...
function find_point(intervals, point) {
  ...
  // binary search
  let st = 0;
  let nd = intervals.length;
  while (st < nd) {
    const mid = Math.floor((st + nd) / 2);
