...
module.exports = function (jiffServer) {
  ...
  jiffServer.handlers.initializeParty = async function (computation_id, party_id, party_count, msg, _s1) {
    ...
    if (_s1 !== true && party_id === 's1' && jiffServer.options.external_crypto_provider !== true) {
      return {
        success: false,
    ...
    try {
      ...
      var hook_result = await jiffServer.hooks.execute_array_hooks('beforeInitialization', [jiffServer, computation_id, msg, params], 3);
    ...
    message = await jiffServer.hooks.execute_array_hooks('afterInitialization', [jiffServer, computation_id, message], 2);
  ...
  jiffServer.handlers.share = async function (computation_id, from_id, msg) {
    jiffServer.hooks.log(jiffServer, 'share from', computation_id, '-', from_id, ' : ', msg);

    try {
      msg = await jiffServer.hooks.execute_array_hooks('beforeOperation', [jiffServer, 'share', computation_id, from_id, msg], 4);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    msg = await jiffServer.hooks.execute_array_hooks('afterOperation', [jiffServer, 'share', computation_id, from_id, msg], 4);
    jiffServer.safe_emit('share', JSON.stringify(msg), computation_id, to_id);

    return { success: true };
  };

  jiffServer.handlers.open = async function (computation_id, from_id, msg) {
    jiffServer.hooks.log(jiffServer, 'open from', computation_id, '-', from_id, ' : ', msg);

    try {
      msg = await jiffServer.hooks.execute_array_hooks('beforeOperation', [jiffServer, 'open', computation_id, from_id, msg], 4);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    msg = await jiffServer.hooks.execute_array_hooks('afterOperation', [jiffServer, 'open', computation_id, from_id, msg], 4);
    jiffServer.safe_emit('open', JSON.stringify(msg), computation_id, to_id);

    return { success: true };
  };

  jiffServer.handlers.crypto_provider = async function (computation_id, from_id, msg) {
    jiffServer.hooks.log(jiffServer, 'crypto_provider from', computation_id, '-', from_id, ':', msg);

    try {
      msg = await jiffServer.hooks.execute_array_hooks('beforeOperation', [jiffServer, 'crypto_provider', computation_id, from_id, msg], 4);
    } catch (err) {
      return {success: false, error: typeof(err) === 'string' ? err : err.message};
    }
    ...
    // hook and serialize
    response = await jiffServer.hooks.execute_array_hooks('afterOperation', [jiffServer, 'crypto_provider', computation_id, from_id, response], 4);
  ...
  jiffServer.handlers.custom = async function (computation_id, from_id, msg) {
    jiffServer.hooks.log(jiffServer, 'custom from', computation_id, '-', from_id, ':', msg);

    try {
      msg = await jiffServer.hooks.execute_array_hooks('beforeOperation', [jiffServer, 'custom', computation_id, from_id, msg], 4);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    msg = await jiffServer.hooks.execute_array_hooks('afterOperation', [jiffServer, 'custom', computation_id, from_id, msg], 4);
    jiffServer.safe_emit('custom', JSON.stringify(msg), computation_id, receiver);

    return { success: true };
  };

  jiffServer.handlers.free = async function (computation_id, party_id, msg) {
    jiffServer.hooks.log(jiffServer, 'free', computation_id, '-', party_id);

    try {
      await jiffServer.hooks.execute_array_hooks('beforeFree', [jiffServer, computation_id, party_id, msg], -1);
    } catch (err) {
      return { success: false, error: typeof(err) === 'string' ? err : err.message };
    }
    ...
    if (Object.keys(jiffServer.computationMaps.freeParties[computation_id]).length === jiffServer.computationMaps.maxCount[computation_id]) {
      ...
      await jiffServer.hooks.execute_array_hooks('afterFree', [jiffServer, computation_id, party_id, msg], -1);
    }
