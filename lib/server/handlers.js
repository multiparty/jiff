...
module.exports = function (jiffServer) {
  ...
  jiffServer.handlers.initializeParty = function (computation_id, party_id, party_count, msg, _s1) {
    ...
    try {
      const params = { party_id: party_id, party_count: party_count };
      const hook_result = jiffServer.hooks.execute_array_hooks('beforeInitialization', [jiffServer, computation_id, msg, params], 3);
    ...
    const keymap_to_send = jiffServer.handlers.storeAndSendPublicKey(computation_id, party_id, msg);
    let message = { party_id: party_id, party_count: party_count, public_keys: keymap_to_send };
  ...
  jiffServer.handlers.storeAndSendPublicKey = function (computation_id, party_id, msg) {
    ...
    const tmp = jiffServer.computationMaps.keys[computation_id];
    if (tmp['s1'] == null) {
      // generate public and secret key for server if they don't exist
      const genkey = jiffServer.hooks.generateKeyPair(jiffServer);
    ...
    const broadcast_message = JSON.stringify({ public_keys: keymap_to_send });

    // Send the public keys to all previously connected parties, except the party that caused this update
    const send_to_parties = jiffServer.computationMaps.clientIds[computation_id];
    for (let i = 0; i < send_to_parties.length; i++) {
  ...
  jiffServer.handlers.crypto_provider = function (computation_id, from_id, msg) {
    ...
    const op_id = msg['op_id'];
    const receivers_list = msg['receivers'];
    const threshold = msg['threshold'];
    const Zp = msg['Zp'];

    // Try to find stored result in map, or compute it if it does not exist!
    let result = jiffServer.cryptoMap[computation_id][op_id];
    if (result == null) {
      const output = jiffServer.cryptoProviderHandlers[label](jiffServer, computation_id, receivers_list, threshold, Zp, params);

      // Share secrets into plain shares (not secret share objects) and copy values
      const shares = {};
      if (output['secrets'] != null) {
        for (let j = 0; j < receivers_list.length; j++) {
          shares[receivers_list[j]] = [];
        }

        for (let i = 0; i < output['secrets'].length; i++) {
          const oneShare = jiffServer.hooks.computeShares(jiffServer, output['secrets'][i], receivers_list, threshold, Zp);
