...
ServerHooks.prototype.beforeInitialization = [async function (jiff, computation_id, msg, params) {
...
// Executor
ServerHooks.prototype.execute_array_hooks = async function (hook_name, params, acc_index) {
  var arr = this[hook_name];
  arr = (arr == null ? [] : arr);
  for (var i = 0; i < arr.length; i++) {
    var result = await arr[i].apply(this.jiff, params);
    if (acc_index > -1) {
