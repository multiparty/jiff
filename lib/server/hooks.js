const shamir_share = require('../client/protocols/shamir/share.js');

const intervals = require('./datastructures/intervals.js');
...
function ServerHooks(jiffServer) {
  ...
  for (let hook in ServerHooks.prototype) {
...
ServerHooks.prototype.beforeInitialization = [
  function (jiff, computation_id, msg, params) {
    const party_count = params.party_count;
...
ServerHooks.prototype.onInitializeUsedId = function (jiff, computation_id, party_id, party_count, msg) {
  ...
  const previous_socket_id = jiff.socketMaps.socketId[computation_id][party_id];
  const previous_socket = jiff.io.sockets.sockets.get(previous_socket_id);
...
ServerHooks.prototype.generateKeyPair = function (jiff) {
  if (jiff.sodium) {
    const key = jiff.sodium.crypto_box_keypair(); // this party's public and secret key
...
ServerHooks.prototype.execute_array_hooks = function (hook_name, params, acc_index) {
  let arr = this[hook_name];
  arr = arr == null ? [] : arr;

  for (let i = 0; i < arr.length; i++) {
    const result = arr[i].apply(this.jiff, params);
    if (acc_index > -1) {
