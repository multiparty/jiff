...
exports.initPrototype = function (JIFFServer) {
  ...
  JIFFServer.prototype.safe_emit = async function (label, msg, computation_id, to_id) {
    if (to_id === 's1' && this.options.external_crypto_provider !== true) {
      // if computation instance is not yet created, store in mailbox.
      if (this.computation_instances_map[computation_id] == null) {
        await this.hooks.putInMailbox(this, label, msg, computation_id, to_id);
      } else {
        // Otherwise, pass on directly to computation instance.
        this.computation_instances_map[computation_id].socket.receive(label, msg);
      }
    } else {
      // store message in mailbox so that it can be resent in case of failure.
      var jiff = this;
      var store_id = await this.hooks.putInMailbox(this, label, msg, computation_id, to_id);
  ...
  JIFFServer.prototype.resend_mailbox = async function (computation_id, party_id) {
    ...
    for (var i = 0; i < mailbox.length; i++) {
      ...
      if (party_id === 's1' && this.options.external_crypto_provider !== true) {
        callback.call(this, store_id);
        // computation instance must exist if we are resending its mailbox.
        this.computation_instances_map[computation_id].socket.receive(label, msg);
      } else {
