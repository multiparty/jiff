  ...
  JIFFServer.prototype.initSocket = function () {
    ...
    this.io.on('connection', async function (socket) {
      ...
      socket.on('initialization', async function (msg) {
        ...
        var output = await jiff.handlers.initializeParty(computation_id, party_id, party_count, msg, false);
        // END OF COMPUTATION
        ...
        if (output.success) {
          ...
          await jiff.io.to(socket.id).emit('initialization', output.message);
          ...
          await jiff.io.to(socket.id).emit('error', JSON.stringify({label: 'initialization', error: output.error}));
        }
      ...
      socket.on('share', async function (msg, callback) {
        ...
        var output = await jiff.handlers.share(computation_id, from_id, json_msg);
        if (!output.success) {
      ...
      socket.on('open', async function (msg, callback) {
        ...
        var output = await jiff.handlers.open(computation_id, from_id, json_msg);
        if (!output.success) {
      ...
      socket.on('custom', async function (msg, callback) {
        ...
        var output = await jiff.handlers.custom(computation_id, from_id, json_msg);
        if (!output.success) {
      ...
      socket.on('crypto_provider', async function (msg, callback) {
        callback(true); // send ack to client
        ...
        var res = await jiff.handlers.crypto_provider(computation_id, from_id, msg);
        if (!res.success) {
      ...
      socket.on('disconnect', async function (reason) {
        ...
        await jiff.hooks.execute_array_hooks('onDisconnect', [jiff, computation_id, from_id], -1);
      ...
      socket.on('free', async function (msg, callback) {
        ...
        var output = await jiff.handlers.free(computation_id, from_id, msg);
        if (!output.success) {
