...
In this tutorial, we'll look at an efficient implementation of the standard deviation for floating-point numbers.

# Setting up a simple server
We're going to stick with our simple server-as-message-router model from before. The `express` package provides a web 
framework, which we want to use over `http`. This code goes in the file `server.js`.
...
To sum the secret-shared, we use the secret addition function, `sadd`
```javascript
var in_sum = shares[1];
...
```
`compute` function looks like this:
